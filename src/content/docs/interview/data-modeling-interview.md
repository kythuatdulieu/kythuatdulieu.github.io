...
* **Luôn sử dụng [Surrogate Key](/concepts/2-storage/data-warehouse/surrogate-key/) (Khóa nhân tạo)**: Tuyệt đối không dùng các ID của hệ thống nguồn (như UUID dạng string) làm khóa chính cho các bảng Dimension. Hãy tự tạo một khóa tự tăng kiểu số nguyên (Integer/BigInt). Khóa nhân tạo giúp tối ưu hiệu năng của các phép JOIN và là bắt buộc nếu bạn muốn lưu vết lịch sử thay đổi dữ liệu (SCD).
