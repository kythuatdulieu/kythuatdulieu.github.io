...
Bất kể bạn đang làm việc trên cơ sở dữ liệu truyền thống (PostgreSQL, MySQL) hay các kho dữ liệu đám mây hiện đại (BigQuery, Snowflake), việc tối ưu hiệu năng luôn xoay quanh 4 nguyên tắc cốt lõi sau:
...
* **Tổ chức dữ liệu vật lý thông minh (Data Organization)**: Phân chia dữ liệu trên ổ đĩa thông qua cơ chế Phân vùng (Partitioning - chia thành các thư mục nhỏ) và gom cụm (Clustering / Z-Ordering - xếp các dòng có giá trị tương tự nằm gần nhau). Việc này giúp công cụ truy vấn có thể bỏ qua (skip) hàng loạt tệp tin không liên quan khi tìm kiếm.
* **Đánh chỉ mục hợp lý (Indexing)**: Sử dụng các cấu trúc dữ liệu bổ trợ như B-Tree Index (phù hợp cho các hệ thống giao dịch OLTP) hoặc Bitmap Index (phù hợp cho các hệ thống phân tích OLAP) để thay thế phép quét toàn bộ bảng (Full Table Scan) bằng các phép tìm kiếm nhị phân có độ phức tạp thấp $O(\log n)$.
