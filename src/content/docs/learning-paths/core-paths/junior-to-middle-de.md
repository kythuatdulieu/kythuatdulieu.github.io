...
Khi lượng dữ liệu lên tới hàng triệu hay hàng tỷ dòng (Big Data), việc quét toàn bộ bảng để lấy thông tin là một "thảm họa" về chi phí và tốc độ. Lúc này, **[Partitioning](/concepts/3-storage-engines-formats/partitioning)** và **[Clustering](/concepts/3-storage-engines-formats/clustering)** là hai kỹ thuật sống còn *(theo Community Tech Alliance, Milvus.io)*. Bạn cần làm chủ kỹ thuật chia ngang dữ liệu thành các "thư mục" hay phân vùng vật lý riêng biệt (thường theo cột `date`/`timestamp`), cho phép engine bỏ qua (prune) các phần dữ liệu không cần thiết khi truy vấn. Bên cạnh đó, bạn cần áp dụng phân cụm thông minh (Clustering) - sắp xếp dữ liệu bên trong các phân vùng dựa trên các cột có cardinality cao (độ phân tán lớn, ví dụ: `user_id`), giúp các thuật toán tìm kiếm cục bộ (local lookups) diễn ra cực kỳ nhanh trên cơ sở dữ liệu và các tệp định dạng như Parquet.

### Bước 4: Đồng bộ dữ liệu thời gian thực với Change Data Capture (CDC)
Trong thực tế, bạn không thể cứ sử dụng phương pháp quét toàn bộ bảng (full-table load) để lấy dữ liệu mới mỗi giờ vì nó vốn gây quá tải cho database nguồn. Bạn cần tìm hiểu cơ chế **CDC ([Change Data Capture](/concepts/2-data-ingestion-integration/change-data-capture))** để tối ưu quy trình Extract *(theo Databricks, Dev.to)*. CDC hoạt động bằng cách đọc trực tiếp từ transaction logs (như Write-Ahead Logs - WAL) hoặc binlogs để chỉ bắt các thay đổi ở mức dòng (`INSERT`, `UPDATE`, `DELETE`) từ hệ thống vận hành ([OLTP](/concepts/3-storage-engines-formats/oltp)). Đây là chìa khóa để xây dựng các pipeline dữ liệu đồng bộ liên tục theo thời gian thực (real-time streaming) hoặc độ trễ thấp (near real-time).

### Bước 5: Làm chủ kho dữ liệu đám mây (Cloud Data Warehouse)
Hãy bắt tay vào thực hành trên các nền tảng kho dữ liệu hiện đại và phổ biến nhất hiện nay như [Google BigQuery](/concepts/3-storage-engines-formats/google-bigquery) hoặc [Snowflake](/concepts/3-storage-engines-formats/snowflake). Bạn sẽ cần học cách vận hành, phân quyền và tối ưu hóa các câu truy vấn phân tích ([OLAP](/concepts/3-storage-engines-formats/olap)) trên các kiến trúc tính toán phân tán này.
