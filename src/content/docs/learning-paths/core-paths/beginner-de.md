...
* **Kỹ sư phần mềm (Software Engineer)** hoặc **Chuyên viên phân tích dữ liệu (Data Analyst)** muốn chuyển mình sang thế giới của kỹ thuật dữ liệu ([Data Engineering](/concepts/1-distributed-systems-architecture/data-engineering)).
...
Cuối cùng, bạn sẽ học cách hình dung và vẽ nên một đường ống dữ liệu ([Data Pipeline](/concepts/1-distributed-systems-architecture/data-pipeline)). Hãy hiểu rõ luồng đi của dữ liệu từ nguồn phát sinh (`Source`), qua các quá trình ETL/ELT (Trích xuất - Extract, Tải - Load, Biến đổi - Transform) để đưa đến điểm lưu trữ đích (`Destination`). Khi nhắc đến điều phối (orchestration) các luồng dữ liệu này, **Apache Airflow** hiện đang là công cụ tiêu chuẩn trong ngành (industry-standard) để lên lịch trình và giám sát hệ thống mà bạn nên làm quen.
