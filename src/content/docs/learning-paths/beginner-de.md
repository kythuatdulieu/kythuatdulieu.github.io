...
* **Kỹ sư phần mềm (Software Engineer)** hoặc **Chuyên viên phân tích dữ liệu (Data Analyst)** muốn chuyển mình sang thế giới của kỹ thuật dữ liệu ([Data Engineering](/concepts/1-foundations/foundation/data-engineering/)).
...
Cuối cùng, bạn sẽ học cách hình dung và vẽ nên một đường ống dữ liệu ([Data Pipeline](/concepts/1-foundations/foundation/data-pipeline/)). Hãy hiểu rõ luồng đi của dữ liệu từ nguồn phát sinh (`Source`), qua quá trình trích xuất (`Extract`), tải dữ liệu (`Load`), và lưu trữ tại điểm đích (`Destination`). Từ đây, tư duy thiết kế hệ thống dữ liệu của bạn sẽ bắt đầu hình thành.
