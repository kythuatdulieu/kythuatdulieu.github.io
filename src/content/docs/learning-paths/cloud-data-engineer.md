...
* Hiểu rõ các nguyên lý thiết kế hệ thống dữ liệu cơ bản, mô hình hóa dữ liệu (data modeling) và các quy trình [ETL](/concepts/3-integration/etl-elt/etl/)/[ELT](/concepts/3-integration/etl-elt/elt/) truyền thống.
...
## Thực chiến: Các dự án E2E Cloud Projects tiêu biểu

Để chứng minh năng lực thực tế và cọ xát với môi trường sản xuất của các nhà cung cấp đám mây lớn, bạn hãy bắt tay xây dựng và thực hành các dự án mẫu sau:

* **AWS**: [Dự án AWS: Real-time Ingestion & Analytics](/projects/aws-e2e-project/)
  * **Mô tả:** Xây dựng luồng streaming từ database nguồn -> AWS MSK (Kafka) -> EMR Spark Streaming -> Apache Iceberg trên S3 -> Glue Catalog -> Athena -> QuickSight.
* **GCP**: [Dự án GCP: Serverless Lakehouse & Modern Data Stack](/projects/gcp-e2e-project/)
  * **Mô tả:** Xây dựng luồng serverless streaming: Event Source -> Pub/Sub -> Dataflow -> BigQuery Storage Write API -> dbt Core -> Cloud Composer (Airflow) -> Looker.
* **Azure**: [Dự án Azure: Enterprise Data Platform](/projects/azure-e2e-project/)
  * **Mô tả:** Ingestion từ on-premise/SaaS -> ADF copy pipeline -> Event Hubs -> Synapse Spark Notebooks -> Delta Lake on ADLS Gen2 -> Serverless SQL pool views -> Power BI DirectQuery.
...
* **Tối ưu hóa chi phí ([Cost Optimization](/concepts/2-storage/cloud-data-platform/cost-optimization/))**: Hãy so sánh chi phí và hiệu năng giữa các giải pháp lưu trữ khác nhau. Bạn đã bao giờ tối ưu hóa thành công một hệ thống dữ liệu đám mây để tiết kiệm chi phí cho công ty chưa?
