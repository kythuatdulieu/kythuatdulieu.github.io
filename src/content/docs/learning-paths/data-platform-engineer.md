...
Lộ trình **Data Platform Engineer** định hướng xây dựng và quản trị hạ tầng dữ liệu dùng chung (Data Platform) cho tổ chức. Nội dung chính tập trung vào các giải pháp quản trị dữ liệu ([Data Governance](/concepts/governance-metadata/data-governance/)), quản lý siêu dữ liệu ([Metadata Management](/concepts/governance-metadata/metadata-management/)), kiểm soát truy cập phân quyền nâng cao ([Access Control](/concepts/governance-metadata/access-control/)) và vận hành hệ thống trên môi trường Kubernetes.
...
Khi dữ liệu phình to, việc kiểm soát ai đang dùng cái gì và dữ liệu có chuẩn chỉnh hay không trở nên sống còn. Bạn cần học cách triển khai và vận hành các giải pháp Data Governance hiện đại hàng đầu như **[Unity Catalog](/concepts/data-warehouse/unity-catalog/)** (Databricks), **Apache Atlas**, hoặc **AWS Lake Formation**. Qua đó, bạn sẽ giúp tổ chức quản lý định danh dữ liệu, áp đặt các quy định về chuẩn hóa dữ liệu, biến kho dữ liệu thô lộn xộn thành một tài sản chung đáng tin cậy.
...
* **Hệ thống theo dõi Data Lineage tự động**: Cài đặt OpenLineage hoặc DataHub để tự động theo dõi dòng chảy của dữ liệu từ hệ thống nguồn PostgreSQL [OLTP](/concepts/database-storage/oltp/) xuyên suốt qua các bước biến đổi cho đến kho dữ liệu phân tích BigQuery hoặc [Snowflake](/concepts/cloud-data-platform/snowflake/).
...
* **Kiến trúc [Data Catalog](/concepts/governance-metadata/data-catalog/)**: Trình bày giải pháp thiết kế một Data Catalog trung tâm giúp đồng bộ hóa mượt mà thông tin giữa nhiều công nghệ lưu trữ dữ liệu khác nhau.
* **Bảo mật thông tin nhạy cảm (PII)**: Làm thế nào để triển khai các chính sách bảo vệ dữ liệu cá nhân nhạy cảm ngay từ lớp nạp dữ liệu đầu vào ([Data Ingestion](/concepts/etl-elt/data-ingestion/)/Source) mà không làm ảnh hưởng đến hiệu năng xử lý chung?
