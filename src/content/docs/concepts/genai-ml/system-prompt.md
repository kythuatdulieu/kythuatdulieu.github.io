...
4. **Cung cấp ngữ cảnh nền (Context Grounding):** Trong các ứng dụng [RAG](/concepts/genai-ml/rag/) (Retrieval-Augmented Generation), tài liệu tìm kiếm được từ cơ sở dữ liệu sẽ được nhúng trực tiếp vào System Prompt để LLM dựa vào đó trả lời một cách chính xác.
...
**System Prompt Xuất sắc (Sử dụng kỹ thuật phân tách cấu trúc):**```text
...
* **Ngốn [Token](/concepts/genai-ml/token/) cơ sở:** System Prompt càng dài thì số lượng Token Input đầu vào càng lớn. Do cấu trúc hội thoại, System Prompt sẽ được gửi kèm trong **mọi câu chat tiếp theo** của phiên làm việc, dẫn đến chi phí sử dụng API tăng lên nhanh chóng (trừ khi bạn sử dụng các tính năng Cache Prompt mới hỗ trợ từ nhà cung cấp).
...
**Bắt buộc dùng khi:**
* Bạn xây dựng các ứng dụng AI cấp độ Product thực tế (Chatbot chăm sóc khách hàng, hệ thống RAG doanh nghiệp, [AI Agent](/concepts/genai-ml/ai-agent/) tự động hóa).
