...
Trong thế giới của các Mô hình Ngôn ngữ Lớn ([LLM](/concepts/genai-ml/llm/)), có một tham số vô cùng quyền lực giúp bạn quyết định xem AI sẽ trả lời một cách cứng nhắc, chính xác như một cuốn sách giáo khoa, hay bay bổng, đầy tính sáng tạo như một nhà thơ. Tham số đó được gọi là **Temperature (Nhiệt độ)**. 
...
* **Nhiệt độ thấp ($T = 0$ - Logic & Chính xác):** Hoàn toàn không có sự sáng tạo. Phù hợp cho các tác vụ hỏi đáp dữ liệu thực tế (QA), trích xuất thông tin, viết mã nguồn, và kiến trúc [RAG](/concepts/genai-ml/rag/). Mức này giúp hạn chế tối đa hiện tượng Ảo giác ([Hallucination](/concepts/genai-ml/hallucination/)) của AI.
