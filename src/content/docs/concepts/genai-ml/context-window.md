...
Khi tương tác với các mô hình ngôn ngữ lớn ([LLM](/concepts/genai-ml/llm/)) như GPT-4, Claude hay Llama, bạn có bao giờ thắc mắc làm thế nào AI có thể nhớ được những câu nói trước đó của bạn trong cuộc hội thoại? Hay tại sao khi bạn gửi một tài liệu quá dài, AI lại báo lỗi hoặc đột ngột quên mất yêu cầu ban đầu? Câu trả lời nằm ở khái niệm **Context Window (Cửa sổ ngữ cảnh)** – một thông số kỹ thuật quyết định trực tiếp đến dung lượng bộ nhớ tạm thời của mô hình.
...
* **Đầu vào + Đầu ra = Tổng dung lượng**: Giới hạn của cửa sổ ngữ cảnh bao gồm cả phần câu hỏi của người dùng `(prompt)`, tài liệu đính kèm thêm (ví dụ qua [RAG](/concepts/genai-ml/rag/)) và cả câu trả lời mà AI sẽ viết ra. Nếu prompt đầu vào của bạn đã chiếm tới 99% giới hạn, mô hình sẽ chỉ còn 1% không gian để viết câu trả lời.
...
* **[System prompt](/concepts/genai-ml/system-prompt/) (chỉ thị hệ thống)**: 100 tokens.
...
## Sự đánh đổi giữa Cửa sổ Lớn và Nhỏ

### Khi sử dụng Context Window lớn
* **Ưu điểm**: Cho phép mô hình phân tích trực tiếp các tài liệu dài (sách, mã nguồn, báo cáo tài chính) mà không cần chia cắt nhỏ. Hỗ trợ việc huấn luyện tức thì qua ví dụ `(In-context learning)` rất mạnh mẽ.
* **Nhược điểm**: Chi phí gọi API tăng vọt (do tính tiền theo token), thời gian phản hồi câu đầu tiên `(Time To First Token)` bị kéo dài, và dễ gặp hiện tượng trôi dạt thông tin.

### Khi sử dụng Context Window nhỏ
* **Ưu điểm**: Thời gian phản hồi cực nhanh, tiết kiệm tối đa chi phí tài nguyên và tài chính.
* **Nhược điểm**: Dễ bị mất bối cảnh hội thoại khi người dùng chat quá dài, không thể xử lý các tài liệu phức tạp trực tiếp.

## Khi nào cần sử dụng Context Window lớn?
...
## Góc phỏng vấn: Thử thách tư duy kỹ thuật
