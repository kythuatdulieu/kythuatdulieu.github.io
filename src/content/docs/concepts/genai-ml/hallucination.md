...
* **Ngôn ngữ ít tài nguyên và từ khóa hiếm (Long-tail knowledge)**: Đối với các từ khóa ít xuất hiện hoặc các ngôn ngữ có ít tài liệu trên mạng (như tiếng Việt), mô hình không có đủ liên kết thống kê vững chắc. Khi đó, nó sẽ tự động "lắp ghép bừa" các [token](/concepts/genai-ml/token/) để tạo nên câu trả lời.
* **Khao khát làm hài lòng người dùng (Sycophancy)**: Quá trình huấn luyện tinh chỉnh ([RLHF](/concepts/genai-ml/rlhf/)) dạy cho mô hình bài học: *"Hãy luôn tỏ ra hữu ích và cố gắng trả lời người dùng"*. Điều này vô tình tạo ra áp lực khiến mô hình thà bịa ra một câu trả lời nghe có vẻ thuyết phục còn hơn là trung thực nói *"Tôi không biết"*.
* **Tham số sáng tạo ([Temperature](/concepts/genai-ml/temperature/)) quá cao**: Việc thiết lập tham số Nhiệt độ (Temperature) lớn sẽ bẻ cong phân phối xác suất dự đoán từ, buộc mô hình phải chọn các từ có xác suất thấp hơn để tăng tính phong phú, từ đó dễ dẫn đến sai lệch thông tin thực tế.
...
* Lập trình các [AI Agent](/concepts/genai-ml/ai-agent/) có khả năng thực hiện hành động trên môi trường thực tế (như tự động gửi email cho đối tác hoặc dọn dẹp hệ thống).
