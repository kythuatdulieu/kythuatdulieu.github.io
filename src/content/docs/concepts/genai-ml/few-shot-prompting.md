...
Khi bạn gửi prompt chứa các ví dụ, mô hình không hề thay đổi hay cập nhật bất kỳ trọng số (weights) nào trong "não bộ" của nó. Thay vào đó, thông qua cơ chế Attention, nó sẽ phân tích cấu trúc, ngữ điệu và khuôn mẫu của các ví dụ nằm trong khung cửa sổ ngữ cảnh ([Context Window](/concepts/genai-ml/context-window/)) hiện tại, từ đó dự đoán chuỗi từ tiếp theo sao cho khớp với khuôn mẫu đó nhất. Khi hội thoại kết thúc, toàn bộ "kiến thức" tạm thời này cũng sẽ biến mất.
...
* **Chọn số lượng ví dụ vừa đủ**: Thông thường, 3 đến 5 ví dụ là điểm ngọt (sweet spot) mang lại hiệu quả tối ưu nhất. Việc đưa vào quá nhiều ví dụ sẽ ngốn nhiều [token](/concepts/genai-ml/token/) gửi đi và làm chậm thời gian phản hồi của API mà không cải thiện độ chính xác là bao.
...
### 2. Nếu áp dụng Few-shot Prompting with 5 ví dụ nhưng mô hình vẫn trả về kết quả sai định dạng hoặc sai logic, bạn sẽ xử lý thế nào?
...
1. [Language Models are Few-Shot Learners](https://arxiv.org/abs/2005.14165) - Tom B. Brown et al. (The original NeurIPS 2020 paper introducing GPT-3 and few-shot prompting).
2. [Prompt Engineering Guide: Few-shot Prompting](https://www.promptingguide.ai/techniques/fewshot) - Detailed guides and examples of few-shot prompting by DAIR.AI.
3. [OpenAI Cookbook: Few-shot Prompting Examples](https://cookbook.openai.com/) - OpenAI's repository of patterns, recipes, and prompt engineering examples.
4. [In-Context Learning in Large Language Models: A Survey](https://arxiv.org/abs/2301.00234) - Qingxiu Dong et al. (Research survey explaining the mechanisms of In-context Learning).
5. [Anthropic Prompt Engineering Documentation](https://docs.anthropic.com/en/docs/build-with-claude/prompt-engineering/overview) - Official guide on prompt engineering techniques including system prompts and few-shot examples.
