...
2. **Cơ chế truyền tin của Ingestion (Message Queues)**: Các hệ thống phân phối tin nhắn lớn như Apache Kafka hay Google Pub/Sub thường sử dụng cơ chế phân phối **At-least-once** (đảm bảo tin nhắn được gửi đi ít nhất một lần). Điều này có nghĩa là việc một tin nhắn bị gửi lặp lại 2-3 lần trong điều kiện mạng chập chờn là hành vi bình thường của hệ thống.
3. **Lỗi thiết kế pipeline (Non-idempotent Pipelines)**: Đường ống dẫn dữ liệu bị lỗi giữa chừng và khi chạy lại, do thiết kế không tuân thủ tính lũy đẳng (Idempotency), hệ thống tiếp tục ghi đè/nạp thêm (Append) đè lên tập dữ liệu cũ đã nạp trước đó.
...
* **Khử trùng lặp càng sớm càng tốt (Shift Left)**: Hãy cố gắng thực hiện lọc trùng lặp ngay tại cửa ngõ vào (như tầng Staging hoặc tầng Silver trong Data Lakehouse) để ngăn không cho rác dữ liệu lan truyền xuống các bảng báo cáo phía sau.
...
* **Luôn có tiêu chí sắp xếp rõ ràng**: Khi khử trùng lặp, bắt buộc phải có cột sắp xếp tất định (như `updated_at`, `timestamp` hoặc số phiên bản `version_id`) để dbt/SQL luôn chọn đúng bản ghi mới nhất. Tránh sắp xếp theo các cột ngẫu nhiên vì nó sẽ làm mất đi tính nhất quán của dữ liệu sau mỗi lần chạy pipeline.
...
### Đánh đổi (Trade-offs)
* **Tài nguyên tính toán**: Việc thực hiện chia nhóm (`PARTITION BY`) và sắp xếp (`ORDER BY`) trên các bảng dữ liệu khổng lồ (hàng tỷ dòng) sẽ ép hệ thống tính toán phân tán (như Spark hay BigQuery) thực hiện thao tác **Shuffle** truyền tải dữ liệu qua mạng lưới các node. Việc này cực kỳ ngốn CPU/RAM và chi phí cloud.
* **Độ trễ của pipeline**: Thêm các bước khử trùng lặp phức tạp sẽ làm tăng thời gian chạy của các job ETL/ELT.
...
1. **[Data Quality](/concepts/data-quality)** - Hiểu về triết lý chất lượng dữ liệu.
