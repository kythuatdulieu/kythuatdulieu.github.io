...
```mermaid
graph LR
    subgraph optimize [OPTIMIZE: Bin-packing]
    direction LR
    A1(File 10KB) --> MEM[Spark Memory]
    A2(File 50KB) --> MEM
    A3(File 15KB) --> MEM
    MEM --> B(File Mới 1GB)
    end
    
    subgraph vacuum [VACUUM: Dọn dẹp Tombstones]
    direction LR
    C1[File Mới 1GB] -.->|Active| LOG[(Delta Log)]
    C2[File Cũ 10KB] -.->|Tombstone| LOG
    C2 -->|Sau 7 ngày| DEL((Xóa cứng khỏi S3))
    end
```
