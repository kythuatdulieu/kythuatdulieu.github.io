...
Cơ chế nội bộ (Under the hood), Spark tính toán Z-value bằng cách chuyển đổi giá trị các cột sang nhị phân và đan xen chúng (interleave).
...
## 4. Rủi ro Vận hành (Real-world Incidents)

Trong thực tế triển khai, Z-Order mang theo các rủi ro đáng kể về chi phí và hiệu năng nếu không được kiểm soát:

### Incident 1: Write Amplification (Bùng nổ I/O Ghi)
Để tính toán đường cong Z toàn cục, Spark buộc phải kích hoạt **Network Shuffle** trên diện rộng. 
- **Hiện tượng:** Chi phí DBU (Compute) của Databricks tăng vọt sau khi thiết lập cronjob chạy `OPTIMIZE ZORDER BY` hàng đêm trên các bảng quy mô Petabyte.
...
### Incident 2: Dimensionality Decay (Pha loãng chiều dữ liệu)
Khi bạn Z-Order quá nhiều cột, tính hiệu quả của thuật toán sẽ giảm sút do dữ liệu bị phân tán trên không gian đa chiều lớn. 
- **Hiện tượng:** Thời gian thực thi truy vấn chậm dần dù đã chạy Z-Order.
- **Cách khắc phục:** Nguyên tắc thiết kế (Best practice) là **không Z-Order quá 4 cột**. Chỉ chọn những cột thường xuyên xuất hiện trong mệnh đề `WHERE` của các "Slow Queries".
...
1. **Tuning `targetFileSize`**: Mặc định Databricks nhắm mục tiêu 1GB/tệp. Nếu hệ thống của bạn (như Athena, Presto) thích các file nhỏ hơn để nạp song song, hãy giảm xuống:
   ...
   ```
2. **Kiểm soát Indexed Columns**: Theo mặc định, Delta Lake chỉ thu thập Metadata (Min/Max) cho **32 cột đầu tiên** của schema. Nếu bạn Z-Order một cột nằm ở vị trí thứ 40, Data Skipping sẽ không hoạt động. Hãy cấu hình lại:
   ```sql
   ALTER TABLE events SET TBLPROPERTIES (
