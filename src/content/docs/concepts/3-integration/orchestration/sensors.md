...
definition: "Sensors là một loại Operator đặc biệt trong Apache Airflow có nhiệm vụ liên tục kiểm tra (poll) một điều kiện bên ngoài cho đến khi nó được thỏa mãn."
---
...
Hãy tưởng tượng bạn thỏa thuận với đối tác rằng họ sẽ gửi file dữ liệu vào lúc 1h sáng. Bạn đặt lịch [DAG](/concepts/3-integration/orchestration/dag/) chạy lúc `01:05`. Nhưng thực tế, có hôm đối tác gặp sự cố và file chỉ đến lúc `01:30`.
...
![Kiến trúc điều phối và trạng thái ngủ của Sensor trong Airflow](/images/sensors/mwaa-architecture.png)
```mermaid
sequenceDiagram
...
### Điểm mạnh (Pros)
* **Tự động hóa luồng dữ liệu bất đồng bộ:** Giúp pipeline có khả năng tự động chờ đợi tài nguyên bên ngoài mà không cần lập lịch tĩnh mạo hiểm.
* **Tối ưu hạ tầng (Reschedule Mode):** Cho phép giải phóng tài nguyên Worker Slot khi ngủ, giúp chạy song song hàng ngàn tác vụ chờ mà không gây nghẽn.
* **Tiết kiệm tối đa tài nguyên với Async Sensors:** Deferrable Operators gánh hàng ngàn luồng chờ nhờ cơ chế async trong một tiến trình duy nhất.

### Điểm yếu (Cons)
* **Nguy cơ Deadlock ở chế độ Poke:** Dễ làm cạn kiệt Worker Slots nếu cấu hình sai chế độ mặc định (`poke`).
* **Độ trễ phản hồi (Polling Latency):** Phụ thuộc vào `poke_interval`, có thể gây ra độ trễ giữa thời điểm dữ liệu sẵn sàng và lúc Sensor phát hiện.

## Khi nào nên dùng và không nên dùng

### Khi nào nên dùng
* **Chờ file dữ liệu từ đối tác:** Chờ file xuất hiện trên S3/GCS hoặc hệ thống FTP.
* **Ràng buộc chéo giữa các DAG:** Sử dụng `ExternalTaskSensor` để kết nối các pipeline độc lập.
* **Chờ trạng thái hệ thống bên ngoài:** Chờ kết quả truy vấn database (`SqlSensor`) hoặc phản hồi API (`HttpSensor`).

### Khi nào không nên dùng
* **Yêu cầu thời gian thực (Real-time):** Cho các luồng dữ liệu cần xử lý tức thì khi có sự kiện. Thay vào đó hãy dùng mô hình Event-driven Push (S3 Events -> Webhook/Lambda).
* **Cực kỳ thiếu tài nguyên hạ tầng:** Nếu không cấu hình `reschedule` hay `deferrable`, các sensor sẽ nhanh chóng ngốn hết slots chạy.
...
* [Apache Airflow Documentation - Deferrable Operators](https://airflow.apache.org/docs/apache-airflow/stable/authoring-and-scheduling/deferrable.html)
* [Google Cloud Composer - Airflow Sensors overview](https://cloud.google.com/composer/docs/composer-2/composer-overview)
...
* [Snowflake - Intro to Tasks](https://docs.snowflake.com/en/user-guide/tasks-intro)
