...
definition: "Airflow Scheduler là tiến trình trung tâm của Apache Airflow, chịu trách nhiệm quét thư mục DAG, kiểm tra các điều kiện phụ thuộc và lập lịch cho các tác vụ."
---
...
### Điểm mạnh (Pros)
* **Khả năng mở rộng cao (HA Architecture):** Từ phiên bản 2.0, Scheduler hỗ trợ chạy song song nhiều thực thể ở chế độ Active-Active để chia tải và tránh điểm chết duy nhất (SPOF).
* **Tải nóng (Hot Reloading):** Cơ chế quét liên tục giúp nhận diện và hiển thị các cập nhật cấu trúc DAG lên Web UI tự động mà không cần restart dịch vụ.
* **Độ chính xác cao:** Phân tích dependencies ở mức chi tiết thông qua Metadata DB, giúp kiểm soát trạng thái tác vụ cực kỳ chính xác.

### Điểm yếu (Cons)
* **Tiêu tốn CPU (Continuous Parsing Overhead):** Vòng lặp quét DAG liên tục tiêu thụ CPU ngay cả khi không có tác vụ nào chạy.
* **Rủi ro nghẽn do Top-level Code:** Bất kỳ mã nguồn nặng nào nằm ngoài operator (như kết nối DB, gọi API) ở mức top-level đều bị chạy lại mỗi chu kỳ quét, có thể làm sập Scheduler.

## Khi nào nên dùng và không nên dùng

### Khi nào nên dùng
* **Hệ thống ETL/ELT phức tạp:** Khi cần quản lý hàng loạt DAG có cấu trúc phụ thuộc phức tạp, cần cơ chế tự động chạy lại (Retries) và giám sát thời gian chạy (SLAs).
* **Điều phối đa nền tảng:** Khi cần tích hợp và quản lý công việc trên nhiều hệ thống khác nhau (như Spark, Snowflake, dbt) từ một trung tâm điều phối duy nhất.

### Khi nào không nên dùng
* **Hệ thống Streaming/Thời gian thực:** Airflow Scheduler được tối ưu cho mô hình Batch Processing. Không nên dùng Airflow để điều phối với tần suất giây hoặc mili-giây.
* **Tài nguyên phần cứng quá hạn chế:** Máy chủ chạy Scheduler cần tối thiểu 2 vCPU và 4GB RAM để hoạt động ổn định. Nếu hạ tầng quá yếu, nên cân nhắc sử dụng Cron Job đơn giản.
...
* [Apache Airflow Official Documentation - Scheduler](https://airflow.apache.org/docs/apache-airflow/stable/core-concepts/scheduler.html)
* [Google Cloud Composer - Optimize Airflow Scheduler Performance](https://cloud.google.com/composer/docs/composer-2/optimize-scheduler-performance)
