...
metaDescription: "Hiểu sâu về kiến trúc thực thi (Execution Model) trong Apache Spark: vai trò của Driver, Cluster Manager và các Executor trong hệ thống."
definition: "Spark Execution Model định nghĩa cách thức phân chia, điều phối và thực thi các tác vụ tính toán phân tán thông qua bộ ba Driver, Cluster Manager và Executor."
---
...
### Điểm mạnh
* **Khả năng chịu lỗi cao (Fault Tolerance)**: Nhờ có lineage graph và sự giám sát của Driver, các task lỗi sẽ tự động được chạy lại trên node khác.
* **Tương thích đa nền tảng**: Tách biệt Cluster Manager giúp Spark chạy linh hoạt trên Standalone, YARN, Mesos hoặc Kubernetes.
* **Tối ưu hóa tài nguyên**: Cơ chế cấp phát động (Dynamic Allocation) cho phép tự động giải phóng hoặc xin thêm Executor tùy nhu cầu.

### Điểm yếu
* **Nút thắt cổ chai ở Driver**: Nếu thu thập quá nhiều dữ liệu về Driver (ví dụ qua `.collect()`), Driver sẽ bị OOM và làm sập toàn bộ ứng dụng.
* **Chi phí quản lý (Overhead)**: Việc điều phối hàng triệu task nhỏ qua mạng có thể gây nghẽn băng thông và làm chậm quá trình thực thi.

## Khi nào nên dùng

* **Nên dùng khi**: Xử lý dữ liệu quy mô từ trung bình đến rất lớn, yêu cầu các phép biến đổi phức tạp, thuật toán lặp (Machine Learning) hoặc phân tích tương tác thời gian thực.
* **Không nên dùng khi**: Dữ liệu có kích thước quá nhỏ (dưới vài trăm MB) vì chi phí khởi động Executor và Driver sẽ lớn hơn thời gian thực thi thực tế.

## Khái niệm liên quan

* [Apache Spark](/concepts/3-integration/batch-processing/apache-spark/): Công cụ tính toán phân tán hiệu năng cao.
* [Spark Jobs, Stages, Tasks](/concepts/3-integration/batch-processing/spark-jobs-stages-tasks/): Cách Spark phân rã một ứng dụng.
* [Spark Partition](/concepts/3-integration/batch-processing/spark-partition/): Cơ chế phân mảnh dữ liệu của Spark.
...
6. [AWS EMR Spark Guide](https://docs.aws.amazon.com/emr/latest/ReleaseGuide/emr-spark.html) - AWS documentation on EMR cluster resource configuration and Spark properties.
7. [Google Cloud Dataproc Spark Jobs](https://cloud.google.com/dataproc/docs/concepts/jobs/dataproc-architecture) - Google Cloud documentation explaining Dataproc serverless Spark execution model.
