...
definition: "Data Skew (lệch dữ liệu) là hiện tượng dữ liệu phân bố không đều giữa các phân vùng trong tính toán phân tán, khiến một số node quá tải và làm chậm toàn bộ hệ thống."
seoTitle: "Data Skew là gì? Cách xử lý lệch dữ liệu trong Big Data & Spark"
metaDescription: "Tìm hiểu hiện tượng Data Skew (lệch dữ liệu) trong Spark làm treo hệ thống, cách nhận biết qua Spark UI và các phương pháp giải quyết như Salting, Broadcast."
---
...
3. **So sánh lượng dữ liệu truyền tải ([Shuffle](/concepts/3-integration/batch-processing/shuffle/) Read Size)**: Nếu các Task thông thường chỉ đọc vài Megabytes dữ liệu, nhưng có một vài Task cá biệt phải "gánh" tới hàng chục Gigabytes.
...
## Điểm mạnh (Pros) & Điểm yếu (Cons) của các giải pháp xử lý Data Skew

| Phương pháp | Điểm mạnh (Pros) | Điểm yếu (Cons) |
| :--- | :--- | :--- |
| **Lọc NULL (Filter)** | • Cực kỳ nhanh và đơn giản.<br>• Giảm lượng dữ liệu shuffle đáng kể. | • Chỉ áp dụng được nếu logic nghiệp vụ cho phép bỏ qua NULL (như Inner Join). |
| **Broadcast Join** | • Tránh hoàn toàn giai đoạn Shuffle.<br>• Hiệu năng cực cao. | • Chỉ dùng được khi có một bảng nhỏ.<br>• Dễ gây OOM nếu bảng được broadcast vượt quá dung lượng bộ nhớ. |
| **Key Salting** | • Xử lý triệt để skew khi cả hai bảng đều siêu lớn.<br>• Tận dụng tối đa sức mạnh phân tán. | • Code phức tạp, khó debug.<br>• Tăng chi phí I/O và bộ nhớ do phải nhân bản dữ liệu bảng phụ. |
| **AQE (Skew Join)** | • Tự động hóa hoàn toàn ở runtime.<br>• Không cần thay đổi code của lập trình viên. | • Chỉ hoạt động cho một số dạng Join.<br>• Không hỗ trợ Group By hay Window Function. |

---

## Khi nào nên dùng & Khi nào không nên dùng

### Khi nào nên áp dụng các kỹ thuật xử lý Data Skew?
* **Khi phát hiện Task chạy không đồng đều rõ rệt**: Trên Spark UI, nếu thời gian chạy của task lâu nhất gấp hàng chục hay hàng trăm lần trung vị (Median) các task khác.
* **Khi gặp lỗi OOM (Out of Memory)**: Đặc biệt là ở các Stage thực hiện Shuffle (JOIN, GROUP BY) do một vài executor phải ôm lượng dữ liệu quá lớn vượt quá RAM vật lý.
* **Khi tối ưu hóa chi phí vận hành**: Muốn chạy các job Big Data một cách ổn định, tránh tình trạng job bị treo lâu gây lãng phí tài nguyên cloud (EC2, EMR).

### Khi nào không nên lạm dụng/can thiệp thủ công?
* **Khi dữ liệu chênh lệch không đáng kể**: Nếu thời gian xử lý giữa các node chỉ lệch nhau vài giây đến vài phút trên tổng thể job chạy hàng giờ, việc tối ưu hóa thủ công không đem lại nhiều giá trị và chỉ làm tăng độ phức tạp của code.
* **Khi bảng phụ quá lớn để Broadcast**: Tuyệt đối không ép buộc (force) `broadcast()` một DataFrame có kích thước vượt quá bộ nhớ executor, vì điều này chắc chắn sẽ dẫn đến sập hệ thống (OOM).
* **Khi AQE đã tự xử lý tốt**: Từ Spark 3.x, nếu tính năng Adaptive Query Execution hoạt động hiệu quả và tự động cân bằng được các phân vùng bị lệch, ta nên để Spark tự xử lý thay vì tự viết Key Salting thủ công.
...
### 3. Tại sao việc tăng tham số `spark.sql.shuffle.partitions` (ví dụ từ 200 lên 2000) không giải quyết được triệt để vấn đề Data Skew trong Spark?
* **Gợi ý trả lời**: Việc tăng số lượng shuffle partitions chỉ làm tăng số lượng phân vùng tổng thể, giúp chia nhỏ các khóa phân phối đều (keys with normal size). Tuy nhiên, đối với một khóa cụ thể bị lệch dữ liệu nghiêm trọng (skew key), thuật toán băm (Hash Partitioner) luôn định tuyến toàn bộ các bản ghi của cùng một khóa đó về duy nhất một phân vùng. Do đó, node chịu trách nhiệm xử lý phân vùng chứa skew key đó vẫn sẽ bị quá tải hoặc sập nguồn (OOM) như cũ.

### 4. Kỹ thuật "ướp muối" (Key Salting) hoạt động thế nào và cách chọn hệ số muối (salt factor) thích hợp?
* **Gợi ý trả lời**: Kỹ thuật này hoạt động bằng cách ghép thêm một chuỗi/số ngẫu nhiên (ví dụ từ \\$0$ đến $N-1$) vào khóa JOIN của bảng bị lệch nhằm phân tán dữ liệu của khóa đó ra thành $N$ phần nhỏ hơn. Ở bảng còn lại (bảng không lệch hoặc nhỏ hơn), ta nhân bản mỗi dòng thành $N$ dòng với các hậu tố từ \\$0$ đến $N-1$ tương ứng.
  * Việc chọn hệ số muối $N$ cần cân bằng giữa hiệu quả phân tán tải và tài nguyên tiêu hao. Hệ số muối quá nhỏ (ví dụ $N = 2$) có thể không đủ phân tán tải, trong khi hệ số muối quá lớn (ví dụ $N > 100$) sẽ gây bùng nổ dữ liệu ở bảng được nhân bản, tốn dung lượng bộ nhớ và băng thông I/O. Thông thường, hệ số muối lý tưởng nằm trong khoảng từ 10 đến 50.

---

## Các khái niệm liên quan

* **[Spark Joins](/concepts/3-integration/batch-processing/spark-joins/)** - Tìm hiểu sâu hơn các cơ chế JOIN trong Spark.
...
4. [High Performance Spark](https://www.oreilly.com/library/view/high-performance-spark/9781491943199/) - Practical guide to optimizing and troubleshooting distributed Spark code, specifically addressing data skew and custom [partitioning](/concepts/2-storage/database-storage/partitioning/), by Holden Karau and Rachel Warren.
