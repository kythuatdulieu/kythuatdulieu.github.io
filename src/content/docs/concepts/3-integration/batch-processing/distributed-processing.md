...
metaDescription: "Khái niệm cốt lõi về hệ thống xử lý phân tán (Distributed Processing) và lý do chúng thay thế siêu máy chủ đơn lẻ trong kỷ nguyên Big Data."
definition: "Xử lý phân tán (Distributed Processing) là phương pháp chia nhỏ một tác vụ tính toán khổng lồ thành nhiều phần và thực thi song song trên một cụm gồm nhiều máy chủ liên kết với nhau."
---
...
Hầu hết các framework xử lý phân tán nổi tiếng (như [Apache Spark](/concepts/3-integration/batch-processing/apache-spark/) hay [Hadoop MapReduce](/concepts/3-integration/batch-processing/mapreduce/)) đều tổ chức hệ thống theo mô hình điều khiển **Master - Worker** (hoặc Driver - Executor):
```mermaid
flowchart TD
...
* **Shuffle (Trao đổi dữ liệu)**: Trong quá trình xử lý, đôi khi các Worker cần nhóm dữ liệu lại theo một tiêu chí nào đó (ví dụ: `GROUP BY` hay `JOIN`). Lúc này, các Worker buộc phải gửi dữ liệu trung gian qua lại cho nhau thông qua mạng lưới nội bộ của cụm. Quá trình trao đổi dữ liệu xuyên suốt các node này được gọi là [Shuffle](/concepts/3-integration/batch-processing/shuffle/).
...
* **Hiện tượng dữ liệu lệch (Data Skew)**: Đây là tình trạng dữ liệu phân bổ không đều giữa các Worker. Ví dụ: Node 1 chỉ phải xử lý 1 Megabyte dữ liệu và xong trong 1 giây, trong khi Node 2 phải gánh 100 Gigabyte dữ liệu. Cả hệ thống sẽ phải ngồi chờ Node 2 hoàn thành, làm mất đi hoàn toàn lợi thế của việc xử lý song song. Tìm hiểu thêm về [Data Skew](/concepts/3-integration/batch-processing/data-skew/).
...
* [Shuffle](/concepts/3-integration/batch-processing/shuffle/)
* [Data Skew](/concepts/3-integration/batch-processing/data-skew/)
* [MapReduce](/concepts/3-integration/batch-processing/mapreduce/)
...
4. [MapReduce: Simplified Data Processing on Large Clusters](https://research.google/pubs/mapreduce-simplified-data-processing-on-large-clusters/) - Google's original research paper on the MapReduce distributed computing model by Jeffrey Dean and Sanjay Ghemawat.
5. [AWS Distributed Computing](https://aws.amazon.com/what-is/distributed-computing/) - Amazon Web Services' guide to understanding distributed architectures and cloud computing.
6. [Google Cloud Learn: Distributed Computing](https://cloud.google.com/learn/what-is-distributed-computing) - Google Cloud's educational resource on distributed systems concepts.
7. [Databricks Glossary: Distributed Processing](https://www.databricks.com/glossary/distributed-processing) - Databricks documentation on key distributed processing architectures and methods.
