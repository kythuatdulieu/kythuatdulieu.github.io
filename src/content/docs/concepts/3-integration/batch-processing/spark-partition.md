...
seoTitle: "Spark Partition là gì? Tối ưu phân vùng trong Apache Spark"
metaDescription: "Tìm hiểu Partition trong Spark: Đơn vị của tính song song (Parallelism) và cách số lượng partition quyết định trực tiếp hiệu suất ứng dụng Big Data."
definition: "Spark Partition là các phân mảnh logic của một tập dữ liệu lớn được lưu trữ phân tán, đóng vai trò là đơn vị cơ bản quyết định tính song song (parallelism) trong Apache Spark."
---

Khi làm việc với các hệ thống xử lý dữ liệu lớn (Big Data) như [Apache Spark](/concepts/3-integration/batch-processing/apache-spark/), tính toán song song chính là chìa khóa giúp chúng ta xử lý hàng Terabyte dữ liệu chỉ trong vài phút. Thế nhưng, làm thế nào Spark chia nhỏ một bảng dữ liệu khổng lồ thành nhiều phần để giao việc cho các máy tính con? 
...
1. **Khi đọc dữ liệu vào (Input Partitions)**:
   ...
   * Nếu đọc từ các dịch vụ Cloud Object Storage (như Amazon S3, Google [Cloud Storage](/concepts/2-storage/cloud-data-platform/cloud-storage/)) dưới định dạng Parquet hoặc CSV, Spark cũng tự động sử dụng cấu hình `spark.sql.files.maxPartitionBytes` (mặc định là 128MB) để bẻ nhỏ các file đầu vào thành các phân vùng tương ứng.
...
### Điểm mạnh
* **Tận dụng tối đa CPU**: Chia dữ liệu thành nhiều partition cho phép chạy song song trên tất cả các core CPU hiện có của cụm máy.
* **Ngăn ngừa lỗi OOM**: Giới hạn kích thước mỗi partition ở mức an toàn (100MB-200MB) giúp tránh quá tải bộ nhớ RAM trên mỗi Executor.
* **Tối ưu hóa đĩa**: Gom file rác hiệu quả trước khi ghi ra lưu trữ ngoài nhờ cơ chế giảm phân vùng thông minh.

### Điểm yếu
* **Chi phí truyền thông mạng**: Thay đổi partition bằng `repartition()` bắt buộc phải thực hiện Shuffle, gây hao phí tài nguyên mạng rất lớn.
* **Quá tải lập lịch (Overhead)**: Số lượng partition quá nhiều (task siêu nhỏ) làm Spark Driver tốn nhiều tài nguyên quản lý.

## Khi nào nên dùng

* **Nên dùng khi**: Cấu hình phân phối dữ liệu cho các tác vụ lớn, xử lý lỗi Data Skew bằng cách phân chia lại khóa, hoặc tối ưu hóa số lượng file đầu ra trước khi lưu trữ.
* **Không nên dùng khi**: Dữ liệu có dung lượng rất nhỏ (dưới vài trăm MB), lúc này cấu hình phân vùng mặc định của Spark hoặc chạy single-node sẽ mang lại hiệu quả tốt hơn.

## Khái niệm liên quan

* [Shuffle](/concepts/3-integration/batch-processing/shuffle/): Cơ chế xáo trộn dữ liệu vật lý.
* [Data Skew](/concepts/3-integration/batch-processing/data-skew/): Hiện tượng lệch phân bố dữ liệu giữa các phân vùng.
* [Spark Execution Model](/concepts/3-integration/batch-processing/spark-execution-model/): Mô hình thực thi tính toán phân tán của Spark.
...
1. [Apache Spark Programming Guide: Key-Value Pairs](https://spark.apache.org/docs/latest/rdd-programming-guide.html#working-with-key-value-pairs) - Official guide detailing data [partitioning](/concepts/2-storage/database-storage/partitioning/), custom partitioners, and key-value transformations.
...
5. [Databricks Performance and Optimization Guide](https://docs.databricks.com/en/optimizations/index.html) - Databricks documentation explaining partition sizing, shuffling strategies, and adaptive query execution.
6. [AWS EMR: Spark Performance Tuning](https://docs.aws.amazon.com/emr/latest/ReleaseGuide/emr-spark-performance.html) - Amazon Web Services' guide on configuring partitions and memory allocations for Spark on EMR.
7. [Google Cloud Dataproc: Spark Tuning](https://cloud.google.com/dataproc/docs/concepts/config/spark-tuning) - Google Cloud documentation on tuning Spark partition sizes for serverless clusters.
