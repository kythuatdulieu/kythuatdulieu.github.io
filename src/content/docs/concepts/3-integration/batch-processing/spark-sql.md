...
definition: "Spark SQL là một phân hệ cốt lõi của Apache Spark dùng để xử lý dữ liệu có cấu trúc bằng cách cung cấp giao diện truy vấn SQL tiêu chuẩn kết hợp DataFrame API và Catalyst Optimizer."
---
...
### Điểm mạnh
* **Tự động tối ưu hóa**: Nhờ Catalyst Optimizer, các câu lệnh SQL/DataFrame được tinh chỉnh tối ưu mà không cần dev can thiệp thủ công.
* **Giao diện thân thiện**: Hỗ trợ SQL tiêu chuẩn và DataFrame API giúp lập trình viên dễ dàng làm quen và sử dụng.
* **Tương thích schema**: Đảm bảo an toàn kiểu dữ liệu và schema rõ ràng, giúp tránh các lỗi logic lúc chạy (run-time errors).

### Điểm yếu
* **Giới hạn tối ưu UDF**: Các hàm UDF tự viết bằng Python là "hộp đen", làm Catalyst mất khả năng tối ưu hóa và giảm hiệu năng.
* **Thiếu linh hoạt kiểu động**: Yêu cầu schema rõ ràng có thể gây khó khăn khi xử lý dữ liệu phi cấu trúc hoặc có cấu trúc thay đổi liên tục.

## Khi nào nên dùng

* **Nên dùng khi**: Thực hiện các tác vụ ETL trên dữ liệu có cấu trúc hoặc bán cấu trúc, xây dựng báo cáo phân tích bằng SQL, hoặc tích hợp các nguồn dữ liệu đa dạng (JSON, Parquet, CSV, Database).
* **Không nên dùng khi**: Dữ liệu hoàn toàn phi cấu trúc (như video, hình ảnh, âm thanh thô) hoặc các bài toán đòi hỏi xử lý luồng byte mức thấp.

## Khái niệm liên quan

* [Apache Spark](/concepts/3-integration/batch-processing/apache-spark/): Bộ máy tính toán phân tán.
* [Spark Execution Model](/concepts/3-integration/batch-processing/spark-execution-model/): Mô hình hoạt động Master-Slave của Spark.
* [Parquet Format](/concepts/2-storage/database-storage/file-formats/): Định dạng file lưu trữ dạng cột tối ưu cho Spark.
...
6. [AWS: Using Spark SQL with EMR](https://docs.aws.amazon.com/emr/latest/ReleaseGuide/emr-spark-sql.html) - AWS guide on utilizing Spark SQL operations and custom metastores on Amazon EMR.
7. [Google Cloud Dataproc: Spark SQL Integration](https://cloud.google.com/dataproc/docs/concepts/connectors/bigquery) - Google Cloud documentation on integrating BigQuery and serverless Spark SQL engines.
