...
definition: "Jobs, Stages và Tasks đại diện cho cấu trúc phân cấp điều phối và thực thi vật lý các tác vụ tính toán phân tán trong một ứng dụng Apache Spark."
---
...
Quá trình chia cắt này được thực hiện bởi một bộ phận đầu não gọi là **[DAG](/concepts/3-integration/orchestration/dag/) Scheduler (Directed Acyclic Graph)**:
...
* **Nên dùng khi**: Cần tối ưu hóa cấu trúc luồng xử lý Spark, gỡ lỗi hiệu năng bằng cách phân tích các stage bị nghẽn (Straggler Tasks) trên Spark UI, hoặc lập kế hoạch partition dữ liệu hợp lý.
* **Không nên dùng khi**: Phép toán không có shuffle (narrow transformation) hoặc dữ liệu cực nhỏ, lúc này cấu trúc phân cấp Spark vẫn được dựng lên nhưng không mang lại nhiều giá trị tối ưu hóa.
...
* [Spark Execution Model](/concepts/3-integration/batch-processing/spark-execution-model/): Mô hình phân bổ tài nguyên Master-Slave.
* [Shuffle](/concepts/3-integration/batch-processing/shuffle/): Cơ chế xáo trộn dữ liệu qua mạng lưới.
* [Spark Partition](/concepts/3-integration/batch-processing/spark-partition/): Các phân mảnh dữ liệu vật lý.
...
6. [AWS: Monitoring Spark Jobs](https://docs.aws.amazon.com/emr/latest/ReleaseGuide/emr-spark-ui.html) - AWS documentation on tracking stages, tasks, and jobs via Spark UI.
7. [Databricks: Spark Jobs and Stages](https://docs.databricks.com/en/compute/index.html) - Databricks guidelines on diagnosing performance using Spark execution stages.
8. [Google Cloud Dataproc: Monitoring Spark Jobs](https://cloud.google.com/dataproc/docs/concepts/jobs/dataproc-architecture) - Google Cloud documentation explaining job and stage architectures on Dataproc.
