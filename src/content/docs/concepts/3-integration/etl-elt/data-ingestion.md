...
definition: "Data Ingestion là quá trình thu thập, vận chuyển và nạp dữ liệu thô từ các nguồn phát sinh ban đầu vào một kho lưu trữ tập trung để sẵn sàng cho phân tích."
...
![Quy trình Ingestion đưa dữ liệu từ các nguồn khác nhau vào hệ thống lưu trữ tập trung](/images/data-ingestion/data-ingestion-flow.png)
...
5. **Nạp dữ liệu**: Ghi khối dữ liệu thô này vào bảng chứa dữ liệu thô (`raw_users`) tại [Data Warehouse](/concepts/2-storage/data-warehouse/data-warehouse/).
...
* **Batch Ingestion**: Nên dùng khi doanh nghiệp chỉ cần cập nhật dữ liệu định kỳ (cuối ngày hoặc theo giờ) để lập báo cáo KPI, báo cáo tài chính hoặc phân tích lịch sử, đồng thời muốn tối ưu hóa chi phí hạ tầng.
* **Streaming Ingestion**: Nên dùng cho các bài toán phân tích thời gian thực yêu cầu độ trễ tính bằng mili-giây như phát hiện gian lận thẻ tín dụng, hệ thống cảnh báo hệ thống hoặc hiển thị thông tin trực tiếp trên ứng dụng khách hàng.
...
* **Gợi ý trả lời**: Chúng ta có 3 giải pháp chính để giải quyết bài toán này:
  1) Chỉ chạy các tác vụ Ingest lớn theo lô (batch) vào khung giờ thấp điểm (đêm muộn) khi lượng truy cập của người dùng ở mức thấp nhất.
...
1. [Fundamentals of Data Engineering](https://www.oreilly.com/library/view/fundamentals-of-data/9781098108298/) - Book by Joe Reis and Matt Housley detail-oriented on the ingestion phase.
2. [Designing Data-Intensive Applications](https://www.oreilly.com/library/view/designing-data-intensive-applications/9781491903063/) - Book by Martin Kleppmann explaining stream processing.
3. [AWS Whitepaper: Streaming Data Ingestion Patterns](https://docs.aws.amazon.com/whitepapers/latest/streaming-data-solutions-amazon-kinesis/streaming-data-solutions-amazon-kinesis.html) - Official AWS architectural guidance on stream ingestion.
4. [Airbyte Documentation: Data Ingestion Architecture](https://docs.airbyte.com/understanding-concepts/connections) - Guide explaining Airbyte's replication process.
5. [Confluent: What is Data Ingestion?](https://www.confluent.io/learn/data-ingestion/) - Comprehensive overview of event streaming ingestion.
6. [Google Cloud: Data Ingestion Patterns](https://cloud.google.com/architecture/data-ingestion-patterns) - Official GCP architecture guide for data ingestion.
7. [Microsoft Learn: Azure Ingestion Guide](https://learn.microsoft.com/en-us/azure/architecture/data-guide/scenarios/data-ingestion) - Microsoft guide on data ingestion services and architectures.

## English Summary

**Data Ingestion** is the foundational process of acquiring and moving data from various disparate sources (databases, APIs, logs) into a centralized storage target ([Data Lake](/concepts/2-storage/data-lake-lakehouse/data-lake/) or Data Warehouse) for downstream analysis. In the context of modern [ELT](/concepts/3-integration/etl-elt/elt/), it focuses purely on secure, scalable transportation (Extract and Load) without applying heavy business logic. Ingestion strategies generally fall into two categories: Batch Ingestion (moving large chunks of data at scheduled intervals, suitable for daily reporting) and Streaming Ingestion (continuously flowing record-by-record, suitable for real-time analytics like fraud detection). Efficient ingestion architectures heavily utilize managed replication tools, event brokers (like Kafka), or CDC to minimize load on operational [source systems](/concepts/1-foundations/foundation/source-systems/).