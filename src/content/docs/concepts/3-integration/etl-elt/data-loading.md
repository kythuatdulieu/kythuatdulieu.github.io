...
definition: "Data Loading là giai đoạn ghi dữ liệu đã trích xuất hoặc biến đổi vào hệ thống lưu trữ đích cuối cùng như Data Warehouse, Data Lake để phục vụ phân tích."
...
Dưới đây là một câu lệnh SQL sử dụng cơ chế **MERGE** (được hỗ trợ bởi các nền tảng lớn như [Snowflake](/concepts/2-storage/cloud-data-platform/snowflake/), BigQuery, [Delta Lake](/concepts/2-storage/data-lake-lakehouse/delta-lake/)) để cập nhật dữ liệu từ bảng tạm (staging) vào bảng chính:
```sql
...
| Chiến lược | Điểm mạnh | Điểm yếu | Trường hợp sử dụng |
| :--- | :--- | :--- | :--- |
...
* **Full Overwrite**: Nên dùng cho các bảng nhỏ (dưới 100MB), bảng cấu hình hoặc bảng danh mục ít thay đổi để đảm bảo tính đồng bộ tuyệt đối với nguồn.
* **Append**: Nên dùng cho dữ liệu bất biến, ghi liên tục theo thời gian thực như log hệ thống, clickstream, dữ liệu cảm biến IoT.
* **Upsert (Merge)**: Nên dùng cho dữ liệu có tính chất cập nhật trạng thái liên tục (ví dụ: thông tin người dùng, trạng thái đơn hàng) nhằm tránh trùng lặp bản ghi và tối ưu dung lượng lưu trữ.
...
1. [Fundamentals of Data Engineering](https://www.oreilly.com/library/view/fundamentals-of-data/9781098108298/) - Book by Joe Reis and Matt Housley detail-oriented on the load phase of the ETL/ELT lifecycle.
2. [Snowflake Documentation: COPY INTO table](https://docs.snowflake.com/en/sql-reference/sql/copy-into-table) - Official reference guide detailing Snowflake's bulk data loading mechanism.
3. [dbt Documentation: Materializations](https://docs.getdbt.com/docs/build/materializations) - Explains table, view, incremental, and ephemeral model strategies.
4. [Databricks Docs: MERGE INTO](https://docs.databricks.com/en/sql/language-manual/sql-ref-syntax-dml-merge.html) - Technical reference on using MERGE INTO in Delta Lake.
5. [AWS Architecture Center: Loading data from Amazon S3](https://docs.aws.amazon.com/redshift/latest/dg/t_Loading-data-from-s3.html) - AWS guide on optimized bulk data loading into Amazon Redshift.
6. [Google Cloud BigQuery: Loading Data Guide](https://cloud.google.com/bigquery/docs/loading-data) - Official GCP documentation on loading data.
7. [Microsoft Learn: Synapse Data Loading Guide](https://learn.microsoft.com/en-us/azure/synapse-analytics/sql-data-warehouse/design-elt-data-loading) - Reference architecture on loading data.
