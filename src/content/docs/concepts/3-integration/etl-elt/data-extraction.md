...
definition: "Data Extraction là bước đầu tiên trong quy trình ETL/ELT nhằm thu thập và trích xuất dữ liệu thô từ nhiều nguồn khác nhau về hệ thống lưu trữ trung tâm."
...
![Sơ đồ phân biệt luồng kiểm soát (Control Flow) và luồng trích xuất dữ liệu (Data Flow)](/images/data-extraction/control-flow-data-flow.png)
...
* **Cách thực hiện**: Ở lần chạy đầu tiên, hệ thống sẽ lấy toàn bộ dữ liệu. Ở các lần chạy tiếp theo, hệ thống sẽ ghi nhớ thời điểm cuối cùng đã trích xuất thành công (gọi là mốc chặn - *High [Watermark](/concepts/4-realtime/streaming-processing/watermark/)*) và chỉ yêu cầu lấy những dòng dữ liệu có cột `updated_at` lớn hơn mốc này.
...
* **Đánh giá**: Việc viết code trích xuất qua API khá phức tạp vì phải xử lý:
  - **Phân trang (Pagination)**: Dữ liệu lớn phải được chia nhỏ ra thành nhiều trang. Code phải tự động lặp để lấy từng trang thông qua các tham số như Limit, Offset hoặc [Token](/concepts/6-ai-ml/genai-ml/token/).
...
| Phương pháp | Điểm mạnh | Điểm yếu | Trường hợp sử dụng |
| :--- | :--- | :--- | :--- |
...
* **Full Extraction**: Nên dùng cho các bảng danh mục nhỏ, dữ liệu cấu hình hoặc bảng có dung lượng dưới 100MB và ít thay đổi.
* **Incremental Extraction**: Nên dùng cho các bảng dữ liệu giao dịch lớn có cột đánh dấu thời gian rõ ràng (ví dụ: `updated_at`, `created_at`).
* **Log-based CDC**: Nên dùng cho các bảng dữ liệu cực lớn, tần suất cập nhật cao, yêu cầu thời gian thực hoặc cần theo dõi các hành động xóa vật lý (hard deletes).
...
* **Gợi ý trả lời**:
  ...
  * **Giải pháp 2 (ELT Pattern)**: Áp dụng mô hình ELT bằng cách trích xuất dữ liệu thô (Raw Data) dưới định dạng bán cấu trúc như JSON/semi-structured, lưu trữ nguyên trạng vào Landing Zone ([Data Lake](/concepts/2-storage/data-lake-lakehouse/data-lake/) hoặc Stage Table của DWH). Việc phân tích cấu trúc cụ thể sẽ được trì hoãn và xử lý bằng các công cụ biến đổi dữ liệu (như [dbt](/concepts/3-integration/transformation-analytics/dbt/)) trong Data Warehouse. Điều này giúp Pipeline trích xuất không bị gián đoạn khi có [Schema Drift](/concepts/5-quality-governance/observability-reliability/schema-drift/) nhẹ.
...
1. [Fundamentals of Data Engineering](https://www.oreilly.com/library/view/fundamentals-of-data/9781098108298/) - Book by Joe Reis and Matt Housley detail-oriented on the extraction layer, [source systems](/concepts/1-foundations/foundation/source-systems/), and connectivity.
...
4. [Google Cloud Data Fusion: Extraction Overview](https://cloud.google.com/data-fusion/docs/concepts/3-integration/etl-elt) - Official GCP documentation on ETL/ELT extraction.
5. [Databricks: Data Ingestion and Extraction](https://docs.databricks.com/ingestion/index.html) - Databricks best practices for loading and extracting source data.
6. [Microsoft Learn: Extract, Transform, and Load (ETL)](https://learn.microsoft.com/en-us/azure/architecture/data-guide/relational-data/etl) - Reference architecture on building secure, resilient data extraction flows in cloud environments.

## English Summary

**Data Extraction** is the initial phase in the ETL/ELT pipeline where raw data is pulled from source systems (Databases, SaaS APIs, Logs) to a centralized storage. Depending on data volume and latency requirements, engineers choose between Full Extraction (replicating the entire table, suitable for small reference data), Incremental Extraction (querying only new/updated rows using timestamps, efficient but misses hard deletes), and Log-based [Change Data Capture](/concepts/3-integration/etl-elt/change-data-capture/) (CDC) (reading database transaction logs for real-time, exact replica streams). Working with external APIs presents additional extraction challenges such as pagination and rate-limiting.