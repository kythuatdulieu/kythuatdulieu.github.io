...
definition: "Idempotency (Tính lũy đẳng) là thuộc tính đảm bảo một tác vụ dù thực thi một hay nhiều lần với cùng một đầu vào thì kết quả cuối cùng nhận được vẫn không thay đổi."
seoTitle: "Tính lũy đẳng (Idempotency) trong thiết kế Data Pipeline"
...
Trong bối cảnh hệ thống phân tán và thiết kế đường ống dữ liệu [ETL](/concepts/3-integration/etl-elt/etl/)/[ELT](/concepts/3-integration/etl-elt/elt/), một pipeline có tính lũy đẳng cho phép các kỹ sư dữ liệu thoải mái chạy lại (rerun hoặc [backfill](/concepts/3-integration/etl-elt/backfill/)) các tác vụ bị lỗi hoặc bị trễ mà không cần phải lo lắng về việc dữ liệu bị hỏng, bị ghi đè sai lệch hay sinh ra các bản ghi trùng lặp (duplicates).

## Tại sao chúng ta cần tính lũy đẳng trong hệ thống dữ liệu?

Trong thế giới thực tế của [Data Engineering](/concepts/1-foundations/foundation/data-engineering/), các đường ống dữ liệu thường xuyên phải đối mặt với vô vàn sự cố ngoài ý muốn:
...
* **Luôn thiết lập Phân vùng dữ liệu ([Partitioning](/concepts/2-storage/database-storage/partitioning/))**: Hãy phân vùng bảng dữ liệu theo thời gian (ví dụ: ngày tạo `created_at` hoặc ngày xử lý `execution_date`). Việc này giúp thao tác xóa và ghi đè (Overwrite) chỉ diễn ra trên một phân vùng nhỏ, tối ưu hiệu năng và tốc độ truy vấn đáng kể.
...
1. [Making retries safe with idempotent APIs](https://aws.amazon.com/builders-library/making-retries-safe-with-idempotent-APIs/) - AWS Builder's Library article on designing systems with safe retries.
...
3. [Confluent Blog: Exactly-Once Semantics in Apache Kafka](https://www.confluent.io/blog/exactly-once-semantics-are-demystified-in-apache-kafka/) - Post on idempotence and exactly-once processing in Kafka.
4. [Databricks Docs: MERGE INTO](https://docs.databricks.com/en/sql/language-manual/sql-ref-syntax-dml-merge.html) - Databricks SQL reference documentation for performing idempotent upserts.
5. [dbt Terms: Idempotency](https://docs.getdbt.com/terms/idempotency) - dbt Labs definition and explanation of idempotency in data modeling.
6. [Google Cloud: Building Idempotent Pipelines](https://cloud.google.com/blog/products/data-analytics/how-to-build-idempotent-pipelines) - GCP engineering practices for building robust idempotent pipelines.
7. [Microsoft Learn: Idempotent Data Pipelines](https://learn.microsoft.com/en-us/azure/architecture/data-guide/relational-data/etl) - Reference architecture on building resilient data pipelines.
