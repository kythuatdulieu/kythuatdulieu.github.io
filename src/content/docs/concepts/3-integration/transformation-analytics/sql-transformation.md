...
definition: "SQL Transformation là quá trình sử dụng ngôn ngữ SQL để làm sạch, chuẩn hóa, kết hợp và tổng hợp dữ liệu trực tiếp trong Data Warehouse theo kiến trúc ELT hiện đại."
...
Trong một đường ống dẫn dữ liệu ([Data Pipeline](/concepts/1-foundations/foundation/data-pipeline/)), quá trình biến đổi (T - Transform) bao gồm các thao tác cơ bản nhưng vô cùng quan trọng:
* **Làm sạch (Cleaning):** Loại bỏ dữ liệu rác, xử lý giá trị NULL và khử trùng lặp ([Deduplication](/concepts/3-integration/etl-elt/deduplication/)).
...
> [!NOTE]
> Công cụ [dbt (data build tool)](/concepts/3-integration/transformation-analytics/dbt/) ra đời chính là để giúp chúng ta quản lý cấu trúc chia tầng này một cách chuyên nghiệp và tự động.
...
### Điểm mạnh (Pros)
* **Data Democratization**: Sử dụng ngôn ngữ SQL phổ biến, giúp các Data Analysts dễ dàng tham gia xây dựng và quản lý pipeline dữ liệu.
* **Push-down Compute**: Thực thi trực tiếp trên Data Warehouse, loại bỏ chi phí và độ trễ khi di chuyển dữ liệu ra ngoài.
* **High Performance**: Tận dụng tối đa kiến trúc xử lý song song và tối ưu hóa phần cứng của Cloud Data Warehouses hiện đại.

### Điểm yếu (Cons)
* **Structured Data Limitation**: Khá cồng kềnh và khó bảo trì khi xử lý các cấu hình dữ liệu phi cấu trúc phức tạp (như lồng ghép JSON sâu).
* **No OOP Support**: SQL thiếu các tính năng lập trình hướng đối tượng, khiến việc tái sử dụng code thô gặp nhiều khó khăn nếu không có công cụ bổ trợ như dbt.
...
### Nên áp dụng khi:
* Logic biến đổi chủ yếu xoay quanh dữ liệu dạng bảng (Tabular data) từ các nguồn quan hệ phổ biến.
* Muốn tối ưu hóa hiệu năng và chi phí bằng cách tận dụng sức mạnh tính toán của Cloud Data Warehouses.

### Chưa nên áp dụng khi:
* Cần xử lý dữ liệu phi cấu trúc quy mô lớn (hình ảnh, âm thanh, video) hoặc thực hiện các tác vụ Machine Learning phức tạp.
* Cần xử lý luồng dữ liệu thời gian thực (real-time streaming) với độ trễ tính bằng mili-giây.
...
1. Google Cloud Architecture: [Data transformation using BigQuery SQL](https://cloud.google.com/bigquery/docs/reference/standard-sql/dml-syntax)
2. Snowflake Documentation: [Transforming data with SQL in Snowflake](https://docs.snowflake.com/en/sql-reference-commands)
3. Databricks Guide: [Data transformation best practices with Spark SQL](https://docs.databricks.com/en/sql/index.html)
4. AWS Big Data Blog: [Transform data with SQL on Amazon Redshift](https://aws.amazon.com/blogs/big-data/transforming-data-with-amazon-redshift/)
5. Apache Spark Documentation: [Spark SQL guide and reference](https://spark.apache.org/docs/latest/sql-programming-guide.html)
