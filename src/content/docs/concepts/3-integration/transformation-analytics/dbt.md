...
definition: "dbt (data build tool) là công cụ mã nguồn mở chuyên trách việc chuyển đổi dữ liệu (Transform) trong kiến trúc ELT bằng cách biên dịch và chạy các model SQL trên Data Warehouse."
...
**dbt (data build tool)** là một công cụ mã nguồn mở được phát triển bởi dbt Labs, đảm nhận chuyên biệt chữ **"T" (Transform - Biến đổi)** trong kiến trúc dữ liệu [ELT](/concepts/3-integration/etl-elt/elt/) (Extract, Load, Transform) hiện đại.
...
* **Quá trình**: dbt biên dịch các file này thành mã SQL chuẩn, tính toán thứ tự chạy giữa các bảng (vẽ đồ thị phụ thuộc [DAG](/concepts/3-integration/orchestration/dag/)) và gửi các câu lệnh này xuống [Data Warehouse](/concepts/2-storage/data-warehouse/data-warehouse/) của bạn.
...
![Kiến trúc dbt vận hành trong hệ thống Modern Data Stack](/images/dbt/dbt-architecture.png)
```mermaid
graph LR
...
* **Sử dụng dbt làm công cụ nạp dữ liệu ([ETL](/concepts/3-integration/etl-elt/etl/))**: Cố gắng cấu hình dbt để gọi API hoặc đọc file Excel bên ngoài. dbt sinh ra không phải để làm tác vụ nạp dữ liệu (Extract/Load). Hãy để các công cụ chuyên dụng như Airbyte hoặc Fivetran làm việc đó, sau khi dữ liệu đã nằm gọn trong kho, dbt mới bắt đầu tham gia biến đổi.

## Điểm mạnh và điểm yếu

### Điểm mạnh (Pros)
* **Software Engineering Best Practices**: Đưa Git version control, testing, và CI/CD vào quy trình phân tích dữ liệu.
* **Modular & DRY**: Hỗ trợ chia nhỏ SQL và tái sử dụng code thông qua hàm ref() và macros.
* **Auto-generated Documentation & Lineage**: Tự động sinh tài liệu và vẽ sơ đồ DAG của luồng dữ liệu.

### Điểm yếu (Cons)
* **SQL-only limitation**: Bị giới hạn trong ngôn ngữ SQL (dù các phiên bản mới hỗ trợ Python models nhưng còn hạn chế).
* **Compute Overhead**: Phụ thuộc hoàn toàn vào tài nguyên tính toán của Data Warehouse bên dưới, có thể phát sinh chi phí lớn nếu không tối ưu hóa.
...
### Nên áp dụng khi:
* Doanh nghiệp sử dụng Cloud Data Warehouse hiện đại (Snowflake, BigQuery, Redshift) làm nền tảng lưu trữ và phân tích.
* Đội ngũ phân tích có nhiều Analytics Analysts/Engineers thành thạo SQL muốn tự xây dựng pipeline mà không cần phụ thuộc nhiều vào Python/Spark.

### Chưa nên áp dụng khi:
* Cơ sở dữ liệu phân tích chỉ là các OLTP database nhỏ, truyền thống.
* Cần xử lý các tác vụ nạp dữ liệu (ETL/ELT) từ các nguồn API bên ngoài vào kho dữ liệu.

## Các khái niệm liên quan

* [Xây dựng CI/CD cho Modern Data Pipelines (Slim CI & Data Testing)](/concepts/3-integration/transformation-analytics/data-pipeline-cicd/)
* [Thiết lập Data Contracts với Kafka Schema Registry](/concepts/3-integration/transformation-analytics/data-contracts-schema-registry/)
...
1. dbt Labs Documentation: [What is dbt?](https://docs.getdbt.com/docs/introduction)
2. Google Cloud Architecture: [Modernize data warehousing using dbt and BigQuery](https://cloud.google.com/architecture/modernizing-data-warehousing-with-dbt-and-bigquery)
3. AWS Big Data Blog: [Orchestrate dbt on AWS with Apache Airflow](https://aws.amazon.com/blogs/big-data/orchestrate-dbt-on-amazon-ecs-with-aws-step-functions-and-amazon-mwaa/)
4. Snowflake Developer Guide: [Get started with dbt Core and Snowflake](https://quickstarts.snowflake.com/guide/getting_started_with_dbt_core/)
5. Databricks Partner Connect: [Use dbt Core with Databricks](https://docs.databricks.com/en/partner-connect/dbt-best-practices.html)
