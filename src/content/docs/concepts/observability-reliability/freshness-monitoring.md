...
1. **Lỗi im lặng (Silent failure)**: Các công cụ lập lịch hoặc [ETL](/concepts/etl-elt/etl/) Job báo cáo chạy thành công, nhưng do API nguồn gặp sự cố hoặc phân vùng dữ liệu trống rỗng, quá trình trích xuất chỉ lấy về được 0 dòng. Về mặt kỹ thuật không có lỗi crash nào xảy ra, nhưng thực tế dữ liệu trong kho vẫn là dữ liệu cũ kỹ từ nhiều ngày trước.
...
* **Giám sát ngay từ dữ liệu nguồn (Source Ingestion)**: Đừng chỉ đợi dữ liệu qua tay Spark, [dbt](/concepts/transformation-analytics/dbt/) rồi mới đo Freshness. Hãy giám sát ngay từ lớp dữ liệu thô (Staging/Raw) để phát hiện sớm các lỗi mất kết nối API từ đối tác bên thứ ba.
