...
- **Healthy Lag (Lag đàn hồi):** Do hiện tượng **Spike** (lưu lượng tăng đột biến, ví dụ: Flash Sale). Đồ thị Lag tăng lên dạng chóp bu rồi giảm dần về 0 khi Consumer bắt kịp. Hệ thống vẫn đang xử lý bình thường.
...
- Ghi (Sink) dữ liệu vào Database (PostgreSQL, Elasticsearch) từng dòng một (Row-by-row) thay vì dùng Batch Insert.
...
### Vấn đề của Giao thức Eager Rebalancing (Mặc định ở Kafka cũ)
...
**Hậu quả:** Trong suốt thời gian Rebalance, không có bất kỳ một Consumer nào được phép xử lý dữ liệu. Nếu Group có 100 Consumers, cả 100 cái sẽ phải ngưng đọng. Dữ liệu từ Producer vẫn đổ vào ầm ầm -> **Lag tăng vọt.**
...
## 3. Các Mô Hình Kiến Trúc Khắc Phục Triệt Để

Việc khắc phục đòi hỏi phải hiểu rõ Trade-offs giữa tính khả dụng (Availability) và tính nhất quán (Consistency). Dưới đây là các kỹ thuật thực chiến ở tầm mức Enterprise.
...
Khi bạn deploy ứng dụng Consumer trên Kubernetes bằng Rolling Update, Pod cũ bị hủy (Leave Group) -> Rebalance. Pod mới lên (Join Group) -> Rebalance. Deploy 50 Pods sinh ra 100 lần Rebalance!
...
### 3.3. Xử lý I/O Heavy: Tách Luồng Poll và Luồng Xử Lý (Async Pattern)
