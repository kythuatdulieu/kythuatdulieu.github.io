...
Các thuật toán nén phổ biến nhất trong [Data Engineering](/concepts/foundation/data-engineering/) ngày nay bao gồm: **Snappy** (do Google phát triển), **Gzip** (chuẩn nén GNU kinh điển), **Zstandard/Zstd** (do Facebook phát triển) và **LZ4**. Hầu hết chúng đều dựa trên các biến thể của thuật toán từ điển nổi tiếng LZ77/LZ78 kết hợp với kỹ thuật mã hóa entropy (như Huffman coding).

## Tại sao chúng ta cần nén dữ liệu? (Khi CPU không còn là điểm nghẽn)

Trong các hệ thống tính toán phân tán hiện đại (như [Apache Spark](/concepts/batch-processing/apache-spark/), Hadoop, Kafka), CPU rất ít khi là nút thắt cổ chai làm chậm hệ thống. Thay vào đó, giới hạn lớn nhất về mặt hiệu năng thường nằm ở:
1. **Tốc độ đọc ghi ổ đĩa (Disk I/O)**: Tốc độ đọc dữ liệu thô từ ổ cứng HDD hoặc SSD vào RAM.
2. **Băng thông mạng (Network I/O)**: Quá trình truyền tải dữ liệu giữa các máy chủ trong mạng (ví dụ trong giai đoạn [Shuffle](/concepts/batch-processing/shuffle/) của Spark).

Nén dữ liệu giải quyết cả hai bài toán này theo cách cực kỳ thông minh:
* **Tiết kiệm chi phí lưu trữ**: Giảm dung lượng file trên [Cloud Storage](/concepts/cloud-data-platform/cloud-storage/) (như Amazon S3 hay GCS) xuống nhiều lần, giảm trực tiếp hóa đơn lưu trữ hàng tháng.
...
1. **Chia khối (Block/[Chunking](/concepts/genai-ml/chunking/))**: File dữ liệu lớn được chia thành các nhóm dòng (row groups) hoặc các block nhỏ (ví dụ 64MB hoặc 128MB). Việc nén sẽ được thực hiện độc lập trên từng block này.
...
2. [Compression in Apache Kafka: Gzip, Snappy, LZ4, Zstd](https://www.confluent.io/blog/kafka-compression-snappy-gzip-lz4-zstd/) - Confluent blog post analyzing and benchmarking different compression formats for stream processing.
