...
Khi thế giới bước vào kỷ nguyên Big Data với các công cụ xử lý mạnh mẽ như Hadoop và [Apache Spark](/concepts/batch-processing/apache-spark/), các kỹ sư cần những định dạng tệp mới để:
...
* **Ứng dụng**: Đây được coi là tiêu chuẩn vàng cho các truy vấn phân tích [OLAP](/concepts/database-storage/olap/) và xây dựng [Data Warehouse](/concepts/data-warehouse/data-warehouse/)/[Lakehouse](/concepts/data-lake-lakehouse/lakehouse/).
...
* **Đặc tính nổi bật**: Nén tốt hơn CSV, tốc độ ghi (Write) và tuần tự hóa (Serialization) nhanh vượt trội. Điểm sáng lớn nhất của Avro là khả năng hỗ trợ tiến hóa schema ([Schema Evolution](/concepts/data-lake-lakehouse/schema-evolution/)), cho phép thay đổi cấu trúc bảng mà không làm hỏng các tệp tin cũ.
* **Ứng dụng**: Phù hợp cho các hệ thống truyền nhận dữ liệu thời gian thực (như [Apache Kafka](/concepts/streaming-processing/apache-kafka/)) hoặc lưu trữ dữ liệu thô tại vùng đệm (Landing Zone).
...
* **Nhược điểm**: Thời gian ghi dữ liệu lâu hơn, không thích hợp cho các tác vụ [OLTP](/concepts/database-storage/oltp/) hoặc Streaming.
...
5. [What is Apache Parquet?](https://www.databricks.com/glossary/what-is-parquet) - Entry in the Databricks Glossary discussing Parquet's columnar compression and metadata architecture.
