...
1. **Batch Extraction (Trích xuất theo lô)**:
   ...
   * **Incremental Load**: Chỉ đọc các dòng mới hoặc vừa cập nhật dựa trên cột `updated_at`.
2. **Change Data Capture (CDC)**: Đọc trực tiếp từ file nhật ký ghi (Transaction Log/Binlog) của database để lấy các thay đổi (INSERT, UPDATE, DELETE) ngay lập tức mà không cần câu lệnh `SELECT` làm nặng hệ thống.
...
```
Sau đó, sự kiện này được đẩy vào Kafka và cuối cùng lưu vào Data Lake. Toàn bộ quá trình này không gửi bất kỳ câu lệnh `SELECT` nào vào MySQL, giúp bảo vệ hệ thống nguồn.
