...
Hãy tưởng tượng bạn đang quản lý một kho dữ liệu ([Data Warehouse](/concepts/data-warehouse/data-warehouse/)) khổng lồ lên tới hàng trăm Terabytes trên môi trường Production. Đội ngũ Data Science cần một bản sao y hệt của toàn bộ dữ liệu này để thử nghiệm mô hình học máy mới. Đội ngũ Data Engineer cũng cần một bản sao tương tự để chạy thử nghiệm các pipeline [dbt](/concepts/transformation-analytics/dbt/)/SQL trước khi deploy bản cập nhật. 
...
* **Dữ liệu bất biến (Immutable)**: Trong các kho dữ liệu hiện đại như [Snowflake](/concepts/cloud-data-platform/snowflake/), các tệp dữ liệu lưu trữ vật lý dưới đĩa (thường gọi là Micro-partitions) không bao giờ bị chỉnh sửa trực tiếp. Khi bạn cập nhật (`UPDATE`) hoặc xóa (`DELETE`) một dòng dữ liệu, hệ thống không sửa file cũ mà sẽ ghi một file mới chứa trạng thái đã thay đổi, sau đó cập nhật lại bản đồ siêu dữ liệu để trỏ đến file mới này.
...
* **Nên dùng**: Khi bạn làm việc trên các nền tảng Data Warehouse đám mây có hỗ trợ cơ chế này (Snowflake, BigQuery Table Clones, [Delta Lake](/concepts/data-lake-lakehouse/delta-lake/) Shallow Clones) để phục vụ cho các môi trường Test/Dev, chạy CI/CD hoặc cấp môi trường Sandbox độc lập cho đội ngũ Data Science.
...
1. [Snowflake Documentation - Cloning (Zero-Copy)](https://docs.snowflake.com/en/user-guide/tables-cloning)
2. [Google Cloud BigQuery Documentation - Table Clones](https://cloud.google.com/bigquery/docs/table-clones-intro)
