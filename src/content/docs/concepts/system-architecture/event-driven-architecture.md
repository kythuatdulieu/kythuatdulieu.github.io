...
```mermaid
graph TD
    subgraph Orchestration ["Orchestration (Chỉ huy)"]
        O[Orchestrator]
        S1[Service A]
        S2[Service B]
        S3[Service C]
    ...
    subgraph Choreography ["Choreography (Tự biên đạo)"]
        EB{Event Broker}
        C1[Service A]
        C2[Service B]
        C3[Service C]
        C1 -->|Publishes Event A| EB
        EB -->|Triggers| C2
        C2 -->|Publishes Event B| EB
        EB -->|Triggers| C3
    end
```
...
## Phân tích Rủi ro: Eventual Consistency và Duplicate Events

Việc áp dụng EDA giải quyết nhiều bài toán mở rộng, nhưng cũng đem lại những rủi ro đi kèm đặc thù. Việc nhận diện và kiểm soát chúng là bắt buộc đối với một hệ thống bền vững.

### 1. Nhất quán Tạm thời (Eventual Consistency)
* **Vấn đề**: Trong mô hình nguyên khối hoặc Request-Driven gọi chuỗi API trực tiếp, chúng ta có thể lợi dụng Transaction của cơ sở dữ liệu để thay đổi trạng thái đồng loạt, dữ liệu ngay lập tức nhất quán. Với EDA, thay đổi trạng thái được truyền đi bất đồng bộ. Điều này nghĩa là trong một khoảng thời gian (từ vài mili-giây tới vài phút nếu có lỗi mạng), dữ liệu trên các dịch vụ khác nhau có thể không khớp nhau. 
* **Giải pháp**: Xây dựng tư duy chấp nhận "Eventual Consistency". Thay vì cố gắng tạo ra phân tán giao dịch (Distributed Transactions), kỹ sư thường áp dụng **Saga Pattern**. Nếu một bước phía sau (ví dụ: Thanh toán) bị thất bại, hệ thống sẽ phát sinh ra các **Compensating Events** (sự kiện hoàn tác) để quay ngược lại phục hồi trạng thái ở dịch vụ trước đó (như hoàn trả lại hàng vào kho).

### 2. Sự kiện Trùng lặp (Duplicate Events) và Tính Lũy Đẳng (Idempotency)
* **Vấn đề**: Do đặc thù phân tán và thiết kế mạng, phần lớn các Event Broker (kể cả Kafka) thường chỉ có thể đảm bảo mô hình phân phối là **"At-Least-Once"** (gửi ít nhất một lần). Trong trường hợp lỗi đường truyền hoặc timeout khi xác nhận (acknowledge), một sự kiện có thể được gửi đi gửi lại nhiều lần tới Consumer.
* **Giải pháp - Thiết kế Idempotency**: Dịch vụ Consumer phải được thiết kế có tính "Lũy đẳng". Idempotency đảm bảo rằng dù hệ thống xử lý một sự kiện một lần hay hàng chục lần, kết quả hệ thống cũng không thay đổi. 
  * Cách thực hiện: Mỗi sự kiện phát ra cần một định danh duy nhất (Event ID) kết hợp cùng Transaction ID. Consumer sẽ sử dụng Database (có thể dùng bảng lưu vết để tra cứu Event ID đã xử lý) trước khi thực thi tác vụ. Nếu thấy Event ID đã tồn tại ở trạng thái hoàn thành, nó sẽ bỏ qua tin nhắn thay vì thực hiện nghiệp vụ (như trừ tiền) một lần nữa.
...
Không nên áp dụng EDA cho các ứng dụng nhỏ nguyên khối (Monolith) hoặc các luồng giao dịch yêu cầu tính toàn vẹn dữ liệu nghiêm ngặt trong một phiên giao dịch duy nhất (như chuyển khoản ngân hàng).
...
* [AWS Prescriptive Guidance: Event-Driven Architecture](https://docs.aws.amazon.com/prescriptive-guidance/latest/event-driven-architecture/welcome.html)
