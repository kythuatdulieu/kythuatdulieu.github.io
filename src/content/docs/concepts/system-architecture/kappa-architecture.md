...
Về mặt kiến trúc, Kappa cực kỳ tinh giản dựa trên luồng dữ liệu Kafka -> Flink -> Realtime DB:
```mermaid
graph TD
    A[New Data / Events] -->|Append-only| B[(Apache Kafka 
 Immutable Event Log Store)]
 ...
 Stream Processor Engine]
    ...
    C -->|Continuous Updates| D[(Realtime DB 
 e.g., Apache Pinot / Redis / Druid)]
    D --> E[Client Applications / Realtime BI]
```
...
2. **Stream Processing Engine (Công cụ xử lý luồng)**: Sử dụng các công cụ mạnh mẽ như Apache Flink để liên tục đọc dữ liệu từ Log Store, tính toán và đẩy kết quả cập nhật trực tiếp xuống cơ sở dữ liệu phục vụ (Realtime DB) để hiển thị lên Dashboard của người dùng.
...
## Under the hood: Sự khó khăn của State Management và Backfilling

Mặc dù kiến trúc Kappa mang lại sự tinh gọn bằng cách loại bỏ tầng Batch, nó lại đẩy sự phức tạp xuống tầng vận hành (operations) và cơ chế xử lý luồng, đặc biệt là ở hai khía cạnh:

* **Quản lý trạng thái (State Management) khổng lồ**: Trong quá trình stream processing, việc tính toán các metric (như tổng doanh thu theo tháng, session người dùng kéo dài) đòi hỏi framework (như Apache Flink) phải giữ lại "trạng thái" (state). Khi áp dụng Kappa, việc duy trì một state backend (như RocksDB) chứa lượng dữ liệu khổng lồ đẻ ra nhiều thách thức về mặt bộ nhớ, tốc độ checkpointing và thời gian phục hồi (recovery time) khi hệ thống gặp sự cố. Nếu state quá lớn, quá trình snapshot có thể gây ra nghẽn cổ chai (bottleneck) làm chậm toàn bộ luồng xử lý.
* **Bài toán Backfilling (Điền bù dữ liệu) và Catch-up**: Backfilling là quá trình tính toán và nạp lại dữ liệu trong quá khứ. Trong Kappa, chúng ta làm điều này bằng cách tua lại offset của Kafka từ con số 0. Tuy nhiên, việc một stream processor "tiêu hóa" lượng dữ liệu tích lũy của vài năm trong thời gian ngắn yêu cầu lượng tài nguyên tính toán (CPU/RAM/Network) gấp nhiều lần so với xử lý luồng thời gian thực. Thêm vào đó, việc kéo dữ liệu lịch sử từ các tầng lưu trữ giá rẻ (Tiered Storage trên S3/GCS) lên Flink để backfill cũng gặp giới hạn về thông lượng (throughput) so với đọc trực tiếp từ đĩa cục bộ.
...
* [Questioning the Lambda Architecture](https://www.oreilly.com/content/questioning-the-lambda-architecture/) - Jay Kreps
* [Streaming Systems](https://www.oreilly.com/library/view/streaming-systems/9781491983812/) - Tyler Akidau, Slava Chernyak, and Reuven Lax
* [Kappa Architecture - Where Every Data Is A Stream](https://hazelcast.com/glossary/kappa-architecture/) - Hazelcast
* [Lambda vs. Kappa Architecture](https://www.databricks.com/glossary/lambda-architecture) - Databricks
* [Building a Kappa Architecture with Apache Flink and Kafka](https://flink.apache.org/2020/07/28/flink-sql-demo-building-an-end-to-end-streaming-application.html) - Apache Flink Blog
