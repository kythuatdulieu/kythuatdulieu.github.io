...
Có bao giờ bạn nhảy vào viết code ETL ngay khi vừa nhận được một file CSV hay thông tin kết nối CSDL từ đối tác, để rồi vài ngày sau pipeline bị "sập" liên tục vì những lỗi ngớ ngẩn như cột chứa chữ thay vì số, hay độ dài chuỗi vượt quá khai báo? Nếu câu trả lời là có, bạn không cô đơn đâu. Đó là lúc chúng ta cần đến **Data Profiling (Lập hồ sơ dữ liệu)** — bước "chụp X-quang" giúp bạn nhìn thấu hình hài thực sự của dữ liệu trước khi đặt bút viết dòng code đầu tiên.
...
Hãy tưởng tượng đối tác bàn giao cho bạn một database MySQL và khẳng định chắc nịch rằng: *"Cột `country_code` chỉ chứa mã quốc gia 2 ký tự (như VN, US)"*. Bạn tin tưởng tạo bảng trong Data Warehouse với kiểu dữ liệu `VARCHAR(2)`. Nhưng khi pipeline chạy thực tế, nó "nổ tung" ngay lập tức vì người dùng đã nhập "USA" hoặc "Vietnam".
...
* **Công cụ chuyên dụng (Python / Modern Data Stack)**: Sử dụng các thư viện như `ydata-profiling` trong Python hoặc các hệ thống Data Catalog (như Atlan, DataHub) để tự động quét dữ liệu định kỳ.
...
* **Chỉ quét trên tập mẫu (Sampling)**: Nếu bảng dữ liệu có hàng tỷ dòng, việc chạy `COUNT DISTINCT` hay các hàm phân tích trên toàn bộ bảng sẽ ngốn rất nhiều tài nguyên và chi phí cloud (hoặc làm nghẽn CSDL vận hành OLTP). Hãy lấy một mẫu ngẫu nhiên khoảng 100,000 dòng để phân tích.
...
1. **[Data Quality Dimensions](/concepts/data-quality/data-quality-dimensions/)** - Hiểu về các khía cạnh đánh giá chất lượng dữ liệu.
2. **[Data Catalog](/concepts/governance-metadata/data-catalog/)** - Nơi lưu trữ thông tin metadata thu được từ quá trình Profiling.
3. **"Data Quality: The Accuracy Dimension"** - Sách của tác giả Jack E. Olson, đặc biệt là chương về Analytical Profiling.
4. **ydata-profiling (Pandas Profiling)** - Thư viện Python mã nguồn mở tiêu chuẩn để lập hồ sơ dữ liệu nhanh chóng.
