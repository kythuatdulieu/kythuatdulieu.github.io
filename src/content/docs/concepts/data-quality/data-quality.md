...
3. **Kiểm soát & Giám sát (Validation & Monitoring)**: Nhúng các bài kiểm thử tự động (Data Tests) vào pipeline [ETL](/concepts/etl-elt/etl/)/[ELT](/concepts/etl-elt/elt/) để ngăn chặn dữ liệu bẩn hoặc phát đi cảnh báo ngay lập tức.
...
* **Tự động hóa hoàn toàn**: Loại bỏ việc kiểm tra dữ liệu bằng mắt hay chạy script thủ công. Mọi quy tắc chất lượng phải được viết thành code và chạy tự động trong pipeline (ví dụ: dùng [dbt](/concepts/transformation-analytics/dbt/) test).
...
1. [Monte Carlo Data: What is Data Quality?](https://www.getmontecarlo.com/blog/what-is-data-quality-identify-prevent-fix-common-issues/) - In-depth guide discussing the shift from reactive monitoring to end-to-end [data observability](/concepts/observability-reliability/data-observability/).
