...
Trong quy trình trích xuất, biến đổi và nạp dữ liệu ([ETL](/concepts/etl-elt/etl/)/[ELT](/concepts/etl-elt/elt/) - Extract, Transform, Load / Extract, Load, Transform) kinh điển, chữ cái cuối cùng đại diện cho **Loading (Nạp dữ liệu)**. Đây là bước vật lý trực tiếp ghi hoặc đẩy toàn bộ dữ liệu (cho dù là dữ liệu thô vừa lấy về hay dữ liệu tinh chọn đã qua xử lý) vào hệ thống lưu trữ đích cuối cùng – nơi mà các nhà phân tích hay các công cụ báo cáo thông minh (BI - Business Intelligence) như Tableau, PowerBI đang chờ sẵn để khai thác.
...
Dưới đây là một câu lệnh SQL sử dụng cơ chế **MERGE** (được hỗ trợ bởi các nền tảng lớn như [Snowflake](/concepts/cloud-data-platform/snowflake/), BigQuery, [Delta Lake](/concepts/data-lake-lakehouse/delta-lake/)) để cập nhật dữ liệu từ bảng tạm (staging) vào bảng chính:
```sql
...
* **Chạy MERGE trên dữ liệu nguồn bị trùng lặp**: Nếu Khóa chính ở bảng nguồn bị trùng lặp (ví dụ có 2 dòng cùng chung một `order_id` nhưng mang 2 trạng thái khác nhau), câu lệnh MERGE của [Data Warehouse](/concepts/data-warehouse/data-warehouse/) sẽ bị lỗi kẹt (`Multiple match error`) or ghi đè lung tung. Bắt buộc phải thực hiện khử trùng (Deduplicate) dữ liệu nguồn trước khi nạp.
...
* **Đảm bảo tính lũy đẳng ([Idempotency](/concepts/etl-elt/idempotency/))**: Cho dù chạy lại một job nạp dữ liệu bao nhiêu lần, kết quả cuối cùng trong bảng đích vẫn phải nhất quán không đổi. Đối với chiến lược Append, việc chạy lại dễ khiến dữ liệu bị nhân đôi. Do đó, trước khi nạp thêm, hãy chạy một câu lệnh xóa sạch dữ liệu của riêng ngày hôm đó trước (Ví dụ pattern: `DELETE WHERE date = '2026-06-07'` rồi mới chạy `APPEND`).
...
* **Xây dựng Bảng tạm (Staging Tables)**: Tuyệt đối không nạp dữ liệu trực tiếp từ API vào thẳng bảng báo cáo chính. Hãy nạp toàn bộ vào một bảng tạm trung gian (`staging_table`), chạy các bài kiểm thử chất lượng dữ liệu ([Data Quality](/concepts/data-quality/data-quality/) Tests). Khi dữ liệu đã vượt qua vòng kiểm tra, lúc đó mới thực hiện lệnh `MERGE` vào bảng thật.
...
1. [Fundamentals of Data Engineering](https://www.oreilly.com/library/view/fundamentals-of-data/9781098108298/) - Book by Joe Reis and Matt Housley detail-oriented on the load phase of the ETL/ELT lifecycle, data warehouses, and destinations.
2. [Snowflake Documentation: COPY INTO table](https://docs.snowflake.com/en/sql-reference/sql/copy-into-table) - Official reference guide detailing Snowflake's bulk data loading mechanism from external stages.
3. [dbt Documentation: Materializations](https://docs.getdbt.com/docs/build/materializations) - Explains table, view, incremental, and ephemeral model strategies for loading and creating tables.
4. Databricks Documentation: MERGE INTO - Technical reference on using MERGE INTO in Delta Lake for transactional upserts and deletes.
5. [AWS Architecture Center: Data Ingestion and Loading best practices](https://docs.aws.amazon.com/redshift/latest/dg/t_Loading-data-from-s3.html) - AWS guide on optimized bulk data loading into [Amazon Redshift](/concepts/cloud-data-platform/amazon-redshift/) using the COPY command.
