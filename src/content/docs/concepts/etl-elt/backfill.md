...
# Backfill (Lắp đầy khoảng trống lịch sử): Chuyến du hành ngược thời gian của dữ liệu

Trong trạng thái vận hành bình thường, một đường ống dẫn dữ liệu (Data Pipeline) hoạt động giống như một dòng sông chảy xuôi theo thời gian. Mỗi ngày, đến giờ hẹn, hệ thống sẽ tự động thức dậy, thu thập dữ liệu phát sinh của ngày hôm đó, biến đổi và nạp vào kho lưu trữ (Incremental Run). 
...
Để xử lý những tình huống này, chúng ta cần đến một kỹ thuật cực kỳ quan trọng trong Data Engineering: **Backfill (Chạy bù dữ liệu lịch sử)**. Đây là hành động "quay ngược thời gian" của hệ thống ETL/ELT, buộc các tiến trình phải tính toán lại dữ liệu của các ngày hoặc các tháng đã trôi qua trong quá khứ nhằm lấp đầy các khoảng trống và sửa đổi dữ liệu cho đúng chuẩn.
...
Chìa khóa vàng giúp bạn thực hiện Backfill một cách dễ dàng và an toàn tuyệt đối là **Tính lũy đẳng (Idempotency)**.

> [!IMPORTANT]  
> Một đường ống dữ liệu được coi là lũy đẳng nếu việc bạn chạy nó 1 lần hay chạy lại 100 lần cho cùng một ngày (ví dụ: ngày 01/01/2026) thì kết quả cuối cùng ghi nhận trên Data Warehouse vẫn luôn giống hệt nhau, hoàn toàn không bị nhân đôi số liệu hay sinh ra các bản ghi rác.
...
Hãy cùng xem các bước thực thi một chiến dịch Backfill dữ liệu trong thực tế sử dụng công cụ điều phối Apache Airflow:
...
3. **Kích hoạt lệnh:** Kỹ sư chạy câu lệnh Backfill chuyên dụng từ dòng lệnh terminal:  
   `cmd /c "airflow dags backfill my_daily_etl_job -s 2026-05-01 -e 2026-05-31"`
...
Nếu bạn sử dụng công cụ biến đổi dữ liệu **dbt** (data build tool), việc thực hiện Backfill cho toàn bộ lịch sử cực kỳ đơn giản nhờ tính năng `Full Refresh`.
...
Dưới nền, dbt sẽ tự động chuyển hóa câu lệnh và gửi xuống Data Warehouse (như BigQuery hay Snowflake) hai bước truy vấn:
...
* [Incremental Load](/concepts/etl-elt/incremental-load/)
* [Data Transformation](/concepts/etl-elt/data-transformation/)
