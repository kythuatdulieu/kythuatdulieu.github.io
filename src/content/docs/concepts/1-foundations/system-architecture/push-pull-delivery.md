...
definition: "Push và Pull là hai cơ chế truyền tải thông tin xác định bên gửi (Producer - Push) hay bên nhận (Consumer - Pull) chủ động kiểm soát tốc độ và thời điểm di chuyển dữ liệu."
---
...
![Sơ đồ minh họa kiến trúc Push Pull Delivery](/images/push-pull-delivery/diagram_1.svg)
...
![Quy trình xử lý và luồng dữ liệu Push Pull Delivery](/images/push-pull-delivery/diagram_2.png)
...
Để tận dụng ưu điểm của cả hai thế giới, các kiến trúc sư dữ liệu hiện đại thường thiết kế các hệ thống Hybrid sử dụng Message Broker ở giữa (như [Apache Kafka](/concepts/4-realtime/streaming-processing/apache-kafka/)).
...
### Cơ chế Push (Đẩy)
* **Điểm mạnh (Pros)**:
  * **Độ trễ thấp**: Dữ liệu được chuyển đi ngay khi phát sinh, lý tưởng cho các nhu cầu thời gian thực.
  * **Tiết kiệm tài nguyên bên nhận**: Bên nhận không phải liên tục gửi request thăm dò (polling) vô ích.
* **Điểm yếu (Cons)**:
  * **Dễ gây quá tải**: Khi tốc độ gửi vượt quá tốc độ xử lý của bên nhận, hệ thống nhận có thể bị nghẽn hoặc sập.
  * **Tính phụ thuộc**: Bên nhận phải mở cổng và duy trì trạng thái luôn trực tuyến.

### Cơ chế Pull (Kéo)
* **Điểm mạnh (Pros)**:
  * **Chủ động kiểm soát tải**: Bên nhận tự quyết định nhịp độ xử lý dữ liệu của mình (Backpressure handling).
  * **An toàn bảo mật**: Bên nhận không cần mở cổng đón kết nối, giảm thiểu nguy cơ tấn công mạng.
* **Điểm yếu (Cons)**:
  * **Độ trễ cao hơn**: Dữ liệu phải chờ đến lượt thăm dò tiếp theo mới được lấy về.
  * **Tốn tài nguyên polling**: Việc liên tục thăm dò có thể gây lãng phí băng thông mạng và I/O của nguồn phát.

---

## Khi nào nên dùng

### Khi nào nên dùng cơ chế Push:
1. Các ứng dụng yêu cầu thời gian thực tuyệt đối như hệ thống nhắn tin, thông báo đẩy (push notifications), hoặc dashboard giám sát khẩn cấp thuộc kiến trúc [Real-time Architecture](/concepts/1-foundations/system-architecture/real-time-architecture/).
2. Các dịch vụ Webhook tích hợp với bên thứ ba (như cổng thanh toán gửi thông báo trạng thái giao dịch).

### Khi nào nên dùng cơ chế Pull:
1. Các tiến trình thu thập dữ liệu lô lớn (batch ingestion) từ nhiều nguồn khác nhau về [Data Lake](/concepts/2-storage/data-lake-lakehouse/data-lake/) hoặc [Data Warehouse](/concepts/2-storage/data-warehouse/data-warehouse/).
2. Các kiến trúc stream xử lý dữ liệu phức tạp thuộc [Event-Driven Architecture](/concepts/1-foundations/system-architecture/event-driven-architecture/) cần khả năng tự điều tiết tải (như Kafka Consumer, Flink).

---

## Trọng tâm ôn luyện phỏng vấn

### Câu hỏi 1: Hiện tượng "Backpressure" (Áp suất ngược) là gì? Cơ chế Push hay Pull giúp chúng ta giải quyết vấn đề này tốt hơn?
**Trả lời:**
Backpressure là tình trạng hệ thống gửi dữ liệu (Producer) hoạt động nhanh hơn nhiều so với khả năng xử lý của hệ thống nhận (Consumer). Dữ liệu bị dồn ứ sẽ làm tràn bộ nhớ đệm (RAM) và có thể gây sập ứng dụng nhận. 
...
### Câu hỏi 2: Tại sao Apache Kafka lại lựa chọn cơ chế Consumer "Pull" thay vì chủ động "Push" dữ liệu xuống các Consumer như các Message Broker truyền thống (ví dụ RabbitMQ)?
**Trả lời:**
Kafka được thiết kế để phân phối dữ liệu cho rất nhiều ứng dụng tiêu thụ (Consumer) khác nhau với năng lực phần cứng và tốc độ xử lý vô cùng đa dạng. Nếu Kafka chọn cơ chế Push, bản thân Broker sẽ phải gánh vác việc theo dõi trạng thái, sức khỏe và tính toán tốc độ đẩy dữ liệu phù hợp cho từng Consumer riêng lẻ. Điều này sẽ làm giảm đáng kể hiệu năng và khả năng mở rộng của Broker. 
...
* [AWS Architecture Guide - Introducing EventBridge Pipes Support for Push and Pull Sources](https://aws.amazon.com/blogs/compute/introducing-amazon-eventbridge-pipes-support-for-push-and-pull-sources/)
* [Google Cloud Pub/Sub - Choose between Push and Pull Subscriptions](https://cloud.google.com/pubsub/docs/subscriber)
* [Microsoft Azure Service Bus - Message Delivery Mechanics (Push vs Pull)](https://learn.microsoft.com/en-us/azure/service-bus-messaging/service-bus-messaging-overview)
* [Databricks Blog - Structured Streaming Push and Pull Ingestion Modes](https://www.databricks.com/blog/2021/04/27/structured-streaming-push-vs-pull-ingestion.html)
* [Confluent - Push vs Pull in Event-Driven Streaming Architectures](https://www.confluent.io/blog/push-vs-pull-in-event-driven-architectures/)
* [O'Reilly - Designing Data-Intensive Applications by Martin Kleppmann](https://www.oreilly.com/library/view/designing-data-intensive-applications/9781491903063/)