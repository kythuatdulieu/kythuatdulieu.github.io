...
metaDescription: "Tìm hiểu kiến trúc Lambda (Lambda Architecture) trong Data Engineering: cơ chế hoạt động, sự kết hợp giữa Batch, Speed, và Serving layer cùng câu hỏi phỏng vấn."
definition: "Lambda Architecture là một kiến trúc xử lý dữ liệu lớn kết hợp giữa xử lý lô (Batch Layer) đảm bảo tính chính xác và xử lý luồng (Speed Layer) đảm bảo độ trễ thấp."
---

Trong thế giới Big Data, các kỹ sư dữ liệu từng phải đối mặt với một sự đánh đổi nghiệt ngã: 
- Hoặc chọn **hệ thống xử lý theo lô ([Batch Processing](/concepts/3-integration/batch-processing/batch-processing/))** với độ chính xác tuyệt đối nhưng mất hàng giờ hoặc hàng ngày để ra kết quả.
...
- **Công nghệ thường dùng**: Hadoop HDFS, AWS S3, [Apache Spark](/concepts/3-integration/batch-processing/apache-spark/).
...
### Điểm mạnh (Pros)
* **Đảm bảo tính chính xác tuyệt đối**: Nhờ Batch Layer liên tục recompute dữ liệu gốc ở dạng Master Dataset bất biến.
* **Khả năng chịu lỗi cao**: Dữ liệu gốc là bất biến và chỉ thêm mới (append-only), giúp dễ dàng phục hồi dữ liệu khi code có lỗi hoặc hệ thống sập.
* **Cân bằng giữa tốc độ và độ chính xác**: Cung cấp dữ liệu real-time với độ trễ thấp song song với số liệu lịch sử chính xác cao.

### Điểm yếu (Cons)
* **Độ phức tạp cực cao**: Phải duy trì hai hệ thống xử lý (batch và stream) độc lập, dẫn đến gánh nặng vận hành lớn.
* **Trùng lặp logic code**: Thường phải viết và đồng bộ logic nghiệp vụ ở cả Batch Layer và Speed Layer.
* **Chi phí lưu trữ và tính toán lớn**: Đòi hỏi tài nguyên máy chủ cho cả hai lớp xử lý và Serving Layer.
...
### Không nên chọn khi:
* Bạn muốn tối giản hóa kiến trúc và hạn chế tối đa việc trùng lặp logic code. Khi đó, hãy cân nhắc [Kiến trúc Kappa](/concepts/1-foundations/system-architecture/kappa-architecture/).
...
* [Kappa Architecture](/concepts/1-foundations/system-architecture/kappa-architecture/)
* [Real-time Architecture](/concepts/1-foundations/system-architecture/real-time-architecture/)
...
### Câu hỏi 1: Sự khác biệt chính giữa kiến trúc Lambda và kiến trúc Kappa là gì?
**Trả lời:**
...
### Câu hỏi 2: Làm thế nào để Lambda Architecture xử lý lỗi logic của con người (Human Error) trong code?
**Trả lời:**
...
* [AWS - Lambda Architecture for Real-Time and Batch Processing](https://aws.amazon.com/blogs/big-data/lambda-architecture-for-real-time-and-batch-processing/)
* [Google Cloud - Demystifying Data Architectures: Lambda vs Kappa](https://cloud.google.com/blog/products/data-analytics/demystifying-data-architectures-lambda-vs-kappa)
* [Microsoft Azure - Big Data Batch Processing with Lambda Architecture](https://learn.microsoft.com/en-us/azure/architecture/data-guide/big-data/batch-processing)
* [Databricks Glossary - What is Lambda Architecture?](https://www.databricks.com/glossary/lambda-architecture)
* [Snowflake Blog - What is Lambda Architecture? Principles and Use Cases](https://www.snowflake.com/blog/what-is-lambda-architecture/)
* [Manning - Big Data: Principles and best practices of scalable realtime data systems](https://www.manning.com/books/big-data) - Nathan Marz & James Warren
* [O'Reilly - Designing Data-Intensive Applications](https://www.oreilly.com/library/view/designing-data-intensive-applications/9781491903063/) - Martin Kleppmann