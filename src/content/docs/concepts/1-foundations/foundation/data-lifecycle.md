...
definition: "Vòng đời Dữ liệu (Data Lifecycle) là toàn bộ các giai đoạn mà dữ liệu đi qua, từ lúc được khởi tạo, lưu trữ, xử lý, khai thác, lưu trữ dài hạn cho đến khi bị tiêu hủy hoàn toàn."
...
* **Thói quen lưu trữ vô thời hạn (Hoarding)**: Tâm lý "cứ giữ lại biết đâu sau này có lúc dùng đến" khiến [Data Lake](/concepts/2-storage/data-lake-lakehouse/data-lake/) biến thành một đầm lầy dữ liệu hỗn độn, kéo theo hóa đơn chi phí Cloud tăng vọt ngoài tầm kiểm soát.
...
Thiết lập quy trình Quản lý Vòng đời Dữ liệu là bắt buộc khi:
* Doanh nghiệp lưu trữ lượng lớn dữ liệu trên đám mây và chi phí lưu trữ (Storage cost) tăng nhanh liên tục qua các tháng.
* Doanh nghiệp phải tuân thủ các quy định pháp lý nghiêm ngặt về quyền riêng tư dữ liệu (ví dụ GDPR, CCPA) yêu cầu xóa thông tin cá nhân khách hàng sau một thời hạn hoặc khi được yêu cầu.
* Hiệu năng hệ thống và tốc độ truy vấn bị chậm đi do phải quét qua quá nhiều dữ liệu lịch sử không còn hoạt động.

Quản lý vòng đời dữ liệu có thể được đơn giản hóa hoặc trì hoãn khi:
* Quy mô dữ liệu của doanh nghiệp còn rất nhỏ và chi phí lưu trữ không đáng kể.
* Toàn bộ dữ liệu lịch sử đều cần truy cập với tần suất cao và độ trễ thấp liên tục cho các báo cáo nghiệp vụ.
...
* **Gợi ý trả lời**:
  ...
  * Ví dụ, trong **[Google BigQuery](/concepts/2-storage/cloud-data-platform/google-bigquery/)**, chúng ta có thể cấu hình thuộc tính `partition_expiration_days` cho bảng (ví dụ đặt là 90 ngày). BigQuery sẽ tự động xóa các phân vùng chứa dữ liệu cũ hơn 90 ngày mà không cần can thiệp thủ công.
  * Trong **[Snowflake](/concepts/2-storage/cloud-data-platform/snowflake/)**, chúng ta có thể sử dụng chính sách `Time Travel` kết hợp với `Fail-safe` và thiết lập các cron job định kỳ chạy lệnh SQL để lưu trữ (archive) hoặc xóa dữ liệu cũ nhằm tối ưu hóa chi phí lưu trữ Storage.

## Đọc thêm và Tài liệu tham khảo

### Các khái niệm liên quan
* [Vai trò Kỹ sư Dữ liệu](/concepts/1-foundations/foundation/data-engineering/) - Chi tiết công việc và kỹ năng cần có.
* [Kỹ thuật Dữ liệu](/concepts/1-foundations/foundation/data-engineering/) - Khái niệm tổng quan về ngành dữ liệu.
* [Kiến trúc Nền tảng Dữ liệu](/concepts/1-foundations/system-architecture/data-platform-architecture/) - Thiết kế hệ thống dữ liệu tổng thể.
* [Hệ thống Nguồn](/concepts/1-foundations/foundation/source-systems/) - Điểm khởi đầu sinh ra dữ liệu.
* [Data Ingestion (Thu nạp dữ liệu)](/concepts/3-integration/etl-elt/data-ingestion/) - Giai đoạn nạp dữ liệu vào hệ thống lưu trữ.
* [Data Pipeline (Đường ống dữ liệu)](/concepts/1-foundations/foundation/data-pipeline/) - Quy trình luân chuyển dữ liệu qua các chặng.
* [Data Warehouse (Kho dữ liệu)](/concepts/2-storage/data-warehouse/data-warehouse/) - Hệ thống lưu trữ và phân tích dữ liệu tập trung.

## Xem thêm các khái niệm liên quan
* [Kỹ thuật Dữ liệu & Vai trò Kỹ sư Dữ liệu](/concepts/1-foundations/foundation/data-engineering/)
* [Đường ống Dữ liệu - Data Pipeline](/concepts/1-foundations/foundation/data-pipeline/)
* [Hệ thống Nguồn - Source Systems](/concepts/1-foundations/foundation/source-systems/)

## Tài liệu tham khảo

1. [Amazon S3 Lifecycle Policies](https://docs.aws.amazon.com/AmazonS3/latest/userguide/object-lifecycle-mgmt.html) - AWS documentation on configuring rules to transition or expire objects automatically.
2. [Cloud Storage Object Lifecycle Management](https://cloud.google.com/storage/docs/lifecycle) - Google Cloud documentation on defining lifecycle policies for [cloud storage](/concepts/2-storage/cloud-data-platform/cloud-storage/) buckets.
3. [Microsoft Purview Data Lifecycle Management](https://learn.microsoft.com/en-us/purview/data-lifecycle-management) - Microsoft official documentation on retention and deletion policies.
4. [Managing BigQuery Tables (Table Expiration)](https://cloud.google.com/bigquery/docs/managing-tables#table-expiration) - Google Cloud BigQuery documentation for setting automatic partition/table time-to-live policies.
5. [Snowflake Data Lifecycle and Storage](https://docs.snowflake.com/en/user-guide/data-pipelines-intro) - Snowflake guide on managing data streams and lifecycle features.
6. [Databricks Lakehouse Platform Administration](https://docs.databricks.com/administration-guide/index.html) - Databricks documentation on managing lifecycle and governance of data tables.
7. [Apache Kafka Data Retention](https://kafka.apache.org/documentation/#retention) - Apache Kafka official guide on setting event lifecycle and retention limits.
