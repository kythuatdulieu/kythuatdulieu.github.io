...
definition: "Hệ thống Nguồn (Source Systems) là nơi dữ liệu thô ban đầu được sinh ra từ các giao dịch, nhật ký hoạt động hoặc thiết bị IoT, đóng vai trò là điểm khởi đầu cho mọi đường ống dữ liệu."
...
![Sơ đồ minh họa kiến trúc Source Systems](/images/source-systems/diagram_1.svg)
...
![Quy trình xử lý và luồng dữ liệu Source Systems](/images/source-systems/diagram_2.png)
...
1. **Batch Extraction (Trích xuất theo lô)**:
   ...
   * **[Incremental Load](/concepts/3-integration/etl-elt/incremental-load/)**: Chỉ đọc các dòng mới hoặc vừa cập nhật dựa trên cột `updated_at`.
2. **[Change Data Capture](/concepts/3-integration/etl-elt/change-data-capture/) (CDC)**: Đọc trực tiếp từ file nhật ký ghi (Transaction Log/Binlog) của database để lấy các thay đổi (INSERT, UPDATE, DELETE) ngay lập tức mà không cần câu lệnh `SELECT` làm nặng hệ thống.
...
```
Sau đó, sự kiện này được đẩy vào Kafka và cuối cùng lưu vào [Data Lake](/concepts/2-storage/data-lake-lakehouse/data-lake/). Toàn bộ quá trình này không gửi bất kỳ câu lệnh `SELECT` nào vào MySQL, giúp bảo vệ hệ thống nguồn.
...
Việc kết nối trực tiếp và trích xuất dữ liệu từ các hệ thống nguồn là cần thiết khi:
* Doanh nghiệp cần xây dựng kho dữ liệu tập trung (Data Warehouse) để phục vụ cho việc phân tích toàn diện.
* Cần đồng bộ hóa dữ liệu từ các ứng dụng tác nghiệp (như ERP, CRM, Web App) để theo dõi các chỉ số kinh doanh theo thời gian thực hoặc hàng ngày.
* Cần xây dựng các mô hình dự báo, ML/AI sử dụng dữ liệu thô phát sinh từ hành vi người dùng (clickstreams, logs) hoặc thiết bị IoT.

Tránh kết nối và trích xuất trực tiếp khi:
* Có thể sử dụng các báo cáo tích hợp sẵn ngay trong hệ thống tác nghiệp đó (ví dụ, Salesforce hoặc Google Analytics đã có sẵn dashboard đầy đủ cho nhu cầu hiện tại).
* Quy định bảo mật nghiêm ngặt ngăn cấm hoàn toàn việc truy cập trực tiếp vào cơ sở dữ liệu OLTP vận hành của ứng dụng lõi. Trong trường hợp đó, hệ thống nguồn cần chủ động xuất file mã hóa hoặc đẩy dữ liệu qua SFTP an toàn.

---

## Trọng tâm ôn luyện phỏng vấn

### 1. Tại sao không nên chạy trực tiếp các câu truy vấn SELECT phức tạp trên hệ thống nguồn OLTP?
* **Gợi ý trả lời**: Hệ thống nguồn OLTP được thiết kế và tối ưu cho việc ghi nhận giao dịch nhanh chóng với độ trễ cực thấp để phục vụ người dùng cuối. Chạy các câu truy vấn SELECT phức tạp (JOIN nhiều bảng, quét toàn bộ bảng dữ liệu hàng triệu dòng) sẽ làm khóa bảng (table locking), tiêu tốn tài nguyên CPU/RAM, gây chậm hoặc sập hệ thống vận hành. Để tránh việc này, chúng ta nên trích xuất dữ liệu từ Read Replica (bản sao chỉ đọc) hoặc sử dụng cơ chế CDC (Change Data Capture) không xâm lấn.

### 2. Sự khác biệt giữa Full Ingestion và Incremental Ingestion khi trích xuất dữ liệu từ hệ thống nguồn?
* **Gợi ý trả lời**:
  * **Full Ingestion**: Đọc và sao chép toàn bộ dữ liệu từ hệ thống nguồn sang hệ thống đích trong mỗi lần chạy. Phù hợp cho các bảng dữ liệu nhỏ, tĩnh hoặc ít thay đổi (ví dụ: bảng danh mục sản phẩm, bảng cấu hình tỉnh thành).
  * **Incremental Ingestion**: Chỉ trích xuất các bản ghi mới được tạo hoặc có sự thay đổi kể từ lần chạy cuối cùng, thường dựa trên các cột mốc thời gian như `created_at` hoặc `updated_at`. Phù hợp cho các bảng giao dịch lớn (ví dụ: bảng đơn hàng, log lượt click) để tiết kiệm băng thông và tài nguyên tính toán.

## Khái niệm liên quan

* [OLTP (Hệ thống giao dịch)](/concepts/2-storage/database-storage/oltp/) - Hệ thống xử lý giao dịch trực tuyến.
* [Data Pipeline (Đường ống dữ liệu)](/concepts/1-foundations/foundation/data-pipeline/) - Đường ống luân chuyển dữ liệu.
* [Schema Drift (Trôi dạt cấu trúc)](/concepts/5-quality-governance/observability-reliability/schema-drift/) - Hiện tượng thay đổi cấu trúc dữ liệu nguồn.

## Xem thêm các khái niệm liên quan
* [Vai trò Kỹ sư Dữ liệu](/concepts/1-foundations/foundation/data-engineering/) - Người thiết kế pipeline lấy dữ liệu từ nguồn.
* [Kỹ thuật Dữ liệu](/concepts/1-foundations/foundation/data-engineering/) - Khái niệm tổng quan về Data Engineering.
* [Vòng đời Dữ liệu](/concepts/1-foundations/foundation/data-lifecycle/) - Cách quản lý vòng đời dữ liệu từ nguồn.
* [Kiến trúc Nền tảng Dữ liệu](/concepts/1-foundations/system-architecture/data-platform-architecture/) - Thiết kế kiến trúc tiếp nhận hệ thống nguồn.
...
4. [Google Cloud Data Integration](https://cloud.google.com/solutions/data-integration) - Google Cloud solutions for connecting and integrating diverse operational sources.
5. [Snowflake Ingesting Data](https://docs.snowflake.com/en/user-guide/data-load-overview) - Snowflake official guide on loading data from external source systems and cloud storage.
6. [Confluent Change Data Capture (CDC)](https://www.confluent.io/learn/change-data-capture/) - Confluent learning center article on the mechanics of CDC for streaming [data ingestion](/concepts/3-integration/etl-elt/data-ingestion/).

## English Summary

Source Systems are the operational applications, databases, APIs, and logs where data originates. In [data engineering](/concepts/1-foundations/foundation/data-engineering/), extracting data safely from these systems is critical. Methods range from periodic batch extraction (Full or Incremental) using API polling or SQL queries, to real-time Change Data Capture (CDC) that reads database transaction logs. Best practices mandate protecting the performance of source systems, often by extracting from read replicas or using log-based CDC rather than heavy direct queries.