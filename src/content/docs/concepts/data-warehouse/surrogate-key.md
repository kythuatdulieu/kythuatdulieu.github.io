...
Trong thiết kế kho dữ liệu ([Data Warehouse](/concepts/data-warehouse/data-warehouse/)), **Surrogate Key (Khóa thay thế)** đóng vai trò như một khái niệm kỹ thuật cốt lõi và là một "lớp khiên" bảo vệ hệ thống. Đây là một mã định danh duy nhất (thường là một chuỗi số nguyên tự tăng hoặc mã băm Hash) do chính hệ thống lưu trữ phân tích sinh ra để làm Khóa chính (Primary Key) cho các bảng Dimension (Bảng chiều). 

Đặc điểm lớn nhất của Surrogate Key là nó hoàn toàn vô nghĩa đối với hoạt động kinh doanh (no business logic). Mục đích duy nhất của nó là giúp Data Warehouse độc lập hoàn toàn với các khóa tự nhiên (Natural Keys) – những mã định danh vốn thường xuyên biến động và chứa đầy rủi ro từ các hệ thống vận hành ([OLTP](/concepts/database-storage/oltp/)).
...
* **Surrogate Key (Khóa thay thế):** Là một mã định danh (thường là số nguyên tự tăng như `1`, `2`, `3`... hoặc chuỗi mã băm) được tạo ra hoàn toàn tự động bởi quy trình [ETL](/concepts/etl-elt/etl/) của Data Warehouse. Nó không mang bất kỳ giá trị hay ý nghĩa thực tế nào ngoài việc định danh nội bộ.
...
Trong môi trường Cloud Data Warehouse hiện đại (như BigQuery, [Snowflake](/concepts/cloud-data-platform/snowflake/)) kết hợp với các công cụ như dbt, việc duy trì các cột số nguyên tự tăng (Auto-increment) truyền thống dễ tạo ra hiện tượng "cổ chai" (bottleneck) về mặt hiệu năng do tính chất xử lý phân tán song song.
...
### Điểm yếu
* **Gia tăng độ phức tạp cho quy trình ETL/[ELT](/concepts/etl-elt/elt/):** Hệ thống cần thêm một bước đối chiếu (Lookup) hoặc tính toán hàm băm để dịch từ Natural Key sang Surrogate Key trước khi nạp dữ liệu vào bảng Fact.

## Khi nào nên dùng và Khi nào nên tránh?

**Bắt buộc sử dụng khi:**
* Bạn thiết kế các bảng Dimension theo mô hình [Dimensional Modeling](/concepts/data-warehouse/dimensional-modeling/) (phương pháp Kimball).
...
1. [The Data Warehouse Toolkit, 3rd Edition](https://www.oreilly.com/library/view/the-data-warehouse/9781118530801/) - Ralph Kimball's guide defining surrogate keys in multidimensional modeling on O'Reilly.
2. [Surrogate Keys Keep Control](https://www.kimballgroup.com/2004/03/surrogate-keys-keep-control/) - Ralph Kimball's article on the essential role of surrogate keys in the Kimball Group archives.
3. [Generating surrogate keys with dbt](https://docs.getdbt.com/blog/2022/09/27/generating-surrogate-keys-with-dbt) - Best practices on utilizing cryptographic hashes to construct surrogate keys in dbt Developer Blog.
4. [Surrogate Key](https://en.wikipedia.org/wiki/Surrogate_key) - Wikipedia's definition and comparison of natural and surrogate key architectures in databases.
5. [Difference between Natural Key and Surrogate Key](https://www.geeksforgeeks.org/difference-between-natural-key-and-surrogate-key/) - Structured comparison between real-world natural keys and system-generated surrogate keys on GeeksforGeeks.
