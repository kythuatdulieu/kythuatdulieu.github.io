...
Về mặt bản chất, **Data Catalog** là một siêu nền tảng `(Platform)` chuyên làm nhiệm vụ thu thập, quản lý và tổ chức Siêu dữ liệu `(Metadata)` từ mọi ngóc ngách trong hệ thống công nghệ của doanh nghiệp – từ các cơ sở dữ liệu nguồn, [Data Warehouse](/concepts/data-warehouse/data-warehouse/), cho đến các [ETL](/concepts/etl-elt/etl/) pipeline và các báo cáo BI Dashboard. Nó cung cấp một giao diện web trực quan, hoạt động giống như một thanh công cụ tìm kiếm Google nội bộ dành riêng cho dữ liệu.
...
3. **[Data Lineage](/concepts/governance-metadata/data-lineage/) (Bản đồ phả hệ dữ liệu)**: Vẽ ra sơ đồ trực quan thể hiện đường đi của dữ liệu từ hệ thống gốc, qua các bước biến đổi của ETL, cho tới khi hiển thị trên các biểu đồ Dashboard cuối cùng.
...
* Đang triển khai kiến trúc [Data Lake](/concepts/data-lake-lakehouse/data-lake/)/Data [Lakehouse](/concepts/data-lake-lakehouse/lakehouse/) – nơi dữ liệu phi cấu trúc đổ về ồ ạt như một đầm lầy, nếu không có Catalog thì không ai biết bên trong đầm lầy đó chứa những gì.
