...
1. **Sinh log (Generation):** Database Engine (như PostgreSQL, [Snowflake](/concepts/cloud-data-platform/snowflake/), BigQuery) được bật cấu hình Audit Logging ở cấp hệ thống. Mọi truy vấn gửi tới cơ sở dữ liệu sẽ bị chặn lại để ghi một bản sao chi tiết vào file log nội bộ.
...
3. **Lưu trữ an toàn (Storage):** Log được lưu trữ tại một nơi độc lập, thường là hệ thống SIEM (Security Information and Event Management) như Splunk, Elastic Security, hoặc một [Data Lake](/concepts/data-lake-lakehouse/data-lake/) an toàn có hỗ trợ tính năng chống ghi đè (WORM - Write Once, Read Many).
...
* [Kiểm soát truy cập - Access Control](/concepts/governance-metadata/access-control/)
