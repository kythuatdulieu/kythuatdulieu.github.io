...
* **Khám phá dữ liệu (Data Discovery)**: Quét tự động toàn bộ kho lưu trữ ([Data Warehouse](/concepts/data-warehouse/data-warehouse/), [Data Lake](/concepts/data-lake-lakehouse/data-lake/)) để nhận diện các mẫu dữ liệu nhạy cảm thông qua các thuật toán so khớp chuỗi (Regex) hoặc Học máy (Machine Learning).
...
* **Quản lý siêu dữ liệu ([Metadata Management](/concepts/governance-metadata/metadata-management/))**: Lưu trữ các nhãn phân loại này tập trung trong các hệ thống Data Catalog để làm cơ sở cho việc phân quyền tự động `(Access Control)` và che giấu dữ liệu `(Data Masking)`.
...
* **Mục đích câu hỏi**: Đánh giá sự hiểu biết của ứng viên về bức tranh tổng thể của quản trị dữ liệu ([Data Governance](/concepts/governance-metadata/data-governance/)).
* **Gợi ý trả lời**:
...
**Khái niệm liên quan:**
* [Access Control (Kiểm soát truy cập)](/concepts/governance-metadata/access-control/)
* [Audit Logging (Nhật ký kiểm toán)](/concepts/governance-metadata/audit-logging/)
