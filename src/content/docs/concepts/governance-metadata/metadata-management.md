...
3. **Operational Metadata (Siêu dữ liệu vận hành)**: Mô tả trạng thái hoạt động của hệ thống và các đường ống dẫn dữ liệu (data pipelines).
   * *Ví dụ*: Đường ống [ETL](/concepts/etl-elt/etl/) vừa hoàn thành lúc 2:00 AM, chạy mất 15 phút, đã chèn thêm 10.000 dòng dữ liệu mới và không phát hiện lỗi nào.
...
1. **Tự động thu thập (Ingestion)**: Các bộ quét (crawlers) của hệ thống quản lý metadata (như DataHub, OpenMetadata) sẽ tự động kết nối qua API tới [Data Warehouse](/concepts/data-warehouse/data-warehouse/) và các công cụ lập lịch (Airflow, [dbt](/concepts/transformation-analytics/dbt/)) để lấy schema, lịch sử chạy và logs của hệ thống theo chu kỳ ngắn.
