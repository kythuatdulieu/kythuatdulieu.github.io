...
### So sánh Kiến trúc ETL Truyền thống và ELT/Lakehouse Hiện đại

```mermaid
graph TD
    subgraph Traditional ETL
        S1_ETL[Hệ thống Nguồn] --> E_ETL[Extract]
        E_ETL --> T_ETL[Transform Server / ETL Tool]
        T_ETL --> L_ETL[Load]
        L_ETL --> DWH_ETL[Data Warehouse]
    end

    subgraph Modern ELT / Lakehouse
        S1_ELT[Hệ thống Nguồn] --> E_ELT[Extract]
        E_ELT --> L_ELT[Load]
        L_ELT --> DL_ELT[(Data Lake / Landing Zone)]
        DL_ELT --> T_ELT[Transform / SQL]
        T_ELT --> DWH_ELT[(Data Warehouse / Lakehouse)]
    end
```

Kiến trúc ETL truyền thống thực hiện việc biến đổi dữ liệu trên một máy chủ trung gian trước khi nạp vào kho lưu trữ. Điều này phù hợp khi tài nguyên của kho dữ liệu đắt đỏ. Ngược lại, kiến trúc ELT hiện đại (kết hợp Data Lake và Lakehouse) tận dụng sức mạnh tính toán khổng lồ và chi phí lưu trữ rẻ của Cloud để nạp dữ liệu thô vào trước, sau đó mới biến đổi ngay bên trong nền tảng lưu trữ.
...
## Hiện tượng Data Skew và Bottlenecks trong Data Pipeline

Trong quá trình vận hành các Data Pipeline quy mô lớn, kỹ sư dữ liệu thường phải đối mặt với hai vấn đề hiệu suất nghiêm trọng: Data Skew và Bottlenecks.

### 1. Nút thắt cổ chai (Bottlenecks)
**Nút thắt cổ chai** xảy ra khi một thành phần trong pipeline xử lý chậm hơn nhiều so với các thành phần khác, khiến toàn bộ luồng dữ liệu bị ùn ứ.
* **Nguyên nhân**: Băng thông mạng bị giới hạn, tài nguyên CPU/RAM tại máy chủ biến đổi (Transform) không đủ, hoặc hệ thống đích (Database/Data Warehouse) bị quá tải do phải nhận quá nhiều kết nối (connection) cùng lúc.
* **Cách khắc phục**: 
  * Áp dụng xử lý song song (Parallel Processing) bằng cách chia nhỏ dữ liệu thành các lô (chunk) để xử lý đồng thời.
  * Tăng cường tài nguyên (Scale-up/Scale-out) tại các khâu xử lý nặng.
  * Tối ưu hóa câu truy vấn và sử dụng cơ chế Bulk Insert/Copy thay vì chèn từng dòng lệnh `INSERT` riêng lẻ.

### 2. Lệch dữ liệu (Data Skew)
**Data Skew** là hiện tượng dữ liệu phân bổ không đồng đều trên các node xử lý phân tán (như trong Apache Spark hay Hadoop). Một node có thể phải gánh lượng dữ liệu gấp hàng chục lần các node khác, khiến các node khác rảnh rỗi chờ đợi trong khi node đó bị quá tải.
* **Nguyên nhân**: Thường xảy ra khi thực hiện phép toán `JOIN` hoặc `GROUP BY` trên một khóa (key) có giá trị phổ biến lặp lại quá nhiều (ví dụ khóa `country_code='VN'` chiếm 90% dữ liệu).
* **Cách khắc phục**:
  * **Salting**: Thêm một chuỗi ngẫu nhiên (salt) vào các khóa bị lệch để phân tán chúng ra nhiều node khác nhau trước khi thực hiện gom nhóm hoặc join.
  * **Broadcast Join**: Khi thực hiện join một bảng lớn với một bảng rất nhỏ, hãy đưa (broadcast) toàn bộ bảng nhỏ đến tất cả các node để tránh việc phải trộn (shuffle) dữ liệu của bảng lớn qua mạng.
  * Lựa chọn cẩn thận khóa chia vùng (Partition Key) phù hợp với đặc thù phân bố của dữ liệu.
...
* [What is a Data Pipeline?](https://www.ibm.com/topics/data-pipeline) - IBM guide detailing definitions, components, and best practices of data pipelines.
* [What is a Data Pipeline?](https://www.databricks.com/glossary/data-pipeline) - Databricks Glossary explaining different data pipeline architectures.
* [What is a Data Pipeline?](https://cloud.google.com/discover/what-is-a-data-pipeline) - Google Cloud Learn page explaining stages and technologies for building data pipelines.
* [What is a Data Pipeline?](https://aws.amazon.com/what-is/data-pipeline/) - AWS page introducing the concepts, benefits, and use cases of data pipelines.
* [Apache Airflow Documentation](https://airflow.apache.org/docs/) - Official documentation for the industry-standard workflow [orchestration](/concepts/orchestration/orchestration/) platform.
