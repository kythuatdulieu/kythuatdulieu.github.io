...
## Luồng Xử lý Dữ liệu Thực tế và Rủi ro Production (Data Engineering Lifecycle)

Trong ngữ cảnh Kỹ thuật Dữ liệu (Data Engineering), vòng đời dữ liệu hoạt động hay luồng xử lý dữ liệu thường được biểu diễn cụ thể hơn qua 5 chặng cốt lõi, từ lúc sinh ra đến khi phục vụ người dùng. Mỗi chặng đều tiềm ẩn những rủi ro khi vận hành thực tế trên môi trường Production:

```mermaid
graph LR
    A(1. Generation) --> B(2. Ingestion)
    B --> C(3. Storage)
    C --> D(4. Processing)
    D --> E(5. Serving)
...
1. **Generation (Sinh dữ liệu)**
   * Nguồn: Ứng dụng Web/App, IoT sensors, Database OLTP, API bên thứ ba.
   * **Rủi ro Production**: **Schema drift** (sự thay đổi cấu trúc dữ liệu không báo trước từ phía nguồn như thêm/xóa cột, đổi kiểu dữ liệu) làm gãy Data Pipeline; hoặc mất kết nối (Outage) từ hệ thống nguồn dẫn đến mất mát dữ liệu.
2. **Ingestion (Thu nạp dữ liệu)**
   * Nạp dữ liệu theo Batch (hàng ngày/giờ) hoặc Streaming (thời gian thực) bằng các công cụ như Kafka, Fivetran, Airbyte.
   * **Rủi ro Production**: Quá tải (Spike) lưu lượng khiến hệ thống Ingestion bị sập (OOM - Out of Memory); độ trễ cao (High Latency) làm dữ liệu đến trễ so với SLA; hoặc bản tin bị lặp lại/thất thoát (Data duplication/loss).
3. **Storage (Lưu trữ)**
   * Lưu trữ vào Data Lake (S3, GCS) hoặc Data Warehouse.
   * **Rủi ro Production**: File rác sinh ra quá nhiều làm tăng chi phí lưu trữ ngoài tầm kiểm soát (Runaway costs); rủi ro bảo mật do cấu hình phân quyền (IAM) lỏng lẻo khiến dữ liệu nhạy cảm bị lộ; hoặc giới hạn số lượng kết nối đồng thời.
4. **Processing (Chế biến dữ liệu)**
   * Thực hiện ETL/ELT bằng Spark, dbt, SQL.
   * **Rủi ro Production**: Job chạy quá lâu gây Timeout hoặc tắc nghẽn (Bottleneck); lỗi logic nghiệp vụ làm sai lệch số liệu báo cáo (Data Quality issues); hoặc phụ thuộc vòng tròn (Circular Dependency) giữa các task trong luồng điều phối (Airflow).
5. **Serving (Phục vụ dữ liệu)**
   * Cung cấp dữ liệu qua BI Dashboards (Tableau, PowerBI), Data API, hoặc Machine Learning Feature Store.
   * **Rủi ro Production**: Dashboard bị treo do truy vấn SQL quá nặng; API phục vụ ML Model bị sập do lượng gọi quá lớn (DDoS hoặc Traffic Spike); hoặc dữ liệu phục vụ (Serving) bị lỗi thời do các chặng trước bị trễ (Stale Data).
...
* [DAMA International DMBOK](https://www.dama.org/cpages/dmbok) - Official page for the DAMA-DMBOK guide on [data governance](/concepts/governance-metadata/data-governance/) and lifecycle management.
* [Amazon S3 Lifecycle Policies](https://docs.aws.amazon.com/AmazonS3/latest/userguide/object-lifecycle-mgmt.html) - AWS documentation on configuring rules to transition or expire objects automatically.
* [Cloud Storage Object Lifecycle Management](https://cloud.google.com/storage/docs/lifecycle) - Google Cloud documentation on defining lifecycle policies for [cloud storage](/concepts/cloud-data-platform/cloud-storage/) buckets.
* [Microsoft Purview Data Lifecycle Management](https://learn.microsoft.com/en-us/purview/data-lifecycle-management) - Microsoft official documentation on retention and deletion policies.
* [Managing BigQuery Tables (Table Expiration)](https://cloud.google.com/bigquery/docs/managing-tables#table-expiration) - Google Cloud BigQuery documentation for setting automatic partition/table time-to-live policies.
* [Fundamentals of Data Engineering (O'Reilly)](https://www.oreilly.com/library/view/fundamentals-of-data/9781098108298/) - Comprehensive guide covering the entire data engineering lifecycle from generation to serving.
