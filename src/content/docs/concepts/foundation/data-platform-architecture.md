...
Dưới đây là sơ đồ kiến trúc tổng thể của một Modern Data Platform bao gồm các thành phần cốt lõi: Ingestion, Lakehouse, Compute, BI, và DataOps.

```mermaid
graph LR
    subgraph Ingestion
        A1[Batch Ingestion]
        A2[Streaming Ingestion]
    end
    
    subgraph Lakehouse
        B1[Cloud Object Storage]
        B2[Open Table Formats]
    end
...
* **Lớp Vàng (Gold / Aggregated)**: Dữ liệu được tổng hợp theo các chỉ số kinh doanh nghiệp vụ, thiết kế dưới dạng [Star Schema](/concepts/data-warehouse/star-schema/) phục vụ trực tiếp và nhanh chóng cho các báo cáo của ban giám đốc.
...
### Build vs. Buy: Tự xây hay Sử dụng dịch vụ Managed?

Trong quá trình thiết kế Data Platform, các tổ chức luôn phải đối mặt với bài toán đánh đổi giữa việc tự xây dựng toàn bộ (Build) và sử dụng các dịch vụ có sẵn (Buy).

* **Build (Tự xây dựng bằng Open Source)**:
  * **Ưu điểm**: Doanh nghiệp có toàn quyền kiểm soát hệ thống, tùy biến sâu phù hợp với nghiệp vụ đặc thù, và chi phí cấp phép phần mềm (license) bằng 0. Không bị phụ thuộc vào bất kỳ nhà cung cấp dịch vụ đám mây nào (vendor lock-in).
  * **Nhược điểm**: Đòi hỏi đội ngũ Data Engineer, DevOps giỏi để thiết lập, vận hành, bảo trì và đảm bảo tính sẵn sàng (HA). Tổng chi phí sở hữu (TCO) thường cao hơn dự kiến do chi phí nhân sự và thời gian triển khai (time-to-market) kéo dài.
* **Buy (Sử dụng Managed Cloud Services)**:
  * **Ưu điểm**: Rút ngắn tối đa thời gian triển khai (time-to-market). Các vấn đề về hạ tầng, bảo mật, và mở rộng tự động (auto-scaling) đều do nhà cung cấp (như AWS, GCP, Snowflake, Databricks) xử lý. Đội ngũ dữ liệu có thể tập trung vào việc xây dựng luồng xử lý tạo ra giá trị kinh doanh.
  * **Nhược điểm**: Chi phí dịch vụ có thể tăng nhanh theo khối lượng dữ liệu và tần suất xử lý. Có rủi ro bị phụ thuộc vào hệ sinh thái của một nhà cung cấp cụ thể (vendor lock-in), gây khó khăn khi muốn chuyển đổi nền tảng sau này.
...
* **Sử dụng định dạng tệp tin mở (Open Formats)**: Hãy lưu dữ liệu dưới định dạng Parquet, Iceberg hoặc [Delta Lake](/concepts/data-lake-lakehouse/delta-lake/). Việc này giúp bạn dễ dàng chuyển đổi nhà cung cấp công nghệ trong tương lai (ví dụ từ [Snowflake](/concepts/cloud-data-platform/snowflake/) sang Databricks) mà không bị khóa chặt vào một hệ sinh thái độc quyền (vendor lock-in).
* **Tránh biến Data Lake thành "đầm lầy dữ liệu"**: Luôn kết hợp công cụ [Data Catalog](/concepts/governance-metadata/data-catalog/) để quản lý siêu dữ liệu (metadata) và định nghĩa từ điển dữ liệu rõ ràng. Đừng đổ đống dữ liệu lên cloud mà không phân chia thư mục logic.
* **Sai lầm: Không quản lý vòng đời dữ liệu ([Data Lifecycle](/concepts/foundation/data-lifecycle/))**: Lưu giữ dữ liệu thô ở lớp Bronze vô thời hạn mà không thiết lập chính sách lưu trữ dài hạn (archiving) hoặc tự động xóa bỏ dữ liệu cũ, dẫn đến chi phí lưu trữ tăng vọt.
* **Sai lầm: Chọn công cụ quá phức tạp khi chưa cần thiết**: Áp dụng [Apache Spark](/concepts/batch-processing/apache-spark/) và các định dạng Lakehouse phức tạp cho các tập dữ liệu nhỏ dưới vài chục GB, trong khi một cơ sở dữ liệu quan hệ đơn giản (như PostgreSQL) là đủ để giải quyết bài toán nhanh và rẻ hơn nhiều.
...
* **Gợi ý trả lời**: Việc phân tách Compute và Storage giải quyết bài toán lãng phí tài nguyên của kiến trúc truyền thống. Trước đây, khi muốn lưu trữ nhiều dữ liệu hơn, doanh nghiệp phải mua thêm máy chủ có cả ổ cứng và CPU, dẫn đến tình trạng thừa năng lực tính toán nhưng thiếu dung lượng lưu trữ (hoặc ngược lại). Với [Modern Data Stack](/concepts/system-architecture/modern-data-stack/), dữ liệu được lưu trữ trên Cloud Object Storage (như S3) cực kỳ rẻ và vô hạn, trong khi tài nguyên tính toán (như Snowflake, Databricks) chỉ được bật lên khi có nhu cầu truy vấn hoặc chạy [ETL](/concepts/etl-elt/etl/). Điều này giúp tối ưu hóa chi phí vận hành một cách tối đa.
...
* [What is a Data Lakehouse?](https://www.databricks.com/glossary/data-lakehouse) - Databricks Glossary page on the hybrid data lakehouse paradigm.
* [What is Data Architecture?](https://www.snowflake.com/trending/what-is-data-architecture) - Snowflake comprehensive guide on enterprise data platform design.
* [Modern Data Architecture on AWS](https://docs.aws.amazon.com/wellarchitected/latest/analytics-lens/modern-data-architecture.html) - AWS Well-Architected Analytics Lens documentation for designing scalable data architectures.
* [Big Data Architecture Style](https://learn.microsoft.com/en-us/azure/architecture/guide/architecture-styles/big-data) - Microsoft Azure Architecture Center guide for designing large-scale big data architectures.
* [Data Mesh Book](https://www.oreilly.com/library/view/data-mesh/9781492092384/) - O'Reilly page for Zhamak Dehghani's foundational book on decentralized data platform design.

## English Summary

The **Data Platform Architecture** outlines the overarching design of an organization's data ecosystem. The industry has evolved from centralized Data Warehouses (optimized for structured BI) to Data Lakes (cheap storage for vast amounts of unstructured raw data for ML), and now toward the Data Lakehouse paradigm. A Lakehouse combines the cost-efficiency and flexibility of a Data Lake (using open formats like Parquet/Iceberg on [cloud storage](/concepts/cloud-data-platform/cloud-storage/) like S3) with the reliability, ACID transactions, and fast SQL performance of a Data Warehouse. A typical Modern Data Stack decouples storage from compute and utilizes tools for ingestion, transformation (like [dbt](/concepts/transformation-analytics/dbt/)), and [orchestration](/concepts/orchestration/orchestration/) (like Airflow).