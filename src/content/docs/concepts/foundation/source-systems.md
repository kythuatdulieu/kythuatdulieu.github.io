...
## Mổ xẻ Kiến trúc / Under the hood
```mermaid
graph LR
...
### Đánh đổi Thiết kế (Trade-offs): Pull-based vs Push-based

Khi thiết kế luồng thu thập dữ liệu từ Source Systems, Data Engineer phải quyết định chọn kiến trúc Pull-based hay Push-based:

* **Pull-based (Kéo dữ liệu)**: Data Platform chủ động truy vấn hoặc gọi API định kỳ (batch/cron job) để "kéo" dữ liệu từ Source Systems về.
  * **Ưu điểm**: Data Team hoàn toàn chủ động kiểm soát lịch trình, tần suất và lượng dữ liệu lấy mỗi lần. Nguồn không cần tùy biến nhiều.
  * **Nhược điểm**: Gây áp lực cực đại (spike load) lên hệ thống nguồn tại thời điểm pull, đồng thời dữ liệu bị trễ (latency cao).
* **Push-based (Đẩy dữ liệu)**: Hệ thống nguồn chủ động đẩy (push) thông tin mỗi khi có sự kiện/thay đổi xảy ra (ví dụ: Webhooks, Streaming qua Kafka, CDC trực tiếp đẩy event).
  * **Ưu điểm**: Dữ liệu có mặt ở Data Platform với độ trễ cực thấp (near real-time), tải hệ thống được chia đều đặn thay vì dồn cục.
  * **Nhược điểm**: Đòi hỏi hệ thống nguồn phải có năng lực phát sự kiện (event-driven), phức tạp hơn khi triển khai và phải xử lý chặt chẽ vấn đề mất mát thông báo (message loss) hoặc trùng lặp (duplicate).
...
* [Debezium Documentation](https://debezium.io/documentation/reference/stable/) - Red Hat open source platform for monitoring database transaction logs and capturing change data.
* [Databricks Lakeflow](https://www.databricks.com/product/lakeflow) - Databricks product page explaining managed database replication and source system connectivity.
* [AWS Database Migration Service Welcome Guide](https://docs.aws.amazon.com/dms/latest/userguide/Welcome.html) - AWS official guide on migrating and replicating data from source databases.
* [What is Data Integration?](https://www.ibm.com/topics/data-integration) - IBM guide to consolidating and integrating diverse data sources.
* [What is Change Data Capture (CDC)?](https://www.confluent.io/learn/change-data-capture/) - Confluent learning center article on the mechanics of CDC for streaming [data ingestion](/concepts/etl-elt/data-ingestion/).
