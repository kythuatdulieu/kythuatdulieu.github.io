...
## Hệ sinh thái công nghệ (Technology Landscape)

Dưới đây là sơ đồ tổng quan về các mảnh ghép công nghệ phổ biến thường được sử dụng trong hệ thống Data Engineering hiện đại:
...
graph TD
    ...
    subgraph Storage [Lưu Trữ]
        ST1(Amazon S3 / GCS)
        ST2(Snowflake / BigQuery)
        ST3(Delta Lake / Iceberg)
    end
    subgraph Processing [Xử Lý & Biến Đổi]
        P1(Apache Spark)
        P2(dbt)
        P3(Apache Flink)
    end
    ...
    ST1 --> P3
    P3 --> ST2
    O1 -. "Quản lý luồng" .-> Ingestion
    O1 -. "Lập lịch chạy" .-> Processing
```

## Under the hood: Sự dịch chuyển từ Batch sang Streaming trong thập kỷ qua

Trong khoảng 10 năm trở lại đây, kiến trúc dữ liệu đã chứng kiến một cuộc chuyển giao mạnh mẽ: từ việc xử lý dữ liệu theo lô (`Batch Processing`) với độ trễ tính bằng ngày hoặc giờ, sang xử lý dữ liệu luồng (`Streaming Processing`) với độ trễ tính bằng mili-giây.

* **Thời kỳ Batch Processing**: Ban đầu, các tổ chức chủ yếu dựa vào hệ sinh thái Hadoop và các luồng ETL chạy qua đêm. Dữ liệu được gom lại thành các tập lớn (batch) và xử lý đồng loạt. Cách tiếp cận này tiết kiệm tài nguyên nhưng lại tạo ra độ trễ lớn, khiến doanh nghiệp phản ứng chậm (ví dụ: phát hiện gian lận sau khi giao dịch đã hoàn tất nhiều giờ).
* **Kỷ nguyên Streaming và Real-time**: Sự bùng nổ của mạng xã hội, thiết bị di động và IoT đòi hỏi phản hồi tức thì. Các công nghệ như **Apache Kafka** và **Apache Flink** đã thay đổi luật chơi. Kafka đóng vai trò như một hệ thống thần kinh trung ương, tiếp nhận và phân phối hàng triệu sự kiện mỗi giây. Thay vì đợi dữ liệu nằm yên trong kho, hệ thống cho phép "tính toán ngay trên đường bay".
* **Sự khác biệt Under the hood**: Về mặt kỹ thuật, cốt lõi của sự dịch chuyển nằm ở cách hệ thống quản lý trạng thái (`State`) và thời gian (`Time`). Batch processing làm việc với dữ liệu có giới hạn (`Bounded data`), biết chính xác khi nào bắt đầu và kết thúc để thực hiện tính toán tổng hợp. Ngược lại, Streaming đối mặt với dòng dữ liệu vô tận (`Unbounded data`) và các sự kiện đến trễ mạng (`Late arrivals`). Để giải quyết bài toán này, các kỹ sư dữ liệu phải áp dụng các cơ chế phức tạp như `Windowing` (chia dòng dữ liệu thành các cửa sổ thời gian) và `Watermarks` (chấp nhận sự kiện trễ ở mức độ cho phép) nhằm đảm bảo tính chính xác trong thời gian thực.
...
* [What is Data Engineering?](https://www.ibm.com/topics/data-engineering) - IBM comprehensive topic guide on data engineering definition and lifecycle.
* [What is Data Engineering?](https://www.databricks.com/glossary/data-engineering) - Databricks Glossary definition of data engineering and modern cloud [lakehouse](/concepts/data-lake-lakehouse/lakehouse/) solutions.
* [What is Data Engineering?](https://cloud.google.com/discover/what-is-data-engineering) - Google Cloud Learn page for data engineering concepts, tools, and certifications.
...
* [Designing Data-Intensive Applications Book](https://www.oreilly.com/library/view/designing-data-intensive-applications/9781491903063/) - O'Reilly page for Martin Kleppmann's classic book on distributed systems and data design.
* [Apache Kafka Documentation](https://kafka.apache.org/documentation/) - Official documentation for Apache Kafka, a key component in modern data streaming.
