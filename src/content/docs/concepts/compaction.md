...
Trong các kiến trúc Table Format hiện đại như Apache Hudi, Delta Lake hay Apache Iceberg, Compaction còn đảm nhận thêm việc dọn dẹp các bản ghi cũ bị xóa cứng `(deletions)` hoặc hợp nhất các file nhật ký biến động `(changelogs)` vào file dữ liệu gốc `(base files)` để tối ưu hóa tối đa cấu trúc lưu trữ.

## "Nỗi sợ hãi" mang tên Tệp tin nhỏ (Small Files Problem)

Tại sao các kỹ sư dữ liệu lại e sợ các file nhỏ đến vậy? Việc ghi dữ liệu liên tục theo giây/phút sẽ dẫn đến hiện tượng tích tụ hàng triệu file nhỏ trên Cloud Storage (như Amazon S3 hay GCS) sau một thời gian ngắn. Điều này dẫn tới 3 hậu quả nghiêm trọng:
...
Trong môi trường Delta Lake chạy trên nền Apache Spark, bạn có thể kích hoạt quy trình gom file thủ công cực kỳ đơn giản bằng lệnh `OPTIMIZE`:
...
* **Lên lịch chạy vào giờ thấp điểm**: Tiến trình gom file tiêu tốn rất nhiều CPU và tài nguyên mạng. Hãy thiết lập lịch chạy tự động `(Cron Job)` vào lúc nửa đêm hoặc cuối tuần khi hệ thống ít người dùng, tránh gây ảnh hưởng đến các job ETL nghiệp vụ chính vào ban ngày.
...
**Cần áp dụng khi:**
* Bạn xây dựng các hệ thống Data Lake / Data Lakehouse thu thập dữ liệu dạng streaming (từ Kafka, CDC) hoặc micro-batches liên tục.
...
* **Mục đích câu hỏi**: Đánh giá khả năng thiết kế và vận hành luồng dữ liệu (Data Pipeline Architecture) của ứng viên.
* **Gợi ý trả lời**:
