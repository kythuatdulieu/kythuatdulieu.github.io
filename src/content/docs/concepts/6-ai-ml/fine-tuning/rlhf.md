...
seoTitle: "RLHF là gì? Tinh chỉnh LLM từ phản hồi của con người"
...
definition: "RLHF (Reinforcement Learning from Human Feedback) là phương pháp tối ưu hóa mô hình ngôn ngữ lớn bằng học tăng cường dựa trên đánh giá của con người."
---
...
### Điểm mạnh (Pros)
* **Căn chỉnh tự nhiên:** Chuyển đổi mô hình từ một công cụ đoán từ thô sơ thành một trợ lý AI an toàn, có khả năng giao tiếp tự nhiên và hiểu ý người dùng.
* **Tự động hóa chấm điểm:** Reward Model đóng vai trò như một bộ lọc tự động thông minh, giúp chúng ta không cần tốn chi phí thuê con người chấm điểm hàng tỷ câu trả lời trong suốt quá trình huấn luyện lâu dài.
...
### Nên dùng:
* Khi cần căn chỉnh hành vi của mô hình ngôn ngữ sao cho thân thiện, lịch sự và tuân thủ các quy tắc đạo đức, an toàn của tổ chức.
* Khi muốn tối ưu hóa chất lượng câu trả lời của mô hình theo các tiêu chuẩn đánh giá định tính của con người mà không thể mô tả dễ dàng bằng quy tắc lập trình cứng.

### Không nên dùng:
* Khi mô hình chỉ cần thực hiện các tác vụ kỹ thuật thuần túy như phân tích log, viết code hoặc tính toán dữ liệu thô (giai đoạn SFT/Fine-tuning thông thường là đủ và không tốn kém chi phí như RLHF).

## Các khái niệm liên quan

* [Fine-tuning](/concepts/6-ai-ml/genai-ml/fine-tuning/)
* [LLM (Large Language Model)](/concepts/6-ai-ml/genai-ml/llm/)
...
* **Gợi ý trả lời**: SFT dạy mô hình bắt chước các câu chữ mẫu ở cấp độ từng từ ([token](/concepts/6-ai-ml/genai-ml/token/)-level) dựa trên hàm mất mát Cross-Entropy. Điều này khiến mô hình bị giới hạn bởi lượng dữ liệu mẫu có hạn và dễ gặp hiện tượng lệch phân phối (distribution shift) - tức là nếu mô hình viết sai một từ ở đầu câu, các từ sau sẽ bị sai hàng loạt. 
...
2. [AWS - Amazon Bedrock Model Customization with Reinforcement Learning](https://docs.aws.amazon.com/bedrock/latest/userguide/model-customization.html)
3. [Azure - Azure OpenAI Service Security and Alignment Guide](https://learn.microsoft.com/en-us/azure/ai-services/openai/concepts/security)
4. [Databricks - Model Fine-Tuning and Alignment](https://docs.databricks.com/en/large-language-models/index.html)
5. [Snowflake - Snowflake Cortex AI Safety Overview](https://docs.snowflake.com/en/user-guide/snowflake-cortex/llm-overview)
