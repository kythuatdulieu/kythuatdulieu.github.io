...
definition: "Tạo lập Truy xuất Tăng cường (RAG) là kiến trúc kết hợp khả năng ngôn ngữ của LLM với thông tin thực tế từ cơ sở dữ liệu ngoài để sinh câu trả lời chính xác và đáng tin cậy."
seoTitle: "RAG là gì? Tổng quan về Retrieval-Augmented Generation"
metaDescription: "Tìm hiểu kiến trúc Retrieval-Augmented Generation (RAG), cách giảm thiểu ảo giác của LLM bằng việc tích hợp cơ sở dữ liệu Vector để truy xuất thông tin."
---
...
* **Bộ nhớ thực tế (Non-parametric Memory)**: Toàn bộ dữ kiện, số liệu thực tế, tài liệu chính sách của doanh nghiệp... được lưu trữ bên ngoài trong các cơ sở dữ liệu tìm kiếm (như [Cơ sở dữ liệu Vector (Vector Database)](/concepts/6-ai-ml/genai-ml/vector-database/) hay Elasticsearch). Đây là "kho tri thức" có thể chỉnh sửa, thêm mới dễ dàng theo thời gian thực.
...
2. **Cắt nhỏ ([Chunking](/concepts/6-ai-ml/genai-ml/chunking/))**: Cắt văn bản dài thành các đoạn nhỏ (chunk) có kích thước tối ưu (ví dụ 512 hoặc 1024 tokens) và có một phần nội dung gối đầu (overlap) để giữ tính liên tục của ngữ cảnh.
3. **Mã hóa (Embedding)**: Chạy các đoạn text nhỏ này qua một [mô hình nhúng (Embedding Model)](/concepts/6-ai-ml/genai-ml/embedding-model/) để chuyển đổi chúng thành các vector toán học đa chiều biểu diễn ngữ nghĩa.
...
* **Nên dùng:**
  * Khi xây dựng các hệ thống hỏi đáp (Q&A) dựa trên các tài liệu nội bộ thay đổi liên tục của doanh nghiệp.
  * Khi cần một cơ chế giảm thiểu hiện tượng ảo giác (hallucination) của LLM bằng cách cung cấp dữ kiện có nguồn dẫn rõ ràng.
  * Khi cần phân quyền truy cập thông tin (Access Control) trên từng phân đoạn tài liệu trước khi LLM xử lý.
* **Không nên dùng:**
  * Khi bạn muốn LLM học các phong cách viết văn mới, thay đổi ngữ điệu hoặc học sâu một dạng cú pháp ngôn ngữ cụ thể (trong trường hợp này, Fine-tuning là lựa chọn tốt hơn).
  * Khi các câu trả lời hoàn toàn mang tính sáng tạo nghệ thuật và không cần bám vào bất kỳ dữ kiện thực tế nào.
...
* **Gợi ý trả lời**: Hiện tượng này xảy ra khi chúng ta nhồi nhét quá nhiều tài liệu ngữ cảnh vào prompt. Các nghiên cứu thực nghiệm chỉ ra rằng LLM có xu hướng chú ý và khai thác thông tin tốt nhất ở phần đầu và phần cuối của prompt, trong khi năng lực suy luận của nó bị sụt giảm nghiêm trọng đối với thông tin nằm ở đoạn giữa.
  ...
  2. Áp dụng mô hình [xếp hạng lại (Reranker)](/concepts/6-ai-ml/genai-ml/reranker/) để sắp xếp các tài liệu quan trọng nhất lên đầu tiên hoặc đẩy xuống cuối cùng trong vùng ngữ cảnh.
...
* [Google Cloud - Retrieval-Augmented Generation (RAG) Overview](https://cloud.google.com/vertex-ai/docs/generative-ai/agent-builder/rag-overview)
* [Databricks - Retrieval-Augmented Generation (RAG) Glossary](https://www.databricks.com/glossary/retrieval-augmented-generation-rag)
* [Confluent - Build RAG Applications with Confluent Cloud](https://www.confluent.io/blog/build-rag-application-with-confluent-cloud-and-mongodb/)
* [RAG Paper - Retrieval-Augmented Generation for Knowledge-Intensive NLP Tasks](https://arxiv.org/abs/2005.11401) - Lewis et al. (Facebook AI Research, 2020).
* [LlamaIndex - Official LlamaIndex Documentation](https://docs.llamaindex.ai/en/stable/)
