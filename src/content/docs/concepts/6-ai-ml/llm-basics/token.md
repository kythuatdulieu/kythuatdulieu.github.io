...
definition: "Token là đơn vị dữ liệu chữ viết nhỏ nhất mà mô hình ngôn ngữ lớn (LLM) có thể xử lý, có thể biểu diễn một ký tự, một âm tiết hoặc một từ hoàn chỉnh."
---
...
### Điểm mạnh (Pros)
* **Giải quyết lỗi Out-Of-Vocabulary (OOV):** Bất kỳ từ mới lạ hay viết sai chính tả nào cũng có thể tách thành các sub-word có sẵn trong từ điển.
* **Cân bằng hiệu năng tốt:** Giữ độ dài chuỗi đầu vào ở mức vừa phải (ngắn hơn character-level) và kích thước ma trận embedding tối ưu (nhỏ hơn word-level).

### Điểm yếu (Cons)
* **Bất bình đẳng ngôn ngữ:** Các ngôn ngữ phi tiếng Anh (như tiếng Việt) bị chẻ nhỏ hơn, làm tăng chi phí sử dụng API và nhanh cạn kiệt Context Window.
* **Mất mát ngữ nghĩa cục bộ:** Việc chia cắt từ đôi khi tạo ra các token không có nghĩa độc lập, gây khó khăn cho việc biểu diễn ngữ nghĩa của các mô hình nhỏ.

## Khi nào nên dùng

### Nên dùng:
* Khi xây dựng hoặc sử dụng các hệ thống xử lý ngôn ngữ tự nhiên (NLP) hiện đại dựa trên Transformer hoặc LLM.
* Khi làm việc với các tập dữ liệu đa ngôn ngữ hoặc các nguồn văn bản mở (chứa nhiều tiếng lóng, từ chuyên ngành mới).

### Không nên dùng:
* Khi thực hiện các bài toán phân loại văn bản thô cực kỳ đơn giản với số lượng từ khóa cố định, lúc này sử dụng Regular Expression hoặc Word-level Tokenizer truyền thống sẽ nhanh và nhẹ hơn.
...
1. [Google Cloud - Get Token Count in Vertex AI](https://cloud.google.com/vertex-ai/generative-ai/docs/multimodal/get-token-count)
2. [AWS - Amazon Bedrock Key Definitions & Tokens](https://docs.aws.amazon.com/bedrock/latest/userguide/key-definitions.html#tokens)
3. [Databricks - Managing Large Language Models](https://docs.databricks.com/en/large-language-models/index.html)
4. [Snowflake - Snowflake Cortex AI Overview](https://docs.snowflake.com/en/user-guide/snowflake-cortex/llm-overview)
5. [Hugging Face - Summary of Tokenizers](https://huggingface.co/docs/transformers/tokenizer_summary)
