...
definition: "System Prompt (Gợi ý hệ thống) là câu lệnh cấu hình ở mức hệ thống giúp thiết lập vai trò, quy tắc bảo mật, định dạng đầu ra và ngữ cảnh chung cho mô hình ngôn ngữ lớn trước cuộc trò chuyện."
seoTitle: "System Prompt là gì? Cách tối ưu gợi ý hệ thống cho LLM"
metaDescription: "Tìm hiểu chi tiết về System Prompt: vai trò thiết lập Persona, thiết lập rào cản bảo mật, tối ưu hóa ngữ cảnh và các câu hỏi phỏng vấn thực tế."
---
...
4. **Cung cấp ngữ cảnh nền (Context Grounding):** Trong các ứng dụng [RAG](/concepts/6-ai-ml/genai-ml/rag/) (Retrieval-Augmented Generation), tài liệu tìm kiếm được từ cơ sở dữ liệu sẽ được nhúng trực tiếp vào System Prompt để LLM dựa vào đó trả lời một cách chính xác.
...
### Điểm mạnh (Pros)
* **Quyền kiểm soát cao**: Định hướng hành vi và thiết lập Persona nhất quán xuyên suốt cuộc hội thoại.
* **Bảo mật tốt hơn**: Có độ ưu tiên cao hơn và khả năng chống Prompt Injection tốt hơn so với User Prompt thông thường.
* **Tách biệt vai trò**: Giúp nhà phát triển lập trình các quy tắc xử lý ngầm mà không làm ảnh hưởng đến trải nghiệm người dùng cuối.

### Điểm yếu (Cons)
* **Tiêu hao tài nguyên**: System Prompt được gửi kèm trong mỗi câu chat tiếp theo, làm tăng lượng token tiêu thụ và chi phí API.
* **Hiện tượng quên thông tin**: Nếu System Prompt quá dài, các chỉ thị ở giữa có thể bị mô hình bỏ sót (Lost in the Middle).
* **Nguy cơ Leak Prompt**: Dù có rào cản, System Prompt vẫn có thể bị rò rỉ nếu bị Jailbreak bằng các kỹ thuật tinh vi.

## Đánh đổi và giới hạn

* **Ngốn [Token](/concepts/6-ai-ml/genai-ml/token/) cơ sở:** System Prompt càng dài thì số lượng Token Input đầu vào càng lớn. Do cấu trúc hội thoại, System Prompt sẽ được gửi kèm trong **mọi câu chat tiếp theo** của phiên làm việc, dẫn đến chi phí sử dụng API tăng lên nhanh chóng (trừ khi bạn sử dụng các tính năng Cache Prompt mới hỗ trợ từ nhà cung cấp).
...
**Bắt buộc dùng khi:**
* Bạn xây dựng các ứng dụng AI cấp độ Product thực tế (Chatbot chăm sóc khách hàng, hệ thống RAG doanh nghiệp, [AI Agent](/concepts/6-ai-ml/genai-ml/ai-agent/) tự động hóa).
...
**Khái niệm liên quan:**
* [Large Language Model (LLM) - Mô hình ngôn ngữ lớn](/concepts/6-ai-ml/genai-ml/llm/)
* [Zero-shot Learning - Học không ví dụ](/concepts/6-ai-ml/genai-ml/zero-shot/)
* [Few-shot Prompting - Học qua vài ví dụ](/concepts/6-ai-ml/genai-ml/few-shot/)
* [Hallucination - Ảo giác LLM](/concepts/6-ai-ml/genai-ml/hallucination/)
...
1. [OpenAI Prompt Engineering Guide](https://platform.openai.com/docs/guides/prompt-engineering) - Tài liệu hướng dẫn thiết kế prompt và các message roles (bao gồm system role) của OpenAI.
2. [Anthropic Claude System Prompts Guide](https://docs.anthropic.com/en/docs/build-with-claude/prompt-engineering/system-prompts) - Tài liệu hướng dẫn thiết kế System Message tối ưu cho dòng mô hình Claude của Anthropic.
3. [System Message Design for Azure OpenAI](https://learn.microsoft.com/en-us/azure/ai-services/openai/concepts/system-message) - Hướng dẫn chi tiết từ Microsoft về việc thiết kế và lập cấu trúc system prompt trong môi trường doanh nghiệp.
4. [Ignore Previous Prompt: Attack Techniques for Language Models](https://arxiv.org/abs/2211.09527) - Bài báo nghiên cứu đầu tiên phân tích về kỹ thuật tấn công Prompt Injection và các lỗ hổng của System Prompt.
5. [LangChain Message Roles Documentation](https://python.langchain.com/docs/concepts/messages/) - Tài liệu giải thích về cách biểu diễn các đối tượng tin nhắn (SystemMessage, HumanMessage) trong framework LangChain.
6. [AWS Bedrock System Prompt Guidelines](https://docs.aws.amazon.com/bedrock/latest/userguide/model-parameters-anthropic-claude-system-prompt.html) - Hướng dẫn chi tiết sử dụng system prompt trên AWS Bedrock.
7. [Google Cloud Vertex AI System Instructions](https://cloud.google.com/vertex-ai/generative-ai/docs/multimodal/send-chat-prompts-gemini#system-instruction) - Hướng dẫn cấu hình System Instruction trên Google Cloud Vertex AI.
8. [Microsoft Azure OpenAI System Prompts](https://azure.microsoft.com/blog/system-prompts-and-safety-azure-openai/) - Các chính sách và thiết lập system prompt an toàn của Microsoft Azure.
