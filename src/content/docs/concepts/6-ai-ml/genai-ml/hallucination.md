...
definition: "Ảo giác LLM (Hallucination) là hiện tượng mô hình ngôn ngữ lớn tự tin tạo ra các thông tin sai lệch, không có thực hoặc mâu thuẫn logic nhưng được trình bày rất trôi chảy và mạch lạc."
seoTitle: "Ảo giác LLM là gì? Nguyên nhân và cách khắc phục ảo giác AI"
metaDescription: "Tìm hiểu hiện tượng Ảo giác (Hallucination) trong Mô hình Ngôn ngữ Lớn: nguyên nhân thống kê và các kỹ thuật ngăn ngừa như RAG và Chain-of-Thought."
---
...
* **Ngôn ngữ ít tài nguyên và từ khóa hiếm (Long-tail knowledge)**: Đối với các từ khóa ít xuất hiện hoặc các ngôn ngữ có ít tài liệu trên mạng (như tiếng Việt), mô hình không có đủ liên kết thống kê vững chắc. Khi đó, nó sẽ tự động "lắp ghép bừa" các [token](/concepts/6-ai-ml/genai-ml/token/) để tạo nên câu trả lời.
* **Khao khát làm hài lòng người dùng (Sycophancy)**: Quá trình huấn luyện tinh chỉnh ([RLHF](/concepts/6-ai-ml/genai-ml/rlhf/)) dạy cho mô hình bài học: *"Hãy luôn tỏ ra hữu ích và cố gắng trả lời người dùng"*. Điều này vô tình tạo ra áp lực khiến mô hình thà bịa ra một câu trả lời nghe có vẻ thuyết phục còn hơn là trung thực nói *"Tôi không biết"*.
* **Tham số sáng tạo ([Temperature](/concepts/6-ai-ml/genai-ml/temperature/)) quá cao**: Việc thiết lập tham số Nhiệt độ (Temperature) lớn sẽ bẻ cong phân phối xác suất dự đoán từ, buộc mô hình phải chọn các từ có xác suất thấp hơn để tăng tính phong phú, từ đó dễ dẫn đến sai lệch thông tin thực tế.
...
### Điểm mạnh (Pros)
* **Kích thích tính sáng tạo**: Giúp ích rất nhiều cho các tác vụ viết văn, làm thơ, hoặc brainstorm ý tưởng mới mà không bị giới hạn bởi các dữ liệu tĩnh.
* **Hành văn tự nhiên và phong phú**: Giúp câu trả lời của AI có tính biến hóa đa dạng, sinh động như con người.

### Điểm yếu (Cons)
* **Gây sai lệch thông tin thực tế**: Tự bịa đặt ra các sự kiện, liên kết hỏng hoặc trích dẫn giả mạo gây nguy hiểm cho các ứng dụng thực tế.
* **Giảm độ tin cậy của hệ thống**: Khiến doanh nghiệp e ngại việc tích hợp AI tự động hóa vào các tác vụ y tế, pháp lý, hoặc tài chính.

## Khi nào nên dùng và không nên dùng

* **Nên chấp nhận ảo giác (Nới lỏng)**:
  * Khi sử dụng AI làm trợ lý sáng tạo nghệ thuật, viết kịch bản, làm thơ hoặc lên ý tưởng marketing.
  * Các chatbot giải trí, nhập vai nhân vật lịch sử hoặc trò chuyện tán gẫu thông thường.
* **Không được chấp nhận ảo giác (Siết chặt)**:
  * Khi xây dựng các hệ thống y tế, chẩn đoán lâm sàng hoặc kiểm tra tương tác thuốc.
  * Công cụ tự động viết truy vấn cơ sở dữ liệu (Text-to-SQL) hay các công cụ viết code API tài chính.
  * Các [AI Agent](/concepts/6-ai-ml/genai-ml/ai-agent/) thực hiện các hành động trực tiếp ngoài đời thực.
...
* [LLM làm giám khảo (LLM-as-a-judge)](/concepts/6-ai-ml/genai-ml/llm-as-a-judge/)
* [Học không cần ví dụ (Zero-shot)](/concepts/6-ai-ml/genai-ml/zero-shot/)
...
1. [A Survey of Hallucination in Natural Language Generation](https://arxiv.org/abs/2202.03629) - Tài liệu học thuật định nghĩa phân loại Intrinsic và Extrinsic Hallucination.
2. [SelfCheckGPT: Zero-Resource Black-Box Hallucination Detection](https://arxiv.org/abs/2303.11479) - Kỹ thuật Self-Consistency để phát hiện ảo giác.
3. [AWS Bedrock Guardrails and Hallucination Prevention](https://docs.aws.amazon.com/bedrock/latest/userguide/guardrails.html) - Hướng dẫn chi tiết sử dụng Guardrails trên AWS Bedrock.
4. [Google Cloud Vertex AI Grounding to Reduce Hallucinations](https://cloud.google.com/vertex-ai/generative-ai/docs/multimodal/ground-gemini) - Hướng dẫn sử dụng kỹ thuật Grounding trên Google Cloud.
5. [Microsoft Azure OpenAI Prompt Engineering Guidelines](https://azure.microsoft.com/blog/prompt-engineering-techniques-azure-openai/) - Tài liệu về system prompt và an toàn hệ thống của Microsoft.
