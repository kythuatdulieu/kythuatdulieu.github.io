...
definition: "Tìm kiếm kết hợp (Hybrid Search) là kỹ thuật kết hợp đồng thời tìm kiếm từ khóa chính xác (BM25) và tìm kiếm ngữ nghĩa (Vector Search) để tối ưu hóa độ chính xác và độ bao phủ của kết quả truy vấn."
...
![Cloud logo](/images/hybrid-search/diagram_1.png)
...
1. **Tìm kiếm Dày (Dense / Vector Search)**: Đại diện bởi các cơ sở dữ liệu Vector và các mô hình [Embeddings](/concepts/6-ai-ml/genai-ml/embeddings/). Phương pháp này có khả năng thấu hiểu ngữ nghĩa và các từ đồng nghĩa (ví dụ: nó hiểu từ "chó" và "cún" có mối liên hệ mật thiết với nhau).
...
* Mỗi phân đoạn đi qua [Embedding Model](/concepts/6-ai-ml/genai-ml/embedding-model/) để tạo ra Dense Vector (mảng số thực, ví dụ 768 chiều).
...
* **Nên dùng:**
  * Khi xây dựng các ứng dụng RAG quy mô doanh nghiệp yêu cầu tìm kiếm chính xác các mã lỗi, SKU sản phẩm, từ viết tắt song song với hiểu câu hỏi ngữ nghĩa.
  * Khi kho dữ liệu có cấu trúc từ vựng đa dạng, chứa cả văn xuôi tự nhiên lẫn các thuật ngữ kỹ thuật chuyên biệt.
  * Khi muốn tăng độ ổn định của hệ thống tìm kiếm, giảm bớt sự phụ thuộc hoàn toàn vào một mô hình Embedding duy nhất.
* **Không nên dùng:**
  * Khi dữ liệu của bạn quá nhỏ (dưới vài ngàn tài liệu) và cấu trúc đơn giản, việc triển khai song song hai hệ thống chỉ mục sẽ gây lãng phí bộ nhớ vô ích.
  * Khi ứng dụng đòi hỏi độ trễ cực thấp (dưới vài mili-giây) và không thể chấp nhận việc chạy song song hai luồng tìm kiếm cũng như bước hòa trộn kết quả.

## Điểm mạnh (Pros)

### Điểm mạnh (Pros)
* **Độ bao phủ dữ liệu tối đa (High [Recall](/concepts/6-ai-ml/genai-ml/recall/)):** Giải quyết tốt cả bài toán hiểu ngữ nghĩa mơ hồ lẫn bài toán tìm kiếm mã số chính xác tuyệt đối.
...
* [AWS - OpenSearch Service Hybrid Search and RRF](https://aws.amazon.com/blogs/database/introducing-amazon-opensearch-service-hybrid-search-with-reciprocal-rank-fusion-rrf/)
* [Google Cloud - Vertex AI Vector Search in Spanner](https://cloud.google.com/blog/products/databases/vertex-ai-vector-search-in-spanner)
* [Databricks - Introducing Hybrid Search in Databricks](https://www.databricks.com/blog/introducing-hybrid-search-databricks)
* [Confluent - Understanding Vector Embeddings and Vector Databases](https://www.confluent.io/blog/understanding-vector-embeddings-and-vector-databases/)
* [RRF Paper - Reciprocal Rank Fusion outperforms Condorcet and individual Rank Learning Methods](https://plg.uwaterloo.ca/~gvcormac/cormack-sigir09-rrf.pdf) - Cormack et al. (2009).
* [DPR Paper - Dense Passage Retrieval for Open-Domain Question Answering](https://arxiv.org/abs/2004.04906) - Karpukhin et al. (2020).
