...
definition: "Tác nhân AI (AI Agent) là một hệ thống tự trị sử dụng Mô hình Ngôn ngữ Lớn (LLM) làm bộ não trung tâm, kết hợp với khả năng lập kế hoạch, ghi nhớ và sử dụng công cụ để tự động thực hiện các nhiệm vụ phức tạp."
seoTitle: "AI Agent (Tác nhân AI) là gì? Kiến trúc và ứng dụng thực tế"
metaDescription: "Tìm hiểu kiến trúc AI Agent: cách kết hợp bộ não LLM với khả năng lập kế hoạch (Planning), bộ nhớ (Memory) và sử dụng công cụ (Tool Use/Action) hiệu quả."
---
...
3. **Thất bại khi xử lý tác vụ dài hạn:** Khi đối mặt với quy trình làm việc gồm 15 - 20 bước, LLM rất dễ rơi vào tình trạng "ảo giác" ([hallucination](/concepts/6-ai-ml/genai-ml/hallucination/)), quên mất bước trung gian hoặc bị kẹt trong các vòng lặp vô tận.
...
### 2. Memory (Bộ nhớ)
* **Bộ nhớ ngắn hạn (Short-term Memory):** Chính là cửa sổ ngữ cảnh ([Context Window](/concepts/6-ai-ml/genai-ml/context-window/)) lưu lại lịch sử cuộc hội thoại đang diễn ra.
* **Bộ nhớ dài hạn (Long-term Memory):** Khả năng lưu trữ các thông tin quan trọng qua nhiều ngày, nhiều tháng bằng cách kết nối với các cơ sở dữ liệu vector ([Vector Database](/concepts/6-ai-ml/genai-ml/vector-database/)) để thực hiện RAG (Retrieval-Augmented Generation).
...
* **Phối hợp đa tác nhân (Multi-Agent):** Cho phép tạo ra một đội ngũ AI phối hợp với nhau để hoàn thành mục tiêu lớn.

### Điểm yếu (Cons)
* **Chi phí và độ trễ tăng vọt:** Một Agent sử dụng vòng lặp ReAct có thể tốn từ 5 đến 10 lượt gọi API liên tiếp, đẩy chi phí vận hành lên cao và làm tăng độ trễ phản hồi của hệ thống.
* **Không thể đoán trước kết quả (Non-deterministic):** Cùng một câu prompt đầu vào, Agent có thể đưa ra hai kế hoạch giải quyết hoàn toàn khác nhau ở mỗi lần chạy.
* **Mối đe dọa bảo mật (Prompt Injection):** Agent có thể bị tấn công chèn lệnh ẩn từ các trang web bên ngoài mà nó truy cập để lấy dữ liệu.
...
* **Không nên dùng khi:**
  ...
  * Quy trình công việc của bạn là tuyến tính và cố định (Static Workflow). Nếu quy trình luôn diễn ra cố định, hãy dùng code logic hoặc các công cụ tự động hóa tĩnh (Zapier, Airflow) thay vì để Agent tự suy luận lòng vòng.
...
1. [ReAct: Synergizing Reasoning and Acting in Language Models](https://arxiv.org/abs/2210.03629) - Princeton and Google research paper establishing the ReAct agent framework.
2. [LLM Powered Autonomous Agents](https://lilianweng.github.io/posts/2023-06-23-agent/) - Bài viết chuyên sâu của Lilian Weng về kiến trúc Agent tự trị.
3. [AWS Bedrock Agents Guide](https://docs.aws.amazon.com/bedrock/latest/userguide/agents.html) - Hướng dẫn chi tiết xây dựng tác nhân AI trên nền tảng AWS Bedrock.
4. [Google Cloud Vertex AI Agent Builder](https://cloud.google.com/vertex-ai/generative-ai/docs/agent-builder/overview) - Tài liệu hướng dẫn sử dụng công cụ tạo Agent của Google Cloud.
5. [Microsoft Azure AI Agent Services](https://azure.microsoft.com/blog/system-prompts-and-safety-azure-openai/) - Tài liệu về thiết lập và triển khai Agents trên môi trường Azure OpenAI.
