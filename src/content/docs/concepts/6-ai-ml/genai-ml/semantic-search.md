...
definition: "Tìm kiếm ngữ nghĩa (Semantic Search) là phương pháp tìm kiếm thông tin bằng cách thấu hiểu ngữ cảnh và ý định của người dùng, thay vì so khớp từ khóa chính xác."
seoTitle: "Tìm kiếm ngữ nghĩa (Semantic Search) là gì? Khái niệm & Ứng dụng"
...
* **Số hóa ý nghĩa**: Thông qua các mô hình học máy ([Embedding Models](/concepts/6-ai-ml/genai-ml/embedding-models/)), văn bản, hình ảnh hay câu hỏi đều được chuyển đổi thành một chuỗi số thực gọi là Vectơ. Vectơ này biểu diễn vị trí của từ/câu đó trong một không gian đa chiều (thường từ vài trăm đến hàng ngàn chiều).
...
* **Nên dùng:**
  * Khi xây dựng hệ thống hỏi đáp (Q&A), cổng tra cứu tài liệu nội bộ tích hợp [RAG](/concepts/6-ai-ml/genai-ml/rag/).
  * Khi người dùng có xu hướng gõ các truy vấn mô tả ngữ nghĩa, từ đồng nghĩa hoặc câu hỏi tự nhiên.
  * Khi cần tìm kiếm xuyên ngôn ngữ hoặc đa phương thức (ví dụ tìm ảnh bằng chữ).
* **Không nên dùng:**
  * Khi người dùng chủ yếu tìm kiếm chính xác các mã số đặc thù như số seri, mã lỗi, tên biến lập trình hoặc SKU sản phẩm.
  * Khi ngân sách hạ tầng hạn chế và không thể triển khai GPU phục vụ bước suy luận sinh vector nhúng.

## Điểm mạnh (Pros)

### Điểm mạnh (Pros)
* **Thấu hiểu ngữ nghĩa sâu sắc:** Bỏ qua sự khác biệt về mặt chữ để kết nối các khái niệm đồng nghĩa hoặc diễn đạt tương đương.
* **Hỗ trợ xử lý đa ngôn ngữ:** Các mô hình nhúng đa ngôn ngữ giúp tìm kiếm chéo giữa các ngôn ngữ tự nhiên cực kỳ dễ dàng.
* **Khả năng chống lỗi chính tả:** Hệ thống vẫn nhận diện đúng ý định người dùng ngay cả khi họ viết sai chính tả nhẹ nhờ cơ chế biểu diễn ngữ nghĩa.

### Điểm yếu (Cons)
* **Chi phí tính toán cao:** Việc biến đổi câu hỏi thành vector nhúng tại thời gian thực đòi hỏi tài nguyên CPU/GPU và gây độ trễ nhất định.
* **Kém hiệu quả với từ khóa chính xác:** Các từ viết tắt chuyên biệt hoặc mã sản phẩm đặc thù dễ bị làm mờ ý nghĩa trong không gian vector.
* **Độ trễ lưu trữ:** Quá trình ghi nhận và xây dựng chỉ mục vector diễn ra chậm hơn nhiều so với hệ thống cơ sở dữ liệu văn bản truyền thống.
...
### 3. Thuật toán RRF (Reciprocal Rank Fusion) hoạt động như thế nào trong Hybrid Search?
* **Gợi ý trả lời**: RRF là một thuật toán dùng để gộp các bảng xếp hạng kết quả từ nhiều công cụ tìm kiếm khác nhau (như Keyword Search và Semantic Search) thành một danh sách kết quả duy nhất. Điểm số RRF của một tài liệu $d$ được tính bằng tổng nghịch đảo vị trí xếp hạng của nó trong các danh sách:
  $$RRF\_Score(d) = \sum_{m \in M} \frac{1}{k + r_m(d)}$$
  Trong đó $M$ là tập hợp các phương thức tìm kiếm, $r_m(d)$ là thứ hạng của tài liệu $d$ trong phương thức $m$, và $k$ là một hằng số (thường chọn bằng 60). Tài liệu nào xuất hiện ở thứ hạng cao trong cả hai danh sách sẽ có điểm RRF lớn nhất và được xếp đầu ở kết quả cuối cùng.
...
* [AWS - What is Semantic Search?](https://aws.amazon.com/what-is/embeddings-in-machine-learning/)
* [Google Cloud - Text Embeddings and Semantic Search](https://cloud.google.com/vertex-ai/docs/generative-ai/embeddings/get-text-embeddings)
* [Databricks - Semantic Search with Vector Embeddings](https://www.databricks.com/glossary/vector-embeddings)
* [Confluent - Understanding Vector Embeddings and Databases](https://www.confluent.io/blog/understanding-vector-embeddings-and-vector-databases/)
* [Elasticsearch - Official Semantic Search Guide](https://www.elastic.co/guide/en/elasticsearch/reference/current/semantic-search.html)
* [Dense Passage Retrieval for Open-Domain Question Answering](https://arxiv.org/abs/2004.04906) - Vladimir Karpukhin et al. (EMNLP 2020).
