...
definition: "Normalized Discounted Cumulative Gain (NDCG) là chỉ số đo lường hiệu quả xếp hạng của hệ thống tìm kiếm và gợi ý, tính đến mức độ liên quan đa cấp độ và vị trí hiển thị của kết quả."
---

Khi xây dựng một công cụ tìm kiếm (Search Engine), hệ thống gợi ý (Recommender System) hay bước tái xếp hạng ([Reranking](/concepts/6-ai-ml/genai-ml/reranking/)) trong kiến trúc [RAG](/concepts/6-ai-ml/genai-ml/rag/), các chỉ số đo lường truyền thống như Precision (Độ chính xác) hay [Recall](/concepts/6-ai-ml/genai-ml/recall/) (Độ bao phủ) thường không đủ để đánh giá chất lượng trải nghiệm của người dùng. 
...
1. [Amazon Personalize Metrics - NDCG](https://docs.aws.amazon.com/personalize/latest/dg/working-with-prebuilt-recipes-recommendation-metrics.html) - Tài liệu Amazon Personalize giải thích về việc tính toán NDCG để đánh giá hệ thống gợi ý.
2. [Google Cloud Vertex AI Search - Evaluate Quality](https://cloud.google.com/generative-ai-app-builder/docs/evaluate-search-quality) - Hướng dẫn đánh giá chất lượng tìm kiếm bằng NDCG trên Google Cloud.
3. [Azure Machine Learning Model Evaluation](https://azure.microsoft.com/en-us/blog/evaluating-machine-learning-models/) - Tài liệu từ Microsoft Azure Blog bàn về các phương pháp và chỉ số đo lường mô hình.
4. [Apache Spark MLlib - Ranking Systems Evaluation Metrics](https://spark.apache.org/docs/latest/mllib-evaluation-metrics.html) - Tài liệu chính thức của Apache Spark về cách đo lường xếp hạng với NDCG và MAP.
5. [Databricks MLflow Evaluation for Retrieval & Ranking](https://docs.databricks.com/en/machine-learning/model-evaluation/index.html) - Tài liệu Databricks tích hợp đánh giá xếp hạng và NDCG.
6. [Discounted Cumulative Gain - Wikipedia](https://en.wikipedia.org/wiki/Discounted_cumulative_gain) - Lý thuyết toán học và nguồn gốc của chỉ số DCG/NDCG trên Wikipedia.
