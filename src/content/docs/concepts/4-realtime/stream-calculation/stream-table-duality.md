...
definition: "Stream-Table Duality là tính lưỡng tính dòng - bảng, biểu thị mối quan hệ mật thiết giữa luồng dữ liệu biến động (Stream) và bảng dữ liệu trạng thái tĩnh (Table) trong xử lý dữ liệu thời gian thực."
---
...
![Sơ đồ minh họa kiến trúc Stream Table Duality](/images/stream-table-duality/diagram_1.svg)
...
![Quy trình xử lý và luồng dữ liệu Stream Table Duality](/images/stream-table-duality/diagram_2.svg)
...
* **Chèn dữ liệu thô từ Stream vào Database mà không dùng UPSERT:** Khi bạn tiêu thụ một luồng CDC để ghi đè vào [Data Warehouse](/concepts/2-storage/data-warehouse/data-warehouse/), nếu chỉ dùng lệnh `INSERT` thông thường, bạn sẽ vô tình lưu trữ toàn bộ lịch sử thay đổi của dòng đó. Hãy dùng lệnh `MERGE` hoặc `UPSERT` để ép luồng dữ liệu trở về đúng dạng trạng thái tĩnh của Bảng.
...
* **Nhược điểm:** Xóa bỏ hoàn toàn các trạng thái trung gian. Bạn sẽ không thể biết người dùng đã đổi tên bao nhiêu lần trước khi có cái tên hiện tại, trừ khi bạn chủ động thiết kế thêm các bảng lịch sử phức tạp (như [Slowly Changing Dimension](/concepts/2-storage/data-warehouse/slowly-changing-dimension/)).
...
* [Windowing - Phân mảnh thời gian](/concepts/4-realtime/streaming-processing/windowing/)
* [Change Data Capture (CDC) - Trích xuất thay đổi dữ liệu](/concepts/3-integration/etl-elt/change-data-capture/)
* [Apache Kafka - Nền tảng luồng sự kiện](/concepts/4-realtime/streaming-processing/apache-kafka/)
...
1. [Confluent Guide: Streams and Tables in Apache Kafka](https://www.confluent.io/blog/kafka-streams-tables-primer/) - Confluent Docs
2. [Apache Flink Table and SQL Concepts](https://nightlies.apache.org/flink/flink-docs-stable/docs/dev/table/concepts/) - Apache Flink Documentation
3. [AWS MSK Stream Processing and CDC](https://docs.aws.amazon.com/msk/latest/developerguide/msk-connect.html) - Amazon Web Services
4. [Azure Stream Analytics Stream-Table Joins](https://learn.microsoft.com/en-us/azure/stream-analytics/stream-analytics-stream-join-with-reference-data) - Microsoft Learn
5. [Google Cloud Dataflow Streaming Concepts](https://cloud.google.com/dataflow/docs/concepts/streaming-pipelines) - Google Cloud Docs
