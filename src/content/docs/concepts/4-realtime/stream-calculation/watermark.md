...
definition: "Watermark (Dấu chuẩn thời gian) là cơ chế theo dõi tiến trình thời gian logic trong xử lý dữ liệu luồng, giúp hệ thống xác định khi nào nên đóng các cửa sổ thời gian và xử lý dữ liệu đến muộn."
---

Trong lập trình xử lý luồng dữ liệu (Stream Processing), một trong những thử thách lớn nhất là kiểm soát yếu tố thời gian. Không giống như xử lý theo lô ([Batch Processing](/concepts/3-integration/batch-processing/batch-processing/)) nơi mọi dữ liệu đã nằm yên vị trong kho, dữ liệu Streaming chảy liên tục và thường xuyên đến lệch giờ, mất trật tự do độ trễ mạng hoặc lỗi kết nối từ các thiết bị ngoại vi. Để giải quyết bài toán này khi sử dụng thời gian xảy ra sự kiện (Event Time), các kỹ sư dữ liệu sử dụng một công cụ vô cùng mạnh mẽ: **Watermark** (Dấu chuẩn thời gian).
...
* [Event Time & Processing Time](/concepts/4-realtime/streaming-processing/event-time-processing-time/)
* [Windowing](/concepts/4-realtime/streaming-processing/windowing/)
...
1. [Apache Flink Documentation - Generating Watermarks](https://nightlies.apache.org/flink/flink-docs-stable/docs/dev/datastream/event-time/generating_watermarks/) - Apache Flink Documentation
2. [Google Cloud Dataflow: Stream Processing and Watermarks](https://cloud.google.com/dataflow/docs/concepts/streaming-pipelines) - Google Cloud Docs
3. [AWS MSK Best Practices for Watermarks](https://docs.aws.amazon.com/msk/latest/developerguide/best-practices.html) - Amazon Web Services
4. [Azure Stream Analytics Time Management](https://learn.microsoft.com/en-us/azure/stream-analytics/stream-analytics-time-handling) - Microsoft Learn
5. [Confluent Guide: Handling Late and Out-of-Order Data](https://www.confluent.io/blog/handling-late-and-out-of-order-data-in-apache-kafka/) - Confluent Docs
