...
definition: "Topic trong Apache Kafka là danh mục logic dùng để phân loại dữ liệu, còn Partition là đơn vị phân chia vật lý giúp song song hóa xử lý và mở rộng quy mô lưu trữ trên cụm Broker."
---
...
![Sơ đồ minh họa kiến trúc Kafka Topics Partitions](/images/kafka-topics-partitions/diagram_1.png)
...
![Quy trình xử lý và luồng dữ liệu Kafka Topics Partitions](/images/kafka-topics-partitions/diagram_2.png)
...
* **Nên dùng phân vùng (Partitions) nhiều khi**:
  * Cần nâng cao thông lượng (throughput) đọc và ghi dữ liệu song song trên cụm máy chủ.
  * Cần đảm bảo khả năng chịu lỗi (fault tolerance) bằng cách sao chép (replicate) phân vùng sang nhiều Broker.
* **Nên thiết lập Khóa tin nhắn (Message Key) khi**:
  * Cần đảm bảo thứ tự xử lý tuần tự (ordering guarantee) cho các sự kiện thuộc cùng một đối tượng (ví dụ: các bước giao dịch của cùng một tài khoản).
...
* [Consumer Groups (Nhóm tiêu thụ dữ liệu)](/concepts/4-realtime/streaming-processing/consumer-groups/)
* [Data Skew (Lệch dữ liệu)](/concepts/3-integration/batch-processing/data-skew/)
...
1. [Apache Kafka Topics and Partitions Documentation](https://kafka.apache.org/documentation/#intro_concepts_and_terms) - Apache Software Foundation
2. [Confluent Guide: How to Choose the Number of Topics and Partitions](https://www.confluent.io/blog/how-choose-number-topics-partitions-apache-kafka-cluster/) - Confluent Docs
3. [AWS MSK Best Practices for Partition Scaling](https://docs.aws.amazon.com/msk/latest/developerguide/best-practices.html) - Amazon Web Services
4. [Azure Event Hubs Partitioning Guide](https://learn.microsoft.com/en-us/azure/event-hubs/event-hubs-features#partitions) - Microsoft Learn
5. [Google Cloud Pub/Sub and Kafka Partitions Comparison](https://cloud.google.com/pubsub/docs/comparing-pubsub-kafka) - Google Cloud Docs
6. [Kafka: The Definitive Guide](https://www.oreilly.com/library/view/kafka-the-definitive/9781492044048/) - Neha Narkhede, Gwen Shapira, and Todd Palino
