...
Quy trình hoạt động cơ bản khi có sự tham gia của Metrics Layer (như [dbt](/concepts/transformation-analytics/dbt/) Semantic Layer hoặc Cube.js):
...
4. **Thực thi**: Câu lệnh SQL được chạy trực tiếp trên Data Warehouse (BigQuery, [Snowflake](/concepts/cloud-data-platform/snowflake/)), và kết quả chuẩn xác được trả về cho Tableau.
...
* **Phục vụ đa kênh**: Không chỉ công cụ BI, mà cả các Jupyter Notebook của Data Scientist, ứng dụng nội bộ hay thậm chí là các [AI Agent](/concepts/genai-ml/ai-agent/) đều có thể tái sử dụng chung một định nghĩa chỉ số.
...
* **Đừng nhầm lẫn với [ETL](/concepts/etl-elt/etl/)**: Metrics Layer chỉ nên tập trung vào nhiệm vụ tính toán tổng hợp (aggregations) và lọc dữ liệu. Việc làm sạch dữ liệu và nối bảng phức tạp (data cleansing, heavy joins) nên được xử lý triệt để ở tầng Transformation (như dbt SQL models) trước khi đưa dữ liệu vào Metrics Layer.
...
2. **Fundamentals of [Data Engineering](/concepts/foundation/data-engineering/)** - Joe Reis & Matt Housley.
