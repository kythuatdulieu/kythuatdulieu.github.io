...
2. **Khả năng mở rộng không giới hạn (Extensible):** Nhờ cấu trúc module mở rộng (Providers), Airflow dễ dàng kết nối và tương tác với hầu hết các dịch vụ đám mây và công cụ dữ liệu hiện đại như AWS, GCP, [Snowflake](/concepts/cloud-data-platform/snowflake/), Slack, [dbt](/concepts/transformation-analytics/dbt/),...
...
> [!NOTE]  
> Airflow **không phải** là công cụ trực tiếp xử lý hay tính toán dữ liệu (như [Apache Spark](/concepts/batch-processing/apache-spark/) hay Pandas). Nhiệm vụ cốt lõi của nó chỉ là **điều phối** – ra lệnh cho các hệ thống khác làm việc và giám sát tiến trình.
...
* **Tuyệt đối không lưu mật khẩu trong code:** Hãy sử dụng tính năng `Connections` trên giao diện UI của Airflow hoặc tích hợp với các dịch vụ quản lý bí mật (như HashiCorp Vault, AWS Secrets Manager) để lưu trữ API [token](/concepts/genai-ml/token/) và thông tin đăng nhập database.
...
* Hệ thống của bạn chủ yếu xử lý dữ liệu theo dạng lô ([Batch processing](/concepts/batch-processing/batch-processing/)) với chu kỳ cố định (hàng giờ, hàng ngày, hàng tuần).
* Bạn đang xây dựng một kiến trúc Data Platform tập trung, đóng vai trò điều phối chính cho các công cụ khác hoạt động.

## Các khái niệm liên quan

* [Orchestration](/concepts/orchestration/orchestration/)
* [Directed Acyclic Graph (DAG)](/concepts/orchestration/dag/)
* [Task Dependency](/concepts/orchestration/task-dependency/)
* [Airflow Scheduler](/concepts/orchestration/airflow-scheduler/)
