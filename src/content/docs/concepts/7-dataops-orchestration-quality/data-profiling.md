...
Có bao giờ bạn nhảy vào viết code [ETL](/concepts/etl-elt/etl/) ngay khi vừa nhận được một file CSV hay thông tin kết nối CSDL từ đối tác, để rồi pipeline bị lỗi (crash) giữa chừng vì dữ liệu có chứa giá trị Null, định dạng ngày tháng không nhất quán, hoặc kiểu dữ liệu thay đổi đột ngột? Để tránh những sự cố đó, **Data Profiling (Lập hồ sơ dữ liệu)** ra đời như một bước "khám sức khỏe tổng quát" cho dữ liệu trước khi đưa vào xử lý.
