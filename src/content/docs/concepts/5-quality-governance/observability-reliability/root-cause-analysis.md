...
definition: "Phân tích nguyên nhân gốc rễ (Root Cause Analysis - RCA) là quy trình giải quyết sự cố có hệ thống nhằm xác định nguyên nhân sâu xa nhất của lỗi dữ liệu và thiết lập giải pháp phòng ngừa lâu dài."
---

Trong quá trình vận hành các hệ thống dữ liệu, sự cố là điều không thể tránh khỏi. Một ngày đẹp trời, [data pipeline](/concepts/1-foundations/foundation/data-pipeline/) bỗng dưng bị sập, dữ liệu hiển thị trên dashboard bị sai lệch hoặc mô hình máy học đưa ra các dự đoán bất thường. Khi đó, phản xạ tự nhiên của chúng ta là nhanh chóng "chữa cháy" để hệ thống hoạt động trở lại. Tuy nhiên, nếu chỉ dừng lại ở đó, lỗi sẽ sớm lặp lại vào ngày mai. Để giải quyết triệt để vấn đề, chúng ta cần đến một quy trình phân tích bài bản mang tên **Root Cause Analysis (RCA) - Phân tích nguyên nhân gốc rễ**.

![Biểu đồ minh họa Root Cause Analysis](/images/root-cause-analysis/diagram_1.svg)

![Screenshot of alert history tab with suppression from alert processing rule.](/images/root-cause-analysis/diagram_2.png)
...
* **Các bản vá tạm bợ (Band-aid fixes)**: Khi thấy dữ liệu bị thiếu hụt, kỹ sư thường viết vội các câu lệnh kiểm tra kiểu `IF NULL THEN 0` ở khắp mọi nơi để che mắt người dùng. Lâu dần, kho dữ liệu ([Data Warehouse](/concepts/2-storage/data-warehouse/data-warehouse/)) sẽ biến thành một đống hỗn độn, chạy chậm chạp và tích tụ lượng nợ kỹ thuật (Technical Debt) khổng lồ.
...
4. **Đề xuất hành động ngăn ngừa (Action Items)**: Tạo các task cụ thể trên Jira để bổ sung các bài kiểm thử dữ liệu ([Data Quality](/concepts/5-quality-governance/data-quality/data-quality/) Tests), tối ưu hóa cấu hình cảnh báo, hoặc điều chỉnh quy trình phối hợp giữa các team.
...
* **Tại sao (2) cột `is_active` lại bị ghi nhận là `FALSE`?**
  - Vì mô hình [dbt](/concepts/3-integration/transformation-analytics/dbt/) thực hiện phép nối `LEFT JOIN` với bảng thông tin người dùng `crm_users`, nhưng kết quả join bị trả về `NULL`. Logic code quy định nếu kết quả join bị `NULL` thì mặc định trạng thái hoạt động là `FALSE`.
...
* **Tại sao (4) định dạng của `user_id` bị thay đổi ở hệ thống nguồn mà team Data không hề hay biết?**
  - Vì team Backend vừa triển khai một bản cập nhật hệ thống để đổi thư viện tạo ID nhưng quên không thông báo cho team Data về sự thay đổi cấu trúc này (hiện tượng [Schema Drift](/concepts/5-quality-governance/observability-reliability/schema-drift/)).
...
2. Làm việc với team Backend để thống nhất áp dụng **[Data Contract](/concepts/3-integration/transformation-analytics/data-contract/) (Hợp đồng dữ liệu)** – ràng buộc không tự ý thay đổi cấu trúc bảng nguồn khi chưa có sự đồng ý của các team tiêu thụ dữ liệu.
...
### Ưu điểm:
* Giúp giải quyết dứt điểm các lỗi hệ thống mãn tính, nâng cao chất lượng vận hành kỹ thuật của toàn đội ngũ và giảm thiểu lượng nợ kỹ thuật tích tụ theo thời gian.

### Nhược điểm:
* Đòi hỏi đầu tư thời gian và công sức đáng kể. Một buổi họp phân tích RCA chi tiết có thể tiêu tốn hàng giờ làm việc của những kỹ sư nhiều kinh nghiệm nhất trong team. Đồng thời, nó yêu cầu một môi trường văn hóa công ty cởi mở, minh bạch và tin tưởng lẫn nhau.

## Khi nào nên dùng

### Khi nào nên dùng:
* Sau khi xảy ra các sự cố dữ liệu nghiêm trọng ảnh hưởng trực tiếp đến người dùng cuối, đối tác hoặc doanh thu của doanh nghiệp (sự cố mức SEV-1, SEV-2).
* Khi gặp phải các lỗi mang tính lặp đi lặp lại trong hệ thống data pipeline mà không rõ nguyên nhân sâu xa.
* Khi chuẩn bị thực hiện việc tái cấu trúc (refactoring) các luồng dữ liệu cốt lõi để nâng cao hiệu năng hệ thống.

### Khi nào không nên dùng:
* Đối với các lỗi vặt, lỗi cú pháp hiển nhiên xảy ra trong môi trường phát triển (Development/Staging) và không gây hậu quả gì lớn.
* Khi toàn đội đang ở trong giai đoạn "chữa cháy" khẩn cấp. RCA chỉ nên được thực hiện sau khi sự cố đã được giảm thiểu (mitigate) và hệ thống đã quay lại trạng thái bình thường.

## Các khái niệm liên quan

* [Cảnh báo & Phản ứng sự cố - Alerting & Incident Response](/concepts/5-quality-governance/observability-reliability/alerting-incident-response/)
* [Data Observability](/concepts/5-quality-governance/observability-reliability/data-observability/)
* [Truy vết dữ liệu - Data Lineage](/concepts/5-quality-governance/governance-metadata/data-lineage/)
...
* [Google Cloud Architecture Framework - Reliability Section](https://cloud.google.com/architecture/framework/reliability)
* [Azure Monitor Alerts and Incidents Troubleshooting](https://learn.microsoft.com/en-us/azure/azure-monitor/alerts/alerts-troubleshoot)
* [Databricks Lakehouse Monitoring and Quality Debugging](https://docs.databricks.com/en/lakehouse-monitoring/index.html)
* [Snowflake Data Quality Metric and Alerts Reference](https://docs.snowflake.com/en/user-guide/data-quality-intro)

## English Summary

Root Cause Analysis (RCA) is a systematic problem-solving process used in [Data Engineering](/concepts/1-foundations/foundation/data-engineering/) to uncover the fundamental, underlying reasons for data incidents (e.g., pipeline failures, data anomalies), rather than merely addressing surface symptoms. Employing methodologies like the "5 Whys" and leveraging technical tools like Data Lineage, RCA aims to identify systemic or process flaws (like missing data contracts or absent CI/CD tests) that led to the failure. A critical component of Site Reliability Engineering (SRE), RCA must be conducted in a "blameless" culture to encourage honest investigation, resulting in concrete action items that "fireproof" the architecture and prevent the same issue from ever recurring.