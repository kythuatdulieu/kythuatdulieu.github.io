...
definition: "Phát hiện bất thường về khối lượng dữ liệu (Volume Anomalies) là quy trình giám sát số lượng bản ghi hoặc dung lượng dữ liệu, phát hiện các sụt giảm hoặc tăng vọt bất thường."
---
...
![Data profiling overview](/images/volume-anomalies/diagram_1.png)

![Data profiling diagram](/images/volume-anomalies/diagram_2.png)
...
1. **Mất mát dữ liệu do thay đổi kỹ thuật (Data Loss)**: Đội ngũ Backend vô tình phát hành một bản cập nhật ứng dụng làm mất đi 50% sự kiện tracking click của người dùng. Hệ thống [Data Pipeline](/concepts/1-foundations/foundation/data-pipeline/) vẫn chạy thành công, dữ liệu vẫn mới (Freshness bình thường), cấu trúc không đổi (Schema ổn), nhưng khối lượng giảm sút nghiêm trọng.
2. **Trùng lặp dữ liệu (Data Duplication)**: Do thiết lập cấu hình chạy Airflow [backfill](/concepts/3-integration/etl-elt/backfill/) sai, một khoảng thời gian dữ liệu bị tải lại 2 lần (sử dụng append thay vì merge/upsert), khiến khối lượng tăng gấp đôi.
3. **Sự cố API / Giới hạn Rate Limit**: Khi kéo dữ liệu từ API bên thứ 3 (như Facebook Ads, Salesforce), việc chạm ngưỡng rate limit hoặc [token](/concepts/6-ai-ml/genai-ml/token/) bị hết hạn giữa chừng khiến pipeline chỉ lấy được một phần dữ liệu rồi tự ngắt (Partial Load) nhưng vẫn báo hoàn thành.
...
1. **Thu thập Metrics (Telemetry Collection)**: Mỗi khi một Data Job (Airflow, [dbt](/concepts/3-integration/transformation-analytics/dbt/), Fivetran) kết thúc, hệ thống sẽ log lại thông tin `rows_inserted`, `rows_updated`, `rows_deleted` hoặc truy vấn Metadata DWH để lấy tổng số dòng hiện tại của bảng (`COUNT(*)`).
...
### Khi nào nên dùng:
* Cho toàn bộ các bảng dữ liệu cốt lõi (Core Tables, Fact Tables) lưu trữ giao dịch, sự kiện hệ thống (Logs, Events, Telemetry) có tần suất sinh dữ liệu liên tục và tương đối ổn định.
* Khi luồng dữ liệu của bạn có nguy cơ gặp sự cố Cartesian product (lỗi join làm bùng nổ số lượng bản ghi) hoặc lỗi tải thiếu dữ liệu (partial load).

### Khi nào không nên dùng:
* Với các bảng danh mục nhỏ, ít biến động (Reference/Dimension Tables) như bảng mã quốc gia (`Country_Codes`) hay loại tiền tệ (`Currency_Types`) vì dữ liệu của chúng hầu như không thay đổi hàng ngày.
...
* [Giám sát độ trễ - Freshness Monitoring](/concepts/5-quality-governance/observability-reliability/freshness-monitoring/)
...
* [Google BigQuery Information Schema Tables Reference](https://cloud.google.com/bigquery/docs/information-schema-tables)
* [AWS Glue Data Quality Volume Checks Guide](https://docs.aws.amazon.com/glue/latest/dg/data-quality-rules.html)
* [Azure Monitor Logs Supported Metrics](https://learn.microsoft.com/en-us/azure/azure-monitor/essentials/metrics-supported)
* [Snowflake Table Storage Metrics View](https://docs.snowflake.com/en/sql-reference/info-schema/table-storage-metrics)
* [Databricks Lakehouse Monitoring and Tracking Volume](https://docs.databricks.com/en/lakehouse-monitoring/index.html)
* [Meta Prophet Forecasting Core Documentation](https://facebook.github.io/prophet/)
