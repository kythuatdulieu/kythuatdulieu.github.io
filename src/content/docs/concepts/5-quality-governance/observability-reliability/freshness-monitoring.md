...
definition: "Giám sát độ trễ dữ liệu (Freshness Monitoring) là quy trình theo dõi mức độ tươi mới của dữ liệu so với cam kết chất lượng dịch vụ (Data SLA), đảm bảo phát hiện kịp thời các đường ống dữ liệu bị chậm."
---
...
1. **Lỗi im lặng (Silent failure)**: Các công cụ lập lịch hoặc [ETL](/concepts/3-integration/etl-elt/etl/) Job báo cáo chạy thành công, nhưng do API nguồn gặp sự cố hoặc phân vùng dữ liệu trống rỗng, quá trình trích xuất chỉ lấy về được 0 dòng. Về mặt kỹ thuật không có lỗi crash nào xảy ra, nhưng thực tế dữ liệu trong kho vẫn là dữ liệu cũ kỹ từ nhiều ngày trước.
...
Nền tảng giám sát sẽ tự động thu thập lịch sử cập nhật của các bảng trong quá khứ để tự tìm ra quy luật (Seasonality).
...
* **Giám sát ngay từ dữ liệu nguồn (Source Ingestion)**: Đừng chỉ đợi dữ liệu qua tay Spark, [dbt](/concepts/3-integration/transformation-analytics/dbt/) rồi mới đo Freshness. Hãy giám sát ngay từ lớp dữ liệu thô (Staging/Raw) để phát hiện sớm các lỗi mất kết nối API từ đối tác bên thứ ba.
...
### Khi nào nên dùng:
* Khi hệ thống dữ liệu có các cam kết chặt chẽ về thời gian cung cấp báo cáo (Data SLA) cho các phòng ban kinh doanh hoặc khách hàng.
* Khi luồng dữ liệu nhận dữ liệu từ các bên thứ ba qua API hoặc các hệ thống bên ngoài không thuộc quyền kiểm soát trực tiếp của bạn.
* Khi vận hành các pipeline dữ liệu gần như thời gian thực (near real-time) hoặc các bảng tổng hợp (agg) phục vụ dashboard vận hành trực tiếp.

### Khi nào không nên dùng:
* Đối với các bảng dữ liệu tĩnh như bảng từ điển cấu hình (Configuration Tables) hoặc các bảng danh mục (Dimension) rất ít khi thay đổi.
* Khi hệ thống dữ liệu chưa ổn định và tần suất lỗi hạ tầng quá cao, việc thiết lập Freshness Alerts sẽ tạo ra rất nhiều cảnh báo rác (Alert Fatigue).
...
* **Mục đích câu hỏi**: Đánh giá kiến thức về tối ưu hóa chi phí đám mây (FinOps) và cách khai thác metadata nâng cao của [Data Warehouse](/concepts/2-storage/data-warehouse/data-warehouse/).
* **Gợi ý trả lời**: Để tránh quét qua hàng tỷ dòng dữ liệu gây tốn chi phí, chúng ta nên tận dụng các bảng metadata hệ thống được cập nhật tự động bởi Data Warehouse. Ví dụ: Trong [Google BigQuery](/concepts/2-storage/cloud-data-platform/google-bigquery/), ta có thể truy vấn view hệ thống `__TABLES__` để lấy trường `last_modified_time`. Trong [Snowflake](/concepts/2-storage/cloud-data-platform/snowflake/), ta có thể truy vấn từ `INFORMATION_SCHEMA.TABLES` để lấy thông tin cập nhật mới nhất. Những câu lệnh này thực thi với độ phức tạp $O(1)$, trả về kết quả ngay lập tức và hoàn toàn miễn phí quét dữ liệu. Tuy nhiên, cần lưu ý một điểm yếu là các câu lệnh DDL (như chỉnh sửa mô tả cột) cũng có thể làm thay đổi mốc thời gian này.
...
* [Google Cloud - Designing data pipelines with SLAs](https://cloud.google.com/solutions/designing-data-pipelines-with-slas)
* [Databricks Lakehouse Monitoring Guide](https://docs.databricks.com/en/lakehouse-monitoring/index.html)
* [Snowflake Information Schema Tables Overview](https://docs.snowflake.com/en/sql-reference/info-schema/tables)
* [AWS Glue Data Quality Rules Reference](https://docs.aws.amazon.com/glue/latest/dg/data-quality-rules.html)
* [Apache Airflow SLA Concept and Alerts](https://airflow.apache.org/docs/apache-airflow/stable/core-concepts/tasks.html#slas)
* [Google BigQuery Information Schema Documentation](https://cloud.google.com/bigquery/docs/information-schema-tables)
