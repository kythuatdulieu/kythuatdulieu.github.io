...
definition: "Data Catalog (Danh mục dữ liệu) là nền tảng quản lý siêu dữ liệu tập trung giúp người dùng tìm kiếm, khám phá, hiểu rõ nguồn gốc và quản trị toàn bộ tài sản dữ liệu của doanh nghiệp."
---
...
3. **[Data Lineage](/concepts/5-quality-governance/governance-metadata/data-lineage/) (Bản đồ phả hệ dữ liệu)**: Vẽ ra sơ đồ trực quan thể hiện đường đi của dữ liệu từ hệ thống gốc, qua các bước biến đổi của ETL, cho tới khi hiển thị trên các biểu đồ Dashboard cuối cùng.
...
mcp = MetadataChangeProposalWrapper(
    entityUrn="urn:li:dataset:(urn:li:dataPlatform:bigquery,my_project.my_dataset.dim_subscription_churn,PROD)",
    aspect=properties
)
...
* Đang triển khai kiến trúc [Data Lake](/concepts/2-storage/data-lake-lakehouse/data-lake/)/Data [Lakehouse](/concepts/2-storage/data-lake-lakehouse/lakehouse/) – nơi dữ liệu phi cấu trúc đổ về ồ ạt như một đầm lầy, nếu không có Catalog thì không ai biết bên trong đầm lầy đó chứa những gì.
...
### 2. Sự khác biệt cốt lộ giữa Data Dictionary (Từ điển dữ liệu) và Data Catalog là gì?
...
* [Metadata Management (Quản lý siêu dữ liệu)](/concepts/5-quality-governance/governance-metadata/metadata-management/)
* Data Discovery
* [Data Governance (Quản trị dữ liệu)](/concepts/5-quality-governance/governance-metadata/data-governance/)
...
1. [AWS Glue Data Catalog](https://docs.aws.amazon.com/glue/latest/dg/populate-data-catalog.html) - AWS Glue Data Catalog cung cấp một kho lưu trữ siêu dữ liệu tập trung, phục vụ tìm kiếm và quản trị.
2. [Google Cloud Dataplex Catalog](https://cloud.google.com/dataplex/docs/discover-data) - Hướng dẫn khám phá và tìm kiếm dữ liệu qua dịch vụ Dataplex Catalog trên GCP.
3. [Microsoft Azure Purview Data Catalog](https://azure.microsoft.com/en-us/services/purview/) - Giải pháp quản trị dữ liệu và danh mục dữ liệu hợp nhất của Microsoft.
4. [Snowflake Data Catalog Integration](https://docs.snowflake.com/en/user-guide/data-catalog-integration) - Hướng dẫn tích hợp Snowflake với các hệ thống Data Catalog bên ngoài.
5. [Apache Atlas Metadata & Catalog](https://atlas.apache.org/1.0.0/Atlas-Architecture.html) - Framework quản trị dữ liệu và lập danh mục siêu dữ liệu mã nguồn mở trong hệ sinh thái Apache.
6. [Confluent Schema Registry & Data Catalog](https://docs.confluent.io/platform/current/schema-registry/index.html) - Quản lý schema và luồng dữ liệu thời gian thực trong kiến trúc Event Streaming.

## English Summary

A Data Catalog is an enterprise-wide metadata management and data discovery platform designed to function like an internal "Google search engine" for a company's data assets. By automatically harvesting technical metadata, data lineage, and query logs across data warehouses and BI tools, and combining them with crowdsourced business glossaries and [data profiling](/concepts/5-quality-governance/data-quality/data-profiling/) metrics, it empowers both technical and business users to find, understand, and trust data. Effectively eliminating "tribal knowledge" and tedious data support tickets, a robust Data Catalog is the crucial user-interface layer that enables true self-service analytics and underpins Data Mesh and Data Democratization strategies.