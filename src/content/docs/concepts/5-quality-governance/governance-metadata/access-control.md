...
seoTitle: "Kiểm soát truy cập RBAC và ABAC trong hệ thống dữ liệu"
...
definition: "Kiểm soát truy cập (Access Control) là cơ chế quản lý xác định quyền hạn của người dùng hoặc ứng dụng đối với tài nguyên dữ liệu, thông qua hai mô hình phổ biến là RBAC và ABAC."
---
...
Chúng ta hãy cùng xem cách triển khai kiểm soát truy cập trong [Snowflake](/concepts/2-storage/cloud-data-platform/snowflake/), kết hợp cả Row-level Security (RLS) và Column-level Security (CLS).
...
### Điểm mạnh (Pros)
* **RBAC:**
  * **Đơn giản, dễ hiểu:** Thiết lập nhanh chóng dựa trên cơ cấu tổ chức phòng ban thực tế.
  * **Hiệu năng cao:** Việc kiểm tra quyền truy cập là ánh xạ tĩnh, diễn ra rất nhanh.
  * **Dễ quản lý ở quy mô nhỏ:** Phù hợp với các startup hoặc team dưới 50 người.
* **ABAC:**
  * **Cực kỳ linh hoạt:** Kiểm soát truy cập động dựa trên ngữ cảnh thực tế (thời gian, IP, nhãn nhạy cảm).
  * **Giảm thiểu số lượng Role:** Giải quyết triệt để vấn đề bùng nổ vai trò (Role Explosion).
  * **Bảo mật chi tiết:** Cho phép triển khai Row-Level Security (RLS) và Column-Level Security (CLS) hiệu quả.

### Điểm yếu (Cons)
* **RBAC:**
  * **Bùng nổ vai trò (Role Explosion):** Khó quản lý khi số lượng phòng ban, dự án và cấp bậc tăng lên.
  * **Kém linh hoạt:** Không thể phân quyền dựa trên các điều kiện động như vị trí địa lý hoặc thời gian thực.
* **ABAC:**
  * **Cấu hình phức tạp:** Đòi hỏi thiết kế chính sách chặt chẽ và sử dụng Policy Engine chuyên biệt.
  * **Tác động hiệu năng:** Việc đánh giá nhiều thuộc tính động có thể làm tăng độ trễ (latency) khi truy vấn.
...
* **Nên dùng RBAC khi:** Dự án của bạn ở quy mô vừa và nhỏ, cấu trúc nhân sự và phân chia công việc rõ ràng (ví dụ: Data Engineer viết dữ liệu, Analyst đọc dữ liệu).
* **Nên dùng ABAC khi:** Bạn làm việc trong môi trường Enterprise lớn, đa khách hàng (Multi-tenant) hoặc các ngành đặc thù có yêu cầu khắt khe về bảo mật dữ liệu cá nhân (PII, y tế, tài chính), nơi cần phân quyền chi tiết đến từng dòng/cột dựa trên ngữ cảnh người dùng và vị trí địa lý.
...
1. [AWS IAM Access Control (RBAC vs ABAC)](https://docs.aws.amazon.com/IAM/latest/UserGuide/introduction_attribute-based-access-control.html) - Hướng dẫn chi tiết về phân quyền RBAC và ABAC trên AWS.
2. [Snowflake Row-Level Security](https://docs.snowflake.com/en/user-guide/security-row) - Hướng dẫn cấu hình Row Access Policies trong Snowflake.
3. [Snowflake Column-Level Security](https://docs.snowflake.com/en/user-guide/security-column) - Thiết lập Masking Policies bảo mật cấp cột trong Snowflake.
4. [Google Cloud IAM Overview](https://cloud.google.com/iam/docs/overview) - Kiến trúc quản lý danh tính và kiểm soát truy cập của GCP.
5. [Microsoft Azure Active Directory Access Control](https://azure.microsoft.com/en-us/services/active-directory/) - Dịch vụ quản lý định danh và kiểm soát truy cập của Azure.
6. [Apache Ranger Ranger Documentation](https://ranger.apache.org/blogs.html) - Tài liệu chính thức về framework Ranger phục vụ Access Control trong Hadoop/Spark.
