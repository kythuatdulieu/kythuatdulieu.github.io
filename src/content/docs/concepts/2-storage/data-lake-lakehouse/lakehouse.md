...
definition: "Kiến trúc Lakehouse (Data Lakehouse) là kiến trúc quản lý dữ liệu hiện đại kết hợp tính năng giao dịch ACID và quản lý cấu trúc bảng mạnh mẽ của Data Warehouse cùng khả năng lưu trữ linh hoạt, giá rẻ của Data Lake."
---

Hãy tưởng tượng bạn đang phải quản lý một hệ thống dữ liệu khổng lồ. Một mặt, bạn cần sự linh hoạt của **[Data Lake](/concepts/2-storage/data-lake-lakehouse/data-lake/) (Hồ dữ liệu)** để đổ tất cả các loại file (từ log, ảnh đến JSON, CSV) vào một kho lưu trữ giá rẻ như Amazon S3 hay Google [Cloud Storage](/concepts/2-storage/cloud-data-platform/cloud-storage/). Mặt khác, ban giám đốc và các nhà phân tích BI lại yêu cầu tốc độ truy vấn SQL cực nhanh, tính toàn vẹn dữ liệu và các giao dịch ACID mạnh mẽ vốn là đặc quyền của **[Data Warehouse](/concepts/2-storage/data-warehouse/data-warehouse/) (Kho dữ liệu)**.
...
* **Chi phí đội lên gấp bội**: Bạn phải trả tiền lưu trữ dữ liệu hai lần (một lần trên S3/ADLS dưới dạng file thô, một lần trong các đĩa lưu trữ đắt đỏ của [Snowflake](/concepts/2-storage/cloud-data-platform/snowflake/) hay Redshift) và tốn thêm chi phí compute khổng lồ cho các tác vụ ETL trung gian.
...
### Nên dùng khi:
* Doanh nghiệp của bạn đang xây dựng một hệ thống dữ liệu hiện đại trên Cloud ([Modern Data Stack](/concepts/1-foundations/system-architecture/data-platform-architecture/)) từ đầu.
...
* Hệ thống của bạn phục vụ các ứng dụng [OLTP](/concepts/2-storage/database-storage/oltp/) cần xử lý hàng ngàn giao dịch ghi/đọc siêu nhỏ mỗi giây.
...
* [Data Warehouse](/concepts/2-storage/data-warehouse/data-warehouse/)
...
* [Medallion Architecture](/concepts/2-storage/data-lake-lakehouse/medallion-architecture/)
...
6. [AWS - What is a Data Lakehouse?](https://aws.amazon.com/what-is/data-lakehouse/) - AWS product and architectural learning path overview.
7. [Google Cloud - Data Lakehouse on Dataproc](https://cloud.google.com/blog/products/databases/what-is-a-data-lakehouse) - Google Cloud blog post on Lakehouse architecture.
8. [Microsoft Azure Databricks Delta Lake](https://azure.microsoft.com/en-us/blog/azure-databricks-delta-lake-now-generally-available/) - Microsoft Azure announcement regarding Delta Lake.
9. [Snowflake - Iceberg Tables for Data Lakehouse](https://docs.snowflake.com/en/user-guide/tables-iceberg) - Snowflake's documentation on creating Lakehouse configurations.
10. [Confluent - Iceberg Data Lakehouse Sync](https://www.confluent.io/blog/iceberg-on-confluent/) - Confluent blog detailing streaming ingestion to a data lakehouse.

## English Summary

The Data Lakehouse is a modern data architecture paradigm that unifies the best elements of a Data Lake (cheap, infinitely scalable object storage for all data types like S3 or GCS) with the robust capabilities of a Data Warehouse (ACID transactions, schema enforcement, and high-performance SQL analytics). By abstracting the table management layer away from proprietary database engines and utilizing open table formats (such as Delta Lake, Apache Iceberg, or Apache Hudi) on top of open [file formats](/concepts/2-storage/database-storage/file-formats/) (like Parquet), the Lakehouse eliminates the need to maintain parallel systems or complex ETL pipelines transferring data between a lake and a warehouse. This unified "Single Source of Truth" significantly reduces costs, accelerates data freshness, and seamlessly supports both Machine Learning (AI) and Business Intelligence (BI) workloads simultaneously.