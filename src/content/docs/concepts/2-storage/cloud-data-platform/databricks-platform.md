...
definition: "Databricks Platform là nền tảng phân tích dữ liệu đám mây thống nhất tiên phong kiến trúc Lakehouse, kết hợp ưu điểm của Data Lake và Data Warehouse dựa trên Apache Spark và Delta Lake."
...
Nếu bạn từng làm việc trong một dự án dữ liệu lớn (Big Data), chắc hẳn bạn đã quen với cảnh tượng: Đội ngũ Data Engineer loay hoay viết Spark code trên các cụm máy chủ phức tạp; đội ngũ Data Scientist dùng các file Jupyter Notebook rời rạc trên máy cá nhân để train model; còn các Data Analyst lại mệt mỏi chờ đợi dữ liệu được chuyển đổi từ [Data Lake](/concepts/2-storage/data-lake-lakehouse/data-lake/) sang [Data Warehouse](/concepts/2-storage/data-warehouse/data-warehouse/) để chạy SQL báo cáo. Sự phân mảnh này không chỉ làm chậm tiến độ mà còn tạo ra những bức tường vô hình ngăn cách các phòng ban.

Đó là lý do **Databricks** ra đời — nền tảng đám mây thống nhất tiên phong đưa ra khái niệm **[Lakehouse](/concepts/2-storage/data-lake-lakehouse/lakehouse/)**, giúp kết hợp hoàn hảo những gì tốt nhất của Data Lake và Data Warehouse vào chung một mái nhà.

## Databricks thực chất là gì?

**Databricks** là một nền tảng dữ liệu, phân tích và trí tuệ nhân tạo (AI) chạy trên các dịch vụ đám mây lớn (AWS, Azure, GCP). Nền tảng này được sáng lập bởi chính những người cha đẻ của [Apache Spark](/concepts/3-integration/batch-processing/apache-spark/). 

Bằng cách sử dụng công nghệ lưu trữ Delta Lake cùng động cơ tính toán Apache Spark đã được tối ưu hóa vượt bậc, Databricks cho phép doanh nghiệp quản lý toàn bộ vòng đời của dữ liệu trên một hệ thống duy nhất: từ thu nạp dữ liệu thô, xử lý [ETL](/concepts/3-integration/etl-elt/etl/)/[ELT](/concepts/3-integration/etl-elt/elt/), phân tích BI, cho đến huấn luyện các mô hình học máy (Machine Learning) và AI.
...
* **Delta Lake**: Lớp định dạng lưu trữ mang lại khả năng giao dịch ACID, quản lý phiên bản dữ liệu ([Time Travel](/concepts/2-storage/data-lake-lakehouse/time-travel/)) và hỗ trợ xử lý cả dạng lô (Batch) lẫn dạng luồng (Streaming) đồng thời.
* **[MLflow](/concepts/6-ai-ml/genai-ml/mlflow/)**: Công cụ tiêu chuẩn để quản lý vòng đời phát triển Machine Learning, giúp theo dõi các thí nghiệm, lưu vết tham số và đóng gói mô hình để triển khai dễ dàng.

## Databricks vận hành như thế nào trong thực tế?

Databricks hoạt động như một tầng tính toán linh hoạt (Compute Layer) nằm trên các dịch vụ lưu trữ đám mây của bạn (như Amazon S3, Azure Data Lake Storage, hay Google [Cloud Storage](/concepts/2-storage/cloud-data-platform/cloud-storage/)):
```mermaid
graph TD
...
### Ưu điểm
* **Tốc độ xử lý vượt trội**: Photon engine và Apache Spark được tối ưu hóa sâu giúp thực thi các truy vấn dữ liệu lớn nhanh gấp nhiều lần bản Spark nguồn mở.
* **Môi trường cộng tác tuyệt vời**: Hợp nhất toàn bộ vòng đời dữ liệu từ ETL, BI cho đến Machine Learning / AI trên một giao diện Notebook trực quan, phá bỏ rào cản giữa các phòng ban.
* **Bảo mật và quản trị dữ liệu mạnh mẽ**: Unity Catalog hỗ trợ quản trị phả hệ dữ liệu (Data Lineage) và phân quyền bảo mật chi tiết đến cấp dòng/cột.

### Nhược điểm
* **Chi phí vận hành cao**: Tài nguyên cụm máy chủ Databricks (tính bằng DBU) rất đắt đỏ, dễ gây lãng phí nếu không quản lý tốt chính sách Auto-suspension.
* **Độ phức tạp kỹ thuật lớn**: Đòi hỏi đội ngũ kỹ sư phải có kiến thức lập trình phân tán (Spark) tốt bằng Python, Scala hoặc SQL, không phù hợp cho người dùng không biết viết code.

## Khi nào nên dùng

* **Nên dùng khi:**
  * Doanh nghiệp cần xử lý lượng dữ liệu Big Data khổng lồ ở quy mô Petabyte và có đội ngũ Data Engineers, Data Scientists làm việc cùng nhau.
  * Cần tích hợp luồng dữ liệu thời gian thực (Streaming) kết hợp với dữ liệu lô (Batch) có tính giao dịch cao trên cùng một kho lưu trữ.
  * Hạ tầng dữ liệu hướng tới mô hình Lakehouse hiện đại.

* **Không nên dùng khi:**
  * Quy mô dữ liệu của doanh nghiệp nhỏ, có thể xử lý tốt bằng các RDBMS truyền thống hoặc các giải pháp SaaS nhẹ nhàng.
  * Ngân sách dành cho hạ tầng dữ liệu hạn chế và đội ngũ không có kỹ năng vận hành, viết code Spark.
...
* [Delta Lake](/concepts/2-storage/data-lake-lakehouse/delta-lake/) - Tìm hiểu chi tiết về công nghệ lưu trữ cốt lõi Delta Lake.
* [Lệch dữ liệu - Data Skew](/concepts/3-integration/batch-processing/data-skew/) - Cách nhận diện và khắc phục lỗi lệch dữ liệu trong Spark.
...
1. [Databricks Documentation](https://docs.databricks.com/en/introduction/index.html) - The official Databricks documentation containing guides, tutorials, and developer resources.
...
4. [Delta Lake Home](https://docs.delta.io/latest/index.html) - Official website of Delta Lake, the open-source storage framework underpinning the Lakehouse architecture.
...
The Databricks Platform is a unified cloud-native analytics environment that pioneers the Lakehouse architecture, combining the scalability and low cost of a Data Lake with the ACID transactions and performance of a Data Warehouse. Built on an optimized Apache Spark engine and Delta Lake, it provides an integrated ecosystem for [data engineering](/concepts/1-foundations/foundation/data-engineering/), real-time analytics, data science, and machine learning, fostering seamless collaboration across technical teams while managing massive datasets efficiently.