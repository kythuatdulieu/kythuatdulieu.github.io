...
definition: "Snowflake Data Cloud là nền tảng dữ liệu đám mây (Cloud Data Platform) được cung cấp dưới dạng SaaS, nổi bật với kiến trúc ba lớp phân tách hoàn toàn giữa lưu trữ tập trung và tính toán đa cụm."
...
Snowflake cung cấp giải pháp Kho dữ liệu ([Data Warehouse](/concepts/2-storage/data-warehouse/data-warehouse/)), Hồ dữ liệu ([Data Lake](/concepts/2-storage/data-lake-lakehouse/data-lake/)) và Chia sẻ dữ liệu (Data Exchange) dưới dạng một Dịch vụ phần mềm hoàn chỉnh (SaaS - Software as a Service).
...
Để thấy được giá trị của Snowflake, hãy nhìn lại các kho dữ liệu đám mây thế hệ đầu (ví dụ như [Amazon Redshift](/concepts/2-storage/cloud-data-platform/amazon-redshift/) đời đầu). Các hệ thống này thường gắn chặt năng lực xử lý (Compute - CPU/RAM) và dung lượng lưu trữ (Storage - Ổ cứng) vào một cụm máy chủ cố định (Node).
...
* **Không dùng Snowflake cho các tác vụ [OLTP](/concepts/2-storage/database-storage/oltp/)**: Snowflake được tối ưu hóa cho các truy vấn phân tích (OLAP) theo lô lớn. Nếu bạn cố kết nối backend ứng dụng web vào Snowflake để chạy các lệnh insert/update từng dòng nhỏ lẻ liên tục (single-row inserts), hệ thống sẽ bị chậm và sinh ra hàng triệu tệp tin micro-partition rác làm nghẽn lớp siêu dữ liệu (Metadata).
...
### Ưu điểm
* **Kiến trúc tách biệt Compute/Storage**: Loại bỏ tranh chấp tài nguyên, các phòng ban có thể chạy các tác vụ nặng độc lập trên cùng một kho dữ liệu.
* **Hạ tầng đa đám mây (Cloud-agnostic)**: Chạy mượt mà trên AWS, Azure, GCP, giảm thiểu rủi ro Vendor lock-in.
* **Vận hành Zero-management**: Tiết kiệm tối đa nguồn lực quản trị cơ sở dữ liệu nhờ mô hình SaaS tự động hóa hoàn toàn.
* **Các tính năng đột phá**: Time Travel, Zero-Copy Cloning, và Secure Data Sharing hoạt động cực nhanh và tiết kiệm.

### Nhược điểm
* **Chi phí Credit có thể tăng nhanh**: Nếu không cấu hình Resource Monitors và Auto-suspend chặt chẽ, chi phí có thể vượt kiểm soát.
* **Không tối ưu cho OLTP**: Không thích hợp làm backend cho ứng dụng giao dịch thời gian thực với lượng cập nhật/ghi đơn lẻ liên tục.

## Khi nào nên dùng

* **Nên dùng khi:**
  * Doanh nghiệp cần xây dựng kho dữ liệu đa đám mây (Multi-cloud Data Warehouse) quy mô lớn.
  * Nhiều đội ngũ (kỹ sư dữ liệu, nhà phân tích, nhà khoa học dữ liệu) cần dùng chung kho dữ liệu nhưng muốn độc lập tài nguyên tính toán để tránh nghẽn.
  * Doanh nghiệp muốn tinh gọn đội ngũ vận hành hạ tầng kỹ thuật dữ liệu.

* **Không nên dùng khi:**
  * Quy mô dữ liệu quá nhỏ (dưới vài chục GB) và truy vấn rất đơn giản.
  * Cần một database giao dịch hỗ trợ ghi/sửa từng dòng với độ trễ cực thấp dưới 100ms.

## Các khái niệm liên quan

* [Google BigQuery](/concepts/2-storage/cloud-data-platform/google-bigquery/): Đối thủ cạnh tranh trực tiếp của Snowflake ở phân khúc Serverless DWH.
* [OLAP vs OLTP](/concepts/2-storage/database-storage/olap/): Phân biệt hệ thống phân tích và hệ thống giao dịch.
* **Data [Lakehouse](/concepts/2-storage/data-lake-lakehouse/lakehouse/)**: Kiến trúc kết hợp linh hoạt giữa Data Lake và Data Warehouse.
