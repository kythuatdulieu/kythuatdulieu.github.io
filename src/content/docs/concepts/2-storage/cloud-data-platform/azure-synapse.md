...
definition: "Azure Synapse Analytics là nền tảng phân tích hợp nhất của Microsoft, kết hợp kho dữ liệu doanh nghiệp (Enterprise Data Warehousing) và xử lý dữ liệu lớn (Big Data Analytics) trên đám mây Azure."
...
3. **Độ trễ thông tin lớn (Time-to-insight):** Các đường ống dẫn dữ liệu ([ETL](/concepts/3-integration/etl-elt/etl/)/[ELT](/concepts/3-integration/etl-elt/elt/)) cần rất nhiều thời gian để chuyển đổi và luân chuyển dữ liệu thô từ hồ dữ liệu vào kho phân tích trước khi người dùng cuối có thể xem báo cáo.
...
* **Năng lực tính toán đa dạng:** Cung cấp cả hai công cụ truy vấn SQL quen thuộc (bao gồm Dedicated SQL Pool cho hiệu năng cao và Serverless SQL Pool cho sự linh hoạt) kết hợp với cụm máy chủ [Apache Spark](/concepts/3-integration/batch-processing/apache-spark/) Pool để xử lý dữ liệu lớn bằng Python, Scala, hay [Spark SQL](/concepts/3-integration/batch-processing/spark-sql/).
...
1. **Dedicated SQL Pool (Kho dữ liệu chuyên dụng):** Đây là phiên bản nâng cấp của Azure SQL Data Warehouse. Dữ liệu được tổ chức và lưu trữ trong các bảng phân tán tối ưu, chạy trên các cụm máy chủ tính toán chuyên dụng với cấu hình định trước để phục vụ các tác vụ phân tích hiệu năng cao, tải nặng ([OLAP](/concepts/2-storage/database-storage/olap/)).
...
* **Độ phức tạp ban đầu cao:** Do tích hợp quá nhiều tính năng trong một, giao diện Synapse Workspace có thể gây bối rối cho người dùng mới so với các giải pháp đơn giản chuyên biệt như [Snowflake](/concepts/2-storage/cloud-data-platform/snowflake/).
...
* **Không nên dùng khi:**
  * Doanh nghiệp sử dụng AWS hoặc Google Cloud làm đám mây chính và không muốn phụ thuộc vào dịch vụ của Azure.
  * Đội ngũ chỉ cần một giải pháp lưu trữ/truy vấn đơn giản và có thể tự động tối ưu mà không cần can thiệp sâu vào thiết kế.
...
1. [What is Azure Synapse Analytics?](https://learn.microsoft.com/en-us/azure/synapse-analytics/overview-what-is?azure.microsoft) - Official Microsoft documentation introducing the unified Synapse analytics workspace.
2. [Azure Synapse SQL Architecture Guide](https://learn.microsoft.com/en-us/azure/synapse-analytics/sql-data-warehouse/massively-parallel-processing-mpp-architecture?azure.microsoft) - Microsoft Learn reference guide on dedicated and serverless SQL query processing architecture.
3. [Serverless SQL Pool in Azure Synapse](https://learn.microsoft.com/en-us/azure/synapse-analytics/sql/on-demand-workspace-overview?azure.microsoft) - Official guide on using the on-demand query endpoint to scan files directly on Azure Data Lake Storage.
4. [Azure Synapse Analytics Product Overview](https://azure.microsoft.com/en-us/products/synapse-analytics/) - Microsoft official product page for Azure Synapse.
5. [Azure Synapse Analytics Pricing details](https://azure.microsoft.com/en-us/pricing/details/synapse-analytics/) - Comprehensive guide on pricing structures for Dedicated SQL Pools, Serverless SQL, and Spark.
