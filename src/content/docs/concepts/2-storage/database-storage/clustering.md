...
definition: "Clustering (phân cụm) là kỹ thuật sắp xếp dữ liệu vật lý trên đĩa sao cho các dòng có giá trị tương đồng trên các cột chỉ định nằm cạnh nhau, giúp tối ưu hóa Block Pruning và tăng hiệu năng truy vấn phân tích."
---
...
Trong thực tế dự án, bạn sẽ thường kết hợp cả hai kỹ thuật này khi thiết kế bảng dữ liệu. Dưới đây là câu lệnh SQL tạo bảng mẫu trên [Google BigQuery](/concepts/2-storage/cloud-data-platform/google-bigquery/):
```sql
...
* **Bảo trì và tái phân cụm**: Khi dữ liệu mới liên tục được thêm vào bảng `(INSERT)`, trật tự sắp xếp ban đầu sẽ bị phá vỡ và phân mảnh. Các hệ thống Data Warehouse hiện đại thường tự động chạy các tác vụ Re-clustering ngầm để tái sắp xếp dữ liệu (ví dụ: BigQuery cung cấp tính năng này miễn phí, trong khi [Snowflake](/concepts/2-storage/cloud-data-platform/snowflake/) sẽ tính phí tài nguyên tính toán ngầm - serverless compute cost).
...
**Khái niệm liên quan:**
* [Partitioning](/concepts/2-storage/database-storage/partitioning/)
...
3. [Use Liquid Clustering for Delta tables](https://docs.databricks.com/en/delta/clustering.html) - Official Databricks guide on implementing Liquid Clustering for delta tables.
...
5. [Azure Synapse Analytics - Dedicated SQL Pool Table Design](https://learn.microsoft.com/en-us/azure/synapse-analytics/sql-data-warehouse/sql-data-warehouse-tables-overview) - Microsoft documentation on table distribution, partitioning, and index structures.
6. [Apache Iceberg Spec - Partitioning and Sorting](https://iceberg.apache.org/spec/#partitioning) - Apache Iceberg specification describing partitioning and sort orders.
7. [Difference between Partitioning and Clustering in BigQuery](https://www.geeksforgeeks.org/difference-between-partitioning-and-clustering-in-bigquery/) - Explanatory article on structural and usage differences between partitioning and clustering on GeeksforGeeks.
