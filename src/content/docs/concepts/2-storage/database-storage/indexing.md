...
definition: "Chỉ mục cơ sở dữ liệu (indexing) là cấu trúc dữ liệu phụ (thường là B-Tree) giúp tăng tốc độ tìm kiếm và truy xuất bản ghi trong database, đánh đổi lại dung lượng đĩa và tốc độ ghi."
---
...
3. **Bitmap Index**: Rất phổ biến trong các kho dữ liệu phân tích ([Data Warehouse](/concepts/2-storage/data-warehouse/data-warehouse/)). Cấu trúc này tối ưu cho các cột có tính đa dạng thấp (Low Cardinality) - tức là cột có ít giá trị phân biệt như Giới tính (Nam/Nữ) hay Trạng thái đơn hàng (Thành công/Thất bại).
...
* [Relational Database (Cơ sở dữ liệu quan hệ)](/concepts/2-storage/database-storage/relational-database/)
* [OLTP (Xử lý giao dịch trực tuyến)](/concepts/2-storage/database-storage/oltp/)
...
1. [Use The Index, Luke!](https://use-the-index-luke.com/sql/where-clause) - A developer-centric guide to database indexing by Markus Winand.
...
3. [AWS Redshift Sorting and Indexing Design](https://docs.aws.amazon.com/redshift/latest/dg/t_Sorting_data.html) - AWS documentation explaining sort keys and block-level metadata indexing in Redshift.
4. [Google Cloud Spanner Secondary Indexes](https://cloud.google.com/spanner/docs/secondary-indexes) - Official Google Cloud guide on creating and optimizing secondary indexes.
5. [Azure SQL Database Query Performance Guidance](https://learn.microsoft.com/en-us/azure/azure-sql/database/performance-guidance) - Microsoft documentation on index optimizations and query tuning.
6. [Databricks Delta Lake Indexing and Optimizations](https://docs.databricks.com/en/delta/optimizations-index.html) - Databricks documentation describing Z-Ordering and multi-dimensional clustering indexes.
7. [Optimization and Indexes in MySQL](https://dev.mysql.com/doc/refman/8.0/en/optimization-indexes.html) - Official MySQL reference manual outlining query performance and index optimization.
