...
definition: "Row-based Storage (lưu trữ dạng dòng) là phương pháp tổ chức dữ liệu vật lý trên ổ đĩa theo hàng liên tiếp, tối ưu hóa cho các thao tác đọc ghi toàn bộ thực thể của hệ thống giao dịch (OLTP)."
---
...
* **Gợi ý trả lời**: 
  ...
  - Chúng ta chọn **Column-based** cho các hệ thống [OLAP](/concepts/2-storage/database-storage/olap/) (như Data Warehouse để phân tích) nơi dữ liệu hầu như chỉ đọc (Read-heavy), ít khi cập nhật từng dòng và các câu truy vấn thường quét qua hàng triệu dòng dữ liệu nhưng chỉ tập trung tính toán tổng hợp trên một vài cột cụ thể (như tính tổng doanh thu, đếm số lượng người dùng).
...
2. [AWS - Row-Oriented vs. Column-Oriented Databases](https://docs.aws.amazon.com/redshift/latest/dg/c_columnar_storage_disk_mem_width.html) - AWS comparison guide detailing performance trade-offs, transactional vs. analytical workloads, and storage patterns.
3. [Google Cloud Spanner - Schema and Data Model](https://cloud.google.com/spanner/docs/schema-and-data-model) - Google Cloud guide on physical layout, table storage, and row-oriented structuring.
4. [Azure SQL Database Features and Physical Design](https://learn.microsoft.com/en-us/azure/azure-sql/database/features-comparison) - Microsoft guide on Azure SQL Database PaaS structures and transactional engines.
5. [Snowflake Table Storage and Temp Tables](https://docs.snowflake.com/en/user-guide/tables-temp-transient) - Snowflake guide detailing storage, micro-partitioning, and transactional tables.
6. [PostgreSQL Physical Storage Layout](https://www.postgresql.org/docs/current/storage.html) - Official PostgreSQL documentation on how tables and rows are physically structured as pages and blocks on disk.
7. [C-Store: A Column-Oriented DBMS](https://dl.acm.org/doi/10.1145/1083592.1083658) - Seminal research paper by Michael Stonebraker et al. contrasting column store with traditional row store.
