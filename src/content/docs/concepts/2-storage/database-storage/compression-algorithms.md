...
definition: "Thuật toán nén dữ liệu (compression algorithms) là các kỹ thuật giảm kích thước vật lý của dữ liệu trên đĩa hoặc đường truyền mạng, giải quyết điểm nghẽn Disk/Network I/O trong các hệ thống Big Data."
---
...
![Sơ đồ kiến trúc minh họa cho Compression Algorithms](/images/compression-algorithms/compression-algorithms.png)
...
Các thuật toán nén phổ biến nhất trong [Data Engineering](/concepts/1-foundations/foundation/data-engineering/) ngày nay bao gồm: **Snappy** (do Google phát triển), **Gzip** (chuẩn nén GNU kinh điển), **Zstandard/Zstd** (do Facebook phát triển) và **LZ4**. Hầu hết chúng đều dựa trên các biến thể của thuật toán từ điển nổi tiếng LZ77/LZ78 kết hợp với kỹ thuật mã hóa entropy (như Huffman coding).

## Tại sao chúng ta cần nén dữ liệu? (Khi CPU không còn là điểm nghẽn)

Trong các hệ thống tính toán phân tán hiện đại (như [Apache Spark](/concepts/3-integration/batch-processing/apache-spark/), Hadoop, Kafka), CPU rất ít khi là nút thắt cổ chai làm chậm hệ thống. Thay vào đó, giới hạn lớn nhất về mặt hiệu năng thường nằm ở:
1. **Tốc độ đọc ghi ổ đĩa (Disk I/O)**: Tốc độ đọc dữ liệu thô từ ổ cứng HDD hoặc SSD vào RAM.
2. **Băng thông mạng (Network I/O)**: Quá trình truyền tải dữ liệu giữa các máy chủ trong mạng (ví dụ trong giai đoạn [Shuffle](/concepts/3-integration/batch-processing/shuffle/) của Spark).

Nén dữ liệu giải quyết cả hai bài toán này theo cách cực kỳ thông minh:
* **Tiết kiệm chi phí lưu trữ**: Giảm dung lượng file trên [Cloud Storage](/concepts/2-storage/cloud-data-platform/cloud-storage/) (như Amazon S3 hay GCS) xuống nhiều lần, giảm trực tiếp hóa đơn lưu trữ hàng tháng.
...
1. **Chia khối (Block/[Chunking](/concepts/6-ai-ml/genai-ml/chunking/))**: File dữ liệu lớn được chia thành các nhóm dòng (row groups) hoặc các block nhỏ (ví dụ 64MB hoặc 128MB). Việc nén sẽ được thực hiện độc lập trên từng block này.
...
* **Cấu hình cấp độ nén quá cao (Over-compression)**: Việc tăng cấu hình nén của Zstd lên mức tối đa (ví dụ level 19) để cố gắng tiết kiệm thêm vài Megabytes dữ liệu là không cần thiết. Nó sẽ làm tăng thời gian xử lý của CPU lên gấp 10 lần, kéo dài thời gian hoàn thành của toàn bộ pipeline [ETL](/concepts/3-integration/etl-elt/etl/).
...
### Khi nào nên dùng:
* **Snappy/LZ4**: Sử dụng khi truyền tải dữ liệu giữa các node mạng trong Spark Shuffle hoặc Kafka messages, nơi tốc độ truyền là tối thượng và không muốn tốn nhiều CPU.
* **Zstd**: Sử dụng cho các file dữ liệu phân tích dài hạn lưu trữ trong Data Lake (như Parquet, ORC, Avro) để tối ưu hóa đồng thời dung lượng và hiệu suất đọc.
* **Gzip**: Sử dụng cho cold data hoặc log files lưu trữ dài hạn ít khi truy vấn tới.

### Khi nào không nên dùng:
* Không nén các file hình ảnh, âm thanh, video đã được nén sẵn bằng các định dạng chuyên dụng.
* Không dùng Gzip cho các file text dung lượng lớn nếu cần xử lý song song phân tán (do Gzip không hỗ trợ split-block).
...
* **Gợi ý trả lời**:
  ...
  * Lý do là Zstd giải quyết được bài toán đánh đổi kinh điển giữa tốc độ và dung lượng nén. Nó mang lại tỷ lệ nén tốt ngang ngửa Gzip nhưng tốc độ đọc ghi tiệm cận Snappy. Điều này giúp doanh nghiệp vừa tối ưu được chi phí lưu trữ trên S3/GCS, vừa đảm bảo tốc độ truy vấn vượt trội cho các engine như Athena, Trino hay [Spark SQL](/concepts/3-integration/batch-processing/spark-sql/).

## Khái niệm liên quan & Tài liệu tham khảo

**Khái niệm liên quan:**
* [Columnar Storage](/concepts/2-storage/database-storage/columnar-storage/)
...
* [Định dạng tệp dữ liệu - File Formats Deep Dive](/concepts/2-storage/database-storage/file-formats-deep-dive/)
...
2. [Compression in Apache Kafka: Gzip, Snappy, LZ4, Zstd](https://docs.confluent.io/platform/current/kafka/compression.html) - Confluent documentation analyzing and benchmarking different compression formats for stream processing.
3. [AWS Redshift Compression Encodings](https://docs.aws.amazon.com/redshift/latest/dg/c_Byte_dictionary_compression.html) - AWS documentation describing compression encoding types such as ZSTD, LZO, and AZ64.
4. [Google Cloud BigQuery Column-level Compression](https://cloud.google.com/bigquery/docs/storage) - GCP guide on how BigQuery manages data compression and analytical performance.
5. [Apache Spark Tuning Guide - Compression and Serialization](https://spark.apache.org/docs/latest/tuning.html#data-serialization) - Apache Spark official documentation on optimizing memory and shuffle compression.
6. [Zstandard Compression Project](https://github.com/facebook/zstd) - Official GitHub repository and performance metrics for the Meta Zstd algorithm.
7. [Google Snappy Project](https://github.com/google/snappy) - Official GitHub repository for Google's fast compression/decompression library Snappy.
