...
OLAP là kiến trúc cơ sở dữ liệu được tối ưu hóa đặc biệt cho các tác vụ đọc dữ liệu (read-heavy), tính toán tổng hợp (aggregations) trên các tập dữ liệu lịch sử khổng lồ lên tới hàng Terabyte hoặc Petabyte. Nó chính là công nghệ lõi đứng sau các kho dữ liệu ([Data Warehouse](/concepts/2-storage/data-warehouse/data-warehouse/)) và các công cụ báo cáo thông minh (BI Tools).
...
Nếu chạy trên một database dạng hàng thông thường, câu lệnh này sẽ mất rất nhiều thời gian. Nhưng trên một OLAP Engine sử dụng lưu trữ dạng cột ([Columnar Storage](/concepts/2-storage/database-storage/columnar-storage/)), kết quả sẽ được trả về chỉ trong vòng vài giây ngắn ngủi.
...
* [So sánh OLTP vs OLAP vs HTAP](/concepts/2-storage/database-storage/oltp-olap-htap-comparison/)
...
2. [Designing Data-Intensive Applications](https://www.oreilly.com/library/view/designing-data-intensive-applications/9781491903063/) - Book by Martin Kleppmann analyzing storage engines, column-oriented storage, and OLAP on O'Reilly.
3. [Snowflake Documentation: OLAP vs. OLTP](https://docs.snowflake.com/en/user-guide/intro-key-concepts) - Detailed guide explaining the differences between transactional and analytical processing.
4. [Google Cloud BigQuery Storage Overview](https://cloud.google.com/bigquery/docs/storage) - GCP guide on analytical storage architecture and columnar tables.
5. [AWS Redshift Data Warehouse System Overview](https://docs.aws.amazon.com/redshift/latest/dg/c_challenges_in_data_warehouse_design.html) - AWS documentation outlining dimensional modeling and analytical database design.
6. [Microsoft Azure Analytical Architecture Guide](https://learn.microsoft.com/en-us/azure/architecture/data-guide/relational-data/online-analytical-processing) - Overview of analytical data store architecture and dimensional modeling in Azure.
7. [Databricks Lakehouse Platform Guide](https://docs.databricks.com/en/lakehouse/index.html) - Databricks architectural overview on combining OLAP engine features on data lakes.
8. [ClickHouse Documentation: What is OLAP?](https://clickhouse.com/docs/en/intro/) - High-level introduction to analytical database systems and columnar storage principles.
...
OLAP (Online Analytical Processing) systems are specialized database engines designed to answer complex, multi-dimensional queries and perform high-speed aggregations across vast amounts of historical data. Operating as the backbone for Data Warehouses and BI tools, OLAP typically employs columnar storage and denormalized dimensional models ([Star Schema](/concepts/2-storage/data-warehouse/star-schema/)) to optimize read-heavy analytical workloads. Unlike OLTP, OLAP is not suitable for high-frequency transactional updates, but excels at enabling analysts to slice, dice, roll-up, and drill-down into data to extract business intelligence.