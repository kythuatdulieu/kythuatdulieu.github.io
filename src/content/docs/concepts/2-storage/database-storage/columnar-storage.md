...
definition: "Columnar Storage (lưu trữ dạng cột) là phương pháp tổ chức dữ liệu vật lý trên ổ đĩa theo cột thay vì theo dòng, tối ưu hóa băng thông I/O cho các truy vấn phân tích (OLAP) và nâng cao tỷ lệ nén."
---
...
* **Sử dụng định dạng tệp tin tối ưu**: Khi làm việc trên [Data Lake](/concepts/2-storage/data-lake-lakehouse/data-lake/), hãy chuyển dữ liệu từ các định dạng thô (như JSON, CSV) sang các định dạng cột mở như **Apache Parquet** để tận dụng tối đa tốc độ truy vấn.
...
### Nên chọn khi:
* Bạn xây dựng kiến trúc Data Warehouse, Data [Lakehouse](/concepts/2-storage/data-lake-lakehouse/lakehouse/) hoặc các hệ thống OLAP phục vụ báo cáo phân tích.
...
**Khái niệm liên quan:**
* [Row-based Storage](/concepts/2-storage/database-storage/row-based-storage/)
* [OLAP (Xử lý phân tích trực tuyến)](/concepts/2-storage/database-storage/olap/)
* [File Formats (Định dạng tệp dữ liệu)](/concepts/2-storage/database-storage/file-formats/)
...
2. [Apache Parquet Official Documentation](https://parquet.apache.org/docs/) - Official documentation for Apache Parquet, the industry-standard open source columnar file format.
3. [AWS Redshift Storage Architecture](https://docs.aws.amazon.com/redshift/latest/dg/c_columnar_storage_disk_mem_layout.html) - AWS documentation explaining sorting and columnar data layouts in Amazon Redshift.
4. [Google Cloud BigQuery Storage Overview](https://cloud.google.com/bigquery/docs/storage) - GCP guide on physical storage layouts and analytical performance inside BigQuery.
5. [Azure Synapse Dedicated SQL Pool Table Structures](https://learn.microsoft.com/en-us/azure/synapse-analytics/sql/data-loading-best-practices) - Microsoft documentation detailing columnstore indexes and query optimizations.
6. [What is a Columnar Database?](https://www.snowflake.com/guides/columnar-database/) - Snowflake Guide describing how columnar database storage structures and speeds up OLAP queries.
7. [What is a Column-Oriented Database?](https://clickhouse.com/docs/en/concepts/column-oriented-database) - ClickHouse's conceptual guide to columnar database storage and its performance benefits for analytical workloads.
