...
seoTitle: "Mô hình hóa đa chiều (Dimensional Modeling) trong Data Warehouse"
...
definition: "Mô hình hóa đa chiều (Dimensional Modeling) là phương pháp thiết kế cơ sở dữ liệu tối ưu hóa tốc độ truy vấn phân tích (OLAP), bao gồm hai loại bảng chính: Fact Table và Dimension Table."
---
...
Để trả lời những câu hỏi mang tính chất "bức tranh toàn cảnh" này một cách nhanh chóng, chúng ta không thể dựa vào các cấu trúc dữ liệu giao dịch thông thường. Đó là lý do **Dimensional Modeling** (Mô hình hóa dữ liệu đa chiều) ra đời. Đây là một nghệ thuật thiết kế cơ sở dữ liệu chuyên biệt dành cho Kho dữ liệu ([Data Warehouse](/concepts/2-storage/data-warehouse/data-warehouse/)) và Data Mart, giúp biến những bảng dữ liệu phức tạp thành một cấu trúc trực quan, dễ hiểu và cực kỳ nhanh khi truy vấn.

## Tại sao chúng ta cần Dimensional Modeling?

Trước đây, khi các công nghệ lưu trữ còn sơ khai, hầu hết các hệ thống báo cáo đều truy vấn trực tiếp vào cơ sở dữ liệu giao dịch ([OLTP](/concepts/2-storage/database-storage/oltp/)). Các cơ sở dữ liệu này được thiết kế theo mô hình ER (Entity-Relationship) chuẩn hóa dạng 3 (3NF) với mục tiêu tối thượng là: **đảm bảo dữ liệu không bị trùng lặp**, từ đó giúp việc ghi dữ liệu (insert, update) diễn ra trơn tru nhất.

Tuy nhiên, khi mang mô hình 3NF này đi làm phân tích dữ liệu ([OLAP](/concepts/2-storage/database-storage/olap/)), các kỹ sư lập tức vấp phải hai "cơn ác mộng":
...
2. **Xác định Độ mịn ([Grain](/concepts/2-storage/data-warehouse/grain/))**: Đây là bước cực kỳ quan trọng. Bạn phải quyết định một dòng dữ liệu trong bảng Fact đại diện cho cái gì ở mức chi tiết nhất. Ví dụ: *"Mỗi dòng đại diện cho một đêm nghỉ của một phòng"* (Nightly room grain).
...
```
Nhờ giảm thiểu số lượng JOIN, database engine (đặc biệt là các hệ quản trị phân tích như BigQuery, [Snowflake](/concepts/2-storage/cloud-data-platform/snowflake/), Redshift) sẽ xử lý câu lệnh này với tốc độ đáng kinh ngạc.
...
* **Bảo trì phức tạp khi dữ liệu thay đổi**: Nếu một thuộc tính thay đổi (như tên tỉnh thành bị đổi), hệ thống [ETL](/concepts/3-integration/etl-elt/etl/) sẽ phải cập nhật rất nhiều dòng dữ liệu bị trùng lặp. (Để giải quyết, bạn sẽ cần áp dụng kỹ thuật [Slowly Changing Dimension](/concepts/2-storage/data-warehouse/slowly-changing-dimension/) - SCD).
...
* Dữ liệu dạng phi cấu trúc như hình ảnh, âm thanh, video hay text log thô (đối với nhóm này, [Data Lake](/concepts/2-storage/data-lake-lakehouse/data-lake/) hoặc các giải pháp hồ dữ liệu sẽ phù hợp hơn).

## Khái niệm liên quan
* [Star Schema](/concepts/2-storage/data-warehouse/star-schema/)
* [Snowflake Schema](/concepts/2-storage/data-warehouse/snowflake-schema/)
* [Fact Table](/concepts/2-storage/data-warehouse/fact-table/)
* [Dimension Table](/concepts/2-storage/data-warehouse/dimension-table/)
...
1. [AWS: Dimensional Modeling Guide](https://aws.amazon.com/blogs/big-data/best-practices-for-designing-dimensional-models-in-amazon-redshift/) - Architectural guidelines for implementing dimensional modeling on Amazon Redshift.
2. [Google Cloud: Star Schema in BigQuery](https://cloud.google.com/bigquery/docs/schemadic) - Designing and running dimensional star schemas on GCP's analytical platform.
3. [Azure: Relational Data Warehousing Guide](https://azure.microsoft.com/en-us/solutions/data-warehouse/) - Microsoft handbook on designing dimensional models and fact/dimension tables.
4. [Snowflake: Star Schema and Dimensional Models](https://docs.snowflake.com/en/user-guide/data-share-consumers) - Snowflake documentation on design patterns for dimensional schemas.
5. [Databricks: Gold Layer Dimensional Modeling](https://www.databricks.com/glossary/medallion-architecture) - Implementing structured star schemas within the Medallion lakehouse architecture.
6. [Apache Hive: Hive Modeling Best Practices](https://cwiki.apache.org/confluence/display/Hive/DesignPatterns) - Schema design and optimization principles for Hive tables.
