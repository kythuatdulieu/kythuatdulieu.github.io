...
Khác với các cơ sở dữ liệu vận hành ([OLTP](/concepts/2-storage/database-storage/oltp/) - Online Transaction Processing) được thiết kế tối ưu cho việc ghi nhận giao dịch nhanh chóng với độ trễ cực thấp, Data Warehouse sử dụng kiến trúc [OLAP](/concepts/2-storage/database-storage/olap/) (Online Analytical Processing) tối ưu cho việc đọc dữ liệu quy mô lớn và tổng hợp thông tin (Aggregation) trên hàng triệu, hàng tỷ dòng.
...
4. **Nạp dữ liệu ([Data Loading](/concepts/3-integration/etl-elt/data-loading/))**: Ghi dữ liệu sạch vào các mô hình đa chiều (Dimensional Model) của kho dữ liệu.
...
### Điểm mạnh (Pros)
* **Tập trung hóa dữ liệu**: Hợp nhất dữ liệu từ nhiều nguồn phân tán (OLTP, CRM, ERP, API) thành một nguồn duy nhất đáng tin cậy.
* **Tối ưu hóa phân tích**: Kiến trúc OLAP giúp xử lý các truy vấn phức tạp trên hàng triệu dòng cực nhanh mà không ảnh hưởng đến hệ thống vận hành chính.
* **Bảo toàn lịch sử**: Lưu trữ dữ liệu lịch sử lâu dài, cho phép phân tích xu hướng và so sánh dữ liệu qua nhiều năm.
* **Chất lượng dữ liệu**: Dữ liệu được làm sạch, biến đổi và chuẩn hóa thông nhất trước khi nạp vào kho.

### Điểm yếu (Cons)
* **Chi phí cao**: Chi phí đầu tư hạ tầng, bản quyền phần mềm và tài nguyên tính toán/lưu trữ ban đầu rất lớn.
* **Độ trễ dữ liệu**: Thường chạy theo lô (Batch Processing) định kỳ nên dữ liệu không đạt thời gian thực (Real-time) tuyệt đối.
* **Tính linh hoạt kém**: Khó thay đổi cấu trúc bảng khi nghiệp vụ biến động nhanh và không tối ưu cho dữ liệu phi cấu trúc (Unstructured Data).

## Khi nào nên và không nên dùng

### Khi nào nên dùng
* Khi doanh nghiệp cần làm báo cáo quản trị chuyên sâu (BI) kết hợp thông tin từ nhiều phòng ban khác nhau.
* Khi hiệu năng của các cơ sở dữ liệu giao dịch (OLTP) bị suy giảm nghiêm trọng do các truy vấn phân tích quy mô lớn.
* Khi cần lưu giữ vết lịch sử thay đổi của thực thể kinh doanh để phân tích xu hướng lâu dài.

### Khi nào không nên dùng
* Doanh nghiệp nhỏ, nhu cầu phân tích đơn giản, chỉ cần truy vấn trực tiếp trên cơ sở dữ liệu vận hành.
* Các bài toán đòi hỏi xử lý thời gian thực với độ trễ tính bằng mili-giây (như phát hiện gian lận thẻ tín dụng hoặc IoT telemetry streaming).
* Dữ liệu chủ yếu là phi cấu trúc (ảnh, video, âm thanh) và chưa xác định rõ mô hình khai thác.
...
* **Gợi ý trả lời**:
  ...
  * **Lựa chọn**: Trong thời đại ngày nay, chi phí lưu trữ đĩa cứng đã rất rẻ. Do đó, hầu hết các DWH hiện đại đều ưu tiên chọn **Star Schema** để đổi dung lượng lưu trữ lấy tốc độ truy vấn tối đa. Chỉ chọn [Snowflake](/concepts/2-storage/cloud-data-platform/snowflake/) khi dimension quá lớn và việc cập nhật dữ liệu trùng lặp gặp khó khăn nghiêm trọng về mặt quản trị.
...
* [Star Schema](/concepts/2-storage/data-warehouse/star-schema/)
...
1. [Snowflake: Designing and Organizing a Data Warehouse](https://docs.snowflake.com/en/user-guide/data-share-consumers) - Best practices and architectural designs for modern cloud data warehouses.
2. [AWS: What is a Data Warehouse?](https://aws.amazon.com/data-warehouse/) - Core concepts, architectures, and design trade-offs of AWS analytical solutions.
3. [Google Cloud: Data Warehouse Architectures](https://cloud.google.com/architecture/data-warehouse-architecture-concepts) - Conceptual overview of building enterprise data warehouses on BigQuery.
4. [Azure: Data Warehousing in Microsoft Azure](https://azure.microsoft.com/en-us/solutions/data-warehouse/) - Design patterns and guide for relational data warehousing in Azure Synapse.
5. [Databricks: Medallion Architecture](https://www.databricks.com/glossary/medallion-architecture) - Comprehensive reference on Medallion Architecture and data warehousing using Lakehouse concepts.
6. [Apache Hive: LanguageManual DDL](https://cwiki.apache.org/confluence/display/Hive/LanguageManual+DDL) - Apache Hive official documentation on defining schemas, partitions, and analytical tables.

## English Summary

A Data Warehouse (DWH) is a centralized, integrated, non-volatile, and time-variant database optimized for analytical processing (OLAP) rather than transactional workloads (OLTP). By consolidating data from disparate [source systems](/concepts/1-foundations/foundation/source-systems/), resolving formatting inconsistencies, and maintaining historical changes (using techniques like Slowly Changing Dimensions), a DWH serves as the single source of truth for business intelligence, reporting, and ad-hoc analytics. The architecture typically relies on dimensional modeling (Star or Snowflake Schema) consisting of Fact Tables for metrics and Dimension Tables for descriptive context, utilizing surrogate keys to ensure data integrity and optimal join performance.