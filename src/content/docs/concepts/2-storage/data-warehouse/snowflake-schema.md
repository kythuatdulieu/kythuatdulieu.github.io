...
seoTitle: "Snowflake Schema (Lược đồ bông tuyết) - So sánh và Ứng dụng thực tế"
...
Khi vẽ sơ đồ quan hệ thực thể (ERD), bảng sự kiện ([Fact Table](/concepts/2-storage/data-warehouse/fact-table/)) vẫn nằm ở trung tâm, nhưng các bảng chiều phân cấp kết nối chéo với nhau tỏa ra xung quanh, tạo thành hình dáng phức tạp giống như một bông tuyết.
...
2. **Dimension Tables phân cấp**: Thay vì bẻ phẳng toàn bộ thuộc tính vào một bảng duy nhất (phi chuẩn hóa), [Snowflake](/concepts/2-storage/cloud-data-platform/snowflake/) tách các thông tin có tính phân cấp (hierarchy) thành các bảng tra cứu phụ (lookup tables) và liên kết ngược lại bảng chiều chính bằng khóa ngoại.
...
2. **Khó khăn khi bảo trì (Data Anomalies)**: Nếu doanh nghiệp muốn đổi tên danh mục từ "Electronics" thành "Consumer Electronics", hệ thống [ETL](/concepts/3-integration/etl-elt/etl/) của Star Schema phải quét và cập nhật lại hàng vạn dòng dữ liệu. Với Snowflake, công việc này cực kỳ đơn giản: bạn chỉ cần cập nhật đúng một dòng duy nhất trong bảng `dim_category`.
...
### Điểm mạnh (Pros)
* **Tiết kiệm dung lượng**: Việc chuẩn hóa triệt tiêu tối đa sự trùng lặp chuỗi văn bản dài trong các bảng chiều.
* **Dễ dàng bảo trì**: Khi một thuộc tính phân cấp thay đổi (như đổi tên danh mục), bạn chỉ cần cập nhật một dòng duy nhất tại bảng lookup tương ứng.
* **Hạn chế dị biệt dữ liệu**: Đảm bảo tính nhất quán dữ liệu ở mức độ cao, tránh lỗi logic khi cập nhật.

### Điểm yếu (Cons)
* **Giảm hiệu năng truy vấn**: Việc thực hiện quá nhiều phép JOIN liên tiếp khiến database tiêu tốn nhiều CPU và RAM, tăng thời gian phản hồi.
* **Cấu trúc phức tạp**: Lược đồ phân nhánh nhiều lớp rất khó hiểu đối với Business Analysts khi viết SQL hoặc tự kéo thả báo cáo.
* **Không tối ưu cho DB dạng cột**: Các cơ chế nén cột hiện đại đã giải quyết tốt vấn đề trùng lặp, khiến ưu thế tiết kiệm đĩa của Snowflake không còn rõ rệt.
...
### Khi nào nên dùng
* Khi bảng chiều có kích thước cực kỳ lớn (Monster Dimensions) và có các nhánh phân cấp thay đổi độc lập cần quản lý riêng biệt.
* Khi hệ thống phân tích sử dụng công cụ BI có lớp ngữ nghĩa (Semantic Layer) che giấu được các phép JOIN bên dưới để người dùng cuối không bị rối.

### Khi nào không nên dùng
* Khi xây dựng kho dữ liệu trên các OLAP engine hiện đại dạng cột (BigQuery, Redshift, Snowflake Cloud DWH) vốn ưu tiên Star Schema để đạt tốc độ tối đa.
* Người dùng trực tiếp viết mã SQL để khai thác dữ liệu báo cáo nhanh (Ad-hoc Queries).

---

## Khái niệm liên quan
* [Star Schema](/concepts/2-storage/data-warehouse/star-schema/): Lược đồ hình sao – mô hình đơn giản và phổ biến nhất.
* [Dimensional Modeling](/concepts/2-storage/data-warehouse/dimensional-modeling/): Phương pháp luận mô hình hóa dữ liệu đa chiều.
* Third Normal Form (3NF): Dạng chuẩn hóa dữ liệu thông thường trong các hệ thống [OLTP](/concepts/2-storage/database-storage/oltp/).
...
1. [AWS: Redshift Normalization and Snowflake Schema](https://aws.amazon.com/blogs/big-data/best-practices-for-designing-dimensional-models-in-amazon-redshift/) - Architectural trade-offs of snowflake schemas in Redshift.
2. [Google Cloud: Table Schema and Star/Snowflake Design](https://cloud.google.com/bigquery/docs/schemas) - Best practices on modeling normalization levels in BigQuery.
3. [Azure: Relational Data Warehousing architecture](https://azure.microsoft.com/en-us/solutions/data-warehouse/) - Microsoft handbook on designing and deploying normalized dimension structures in Azure Synapse.
4. [Snowflake: Star vs. Snowflake Schema performance](https://docs.snowflake.com/en/user-guide/data-sharing-intro) - Query tuning and join patterns in Snowflake.
5. [Databricks: Normalizing Dimensions on Delta Lake](https://www.databricks.com/glossary/medallion-architecture) - Managing normalized lookup structures and nested dimensions in Delta tables.
6. [Apache Hive: Hive Design Patterns and Normalization](https://cwiki.apache.org/confluence/display/Hive/DesignPatterns) - Setting up normalized dimensions and lookup tables in Hive.
