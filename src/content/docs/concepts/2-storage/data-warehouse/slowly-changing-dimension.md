...
definition: "Chiều thay đổi chậm (SCD - Slowly Changing Dimension) là tập hợp các kỹ thuật quản lý và lưu trữ dữ liệu lịch sử khi các thuộc tính của bảng chiều biến động theo thời gian."
---
...
Khi dữ liệu nguồn thay đổi, SCD định nghĩa cách thức hệ thống [ETL](/concepts/3-integration/etl-elt/etl/)/[ELT](/concepts/3-integration/etl-elt/elt/) ghi nhận sự thay đổi đó vào kho dữ liệu. Mục tiêu tối thượng của SCD là đảm bảo tính chính xác theo thời gian (point-in-time accuracy), giúp các báo cáo phân tích phản ánh đúng ngữ cảnh lịch sử tại thời điểm sự kiện diễn ra.
...
* **Nếu chúng ta làm như database nguồn ([OLTP](/concepts/2-storage/database-storage/oltp/))**: Chỉ đơn giản chạy lệnh `UPDATE` cập nhật khu vực của John thành "TP.HCM".
...
flowchart TD
    ...
    B -->|No| C["Insert New Record<br/>is_active=TRUE"]
    B -->|Yes| D["Update Old Record<br/>is_active=FALSE<br/>end_date=NOW"]
    D --> E["Insert New Version<br/>is_active=TRUE<br/>start_date=NOW"]
...
Viết code SQL thuần để xử lý logic Update dòng cũ và Insert dòng mới rất phức tạp và dễ nhầm lẫn. Trong [Modern Data Stack](/concepts/1-foundations/system-architecture/data-platform-architecture/), công cụ **[dbt](/concepts/3-integration/transformation-analytics/dbt/) (Data Build Tool)** hỗ trợ giải quyết việc này vô cùng ngắn gọn thông qua tính năng **dbt Snapshots**:
```sql
...
### Điểm mạnh (Pros)
* **Bảo toàn ngữ cảnh lịch sử**: SCD Type 2 giúp lưu trữ chính xác các trạng thái thay đổi theo thời gian, ngăn lỗi sai lệch số liệu lịch sử.
* **Tối ưu hóa dung lượng (Type 1)**: Giúp cập nhật nhanh chóng các thông tin sai sót hoặc không có giá trị phân tích lịch sử mà không tốn dung lượng lưu trữ.
* **Hỗ trợ phân tích đa dạng**: SCD Type 3 và Type 6 hỗ trợ phân tích song song giữa cấu trúc cũ và mới.

### Điểm yếu (Cons)
* **Tăng độ phức tạp của ETL**: Logic xử lý SCD Type 2, 4, 6 đòi hỏi lập trình pipeline phức tạp và kiểm thử kỹ lưỡng.
* **Phình to dung lượng**: Nếu áp dụng Type 2 cho các thuộc tính thay đổi quá nhanh, bảng chiều sẽ phình to nhanh chóng làm giảm hiệu năng truy vấn.

## Khi nào nên và không nên dùng

### Khi nào nên dùng
* Thiết kế các bảng Dimension cốt lõi của Data Warehouse cần theo dõi sự biến động thuộc tính (Khách hàng, Sản phẩm, Nhân viên).
* Xây dựng hệ thống báo cáo BI có yêu cầu so sánh hiệu quả kinh doanh theo thời gian.

### Khi nào không nên dùng
* Khi thuộc tính thay đổi quá nhanh (ví dụ: nhiệt độ, tọa độ GPS, số dư tài khoản ngân hàng) — nên chuyển sang bảng Fact.
* Thiết kế cơ sở dữ liệu giao dịch OLTP yêu cầu ghi dữ liệu nhanh và tránh trùng lặp.

---

## Khái niệm liên quan
* [Dimension Table](/concepts/2-storage/data-warehouse/dimension-table/): Bảng chiều chứa các thông tin mô tả ngữ cảnh.
* [Surrogate Key](/concepts/2-storage/data-warehouse/surrogate-key/): Khóa thay thế nhân tạo trong thiết kế kho dữ liệu.
* [Data Warehouse](/concepts/2-storage/data-warehouse/data-warehouse/): Kho dữ liệu tổng hợp phục vụ phân tích.
...
1. [AWS: Redshift Managing Slowly Changing Dimensions](https://aws.amazon.com/blogs/big-data/best-practices-for-designing-dimensional-models-in-amazon-redshift/) - Strategies for SCD updates in Redshift.
2. [Google Cloud: Star Schema and SCD in BigQuery](https://cloud.google.com/bigquery/docs/schemas) - Best practices for modeling and tracking SCD changes in BigQuery.
3. [Azure: Designing Slowly Changing Dimensions](https://learn.microsoft.com/en-us/azure/architecture/data-guide/relational-data/data-warehousing) - Design patterns for SCD tracking on Azure Synapse.
4. [Snowflake: Tracking History with SCD Type 2](https://www.snowflake.com/trending/data-vault) - Design patterns for managing dimension tables history in Snowflake.
5. [Databricks: Slowly Changing Dimensions (SCD) in Delta Lake](https://docs.databricks.com/en/delta-live-tables/cdc.html) - Technical guide to implementing Delta Lake SCD Type 1 & 2.
6. [Confluent: CDC and slowly changing dimensions](https://www.confluent.io/blog/cdc-and-slowly-changing-dimensions/) - Implementing CDC for slowly changing dimension tracking on streaming databases.
