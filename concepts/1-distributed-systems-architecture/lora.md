...
* **Alpha ($\alpha$):** Hệ số tỉ lệ (scaling factor). Trong thực tế, đầu ra của LoRA được nhân với một tỉ lệ $\frac{\alpha}{r}$. Alpha giúp kiểm soát tầm ảnh hưởng của LoRA adapter lên trọng số gốc. Quy tắc ngón tay cái thông dụng là đặt $\alpha = 2 \times r$.
